...
class NegationDetector:
    ...
    def is_action_negated(
        self, action_type: str, prompt: str
    ) -> Tuple[bool, float, Optional[NegationType]]:
        """
        ...
        for negation in negations:
            ...
            if (
                "*" in negation["action_scope"]
                or action_type in negation["action_scope"]
            ):
...
class FallbackLLM:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.enable_negation_detection = self.config.get(
        ...
        try:
            ...
            for action in actions:
                if action.is_negated and self.skip_negated_actions:
                    action.skip_reason = (
                        f"Action negated with {action.negation_strength:.2f} strength"
                    )
    ...
    def _extract_action_parameters(
        ...
        if "timeframe" in normalized_prompt:
            timeframe_match = re.search(
                r"\b(1m|5m|15m|30m|1h|4h|1d|1w|1M)\b", normalized_prompt
            )
            if timeframe_match:
        ...
        if action_type == "forecast":
            if "model" in normalized_prompt:
                model_match = re.search(
                    r"\b(lstm|arima|xgboost|prophet|ensemble)\b", normalized_prompt
                )
                if model_match:
            ...
            if "metric" in normalized_prompt:
                metric_match = re.search(
                    r"\b(accuracy|precision|recall|f1|profit|sharpe)\b",
                    normalized_prompt,
                )
                if metric_match:
    ...
    def _apply_negation_detection(
        self, actions: List[ParsedAction], prompt: str
    ) -> List[ParsedAction]:
    ...
    def _generate_warnings(self, actions: List[ParsedAction], prompt: str) -> List[str]:
        ...
        action_types = [
            action.action_type for action in actions if not action.is_negated
        ]
        ...
        low_confidence_actions = [
            action for action in actions if action.confidence < self.min_confidence
        ]
        if low_confidence_actions:
            warnings.append(
                f"{len(low_confidence_actions)} actions have low confidence"
            )
        ...
        if negated_actions:
            warnings.append(
                f"{len(negated_actions)} actions are negated and will be skipped"
            )
    ...
    def should_skip_action(self, action_type: str, prompt: str) -> Tuple[bool, str]:
        ...
        if is_negated and self.skip_negated_actions:
            reason = (
                f"Action negated with {strength:.2f} strength ({negation_type.value})"
            )
            return True, reason
    ...
    def get_negation_summary(self, prompt: str) -> Dict[str, Any]:
        ...
        if negations:
            ...
            for negation in negations:
                ...
                summary["negation_types"][neg_type] = (
                    summary["negation_types"].get(neg_type, 0) + 1
                )
