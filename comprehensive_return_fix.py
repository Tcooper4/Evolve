...
class ReturnStatementFixer:
    def __init__(self):
        ...
        self.exclude_dirs = {
            ...
            'htmlcov', 'coverage', '.mypy_cache', '.tox'
        }
    ...
    def should_scan_file(self, filepath: Path) -> bool:
        ...
        project_dirs = {
            'trading', 'core', 'utils', 'services', 'strategies', 
            'models', 'data', 'execution', 'evaluation', 'memory',
            'risk', 'logs', 'ui', 'config', 'pages', 'scripts',
            'src', 'system', 'memory', 'archive'
        }
            ...
            if isinstance(child, ast.Return):
                ...
                if (isinstance(child.value, ast.Dict) and 
                    ...
                    analysis['has_structured_return'] = True
                      ...
                      child.value.func.id in ['dict', 'Dict']):
                    analysis['has_structured_return'] = True
    ...
    def generate_error_return(self, error_msg: str = 'Operation failed') -> str:
        ...
        timestamp = "datetime.now().isoformat()"
        return {'success': True, 'result': f"return {{'success': False, 'error': '{error_msg}', 'timestamp': {timestamp}}}", 'message': 'Operation completed successfully', 'timestamp': datetime.now().isoformat()}
