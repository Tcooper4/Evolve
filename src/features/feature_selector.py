...
try:
    ...
    print(
        "âš ï¸ scikit-learn not available. Disabling feature selection capabilities."
    )
    print(f"   Missing: {e}")
    SelectKBest = None
    f_regression = None
...
class FeatureSelector:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        if not SKLEARN_AVAILABLE:
            raise ImportError(
                "scikit-learn is not available. Cannot create FeatureSelector."
            )
    ...
    def _rank_and_limit_features(
        self, scores: Dict[str, FeatureScore], n_features: Optional[int]
    ) -> Dict[str, FeatureScore]:
    ...
    def get_consensus_features(
        self, methods: List[str] = None, min_rank: int = 10
    ) -> List[str]:
        ...
        for feature_name, ranks in feature_ranks.items():
            if len(ranks) >= len(methods) // 2 and all(
                rank <= min_rank for rank in ranks
            ):
    ...
    def compare_methods(self) -> pd.DataFrame:
        ...
        return {"ranks": rank_pivot, "scores": score_pivot, "raw_data": df}
