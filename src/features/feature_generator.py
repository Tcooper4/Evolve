...
class FeatureGenerator:
    ...
    def generate_features(
        ...
        for feature_name in feature_list:
            ...
            try:
                ...
                if nan_count > 0:
                    self.logger.warning(f"Feature {feature_name} has {nan_count} NaN values")
                    ...
                    if handle_nans == "drop":
                        ...
                        self.logger.info(f"Dropped feature {feature_name} due to {nan_count} NaN values")
                        continue
        ...
        metadata = {
            ...
            "features_generated": len([f for f in feature_list if f in feature_df.columns]),
    ...
    def _generate_single_feature(self, data: pd.DataFrame, feature_name: str) -> pd.Series:
        ...
        if feature_name == "rsi":
            ...
            return self._calculate_volatility(data["Close"], config.parameters["period"])
    ...
    def _calculate_bollinger_upper(self, prices: pd.Series, params: Dict[str, int]) -> pd.Series:
    ...
    def _calculate_bollinger_lower(self, prices: pd.Series, params: Dict[str, int]) -> pd.Series:
    ...
    def _log_feature_generation_summary(self, feature_df: pd.DataFrame, feature_list: List[str]):
