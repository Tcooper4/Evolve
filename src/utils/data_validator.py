        ...
        if handle_multi_index and self._is_multi_index(df):
            ...
            if self.support_multi_index:
                ...
                return ValidationResult(
                    is_valid=False, issues=issues, warnings=warnings, metadata=metadata
                )
    ...
    def _check_missing_values(self, df: pd.DataFrame) -> Dict[str, List[str]]:
        ...
        for column in df.columns:
            ...
            if missing_count == len(df):
                ...
                issues.append(
                    f"Column '{column}' has {missing_percentage:.1f}% missing values (>{self.max_missing_percentage}%)"
                )
            elif missing_count > 0:
                warnings.append(
                    f"Column '{column}' has {missing_count} missing values ({missing_percentage:.1f}%)"
                )
    ...
    def _check_duplicates(self, df: pd.DataFrame) -> Dict[str, List[str]]:
        ...
        if duplicate_count > 0:
            ...
            if duplicate_percentage > 10:
                issues.append(
                    f"High duplicate rate: {duplicate_percentage:.1f}% ({duplicate_count} rows)"
                )
            else:
                warnings.append(
    ...
    def _check_zero_std_columns(self, df: pd.DataFrame) -> Dict[str, List[str]]:
        ...
        for column in numeric_columns:
            ...
            if std_value <= self.zero_std_threshold:
                issues.append(
                    f"Column '{column}' has zero standard deviation (std={std_value})"
                )
            elif std_value < 0.01:
                warnings.append(
                    f"Column '{column}' has very low standard deviation (std={std_value})"
                )
