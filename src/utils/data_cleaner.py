...
class DataCleaner:
    ...
    def _initialize_cleaning_config(self) -> CleaningConfig:
        ...
        return CleaningConfig(
            ...
            optional_columns=self.config.get(
                "optional_columns", ["Volume", "Open", "High", "Low"]
            ),
    ...
    def clean_data(
        ...
        cleaning_metadata["columns_cleaned"].extend(
            optional_metadata["columns_cleaned"]
        )
    ...
    def _handle_critical_columns(
        self, df: pd.DataFrame, critical_columns: List[str], context: Optional[str]
    ) -> Tuple[pd.DataFrame, Dict[str, Any]]:
        ...
        if rows_removed > 0:
            self.logger.info(
                f"Removed {rows_removed} rows due to NaN values in critical columns: {existing_critical}"
            )
    ...
    def _handle_optional_columns(
        self, df: pd.DataFrame, fill_method: str
    ) -> Tuple[pd.DataFrame, Dict[str, Any]]:
        ...
        existing_optional = [
            col for col in self.cleaning_config.optional_columns if col in df.columns
        ]
        ...
        for column in numeric_columns:
            if column in df.columns and df[column].isna().sum() < len(df):
                ...
                if outliers.sum() > 0:
                    ...
                    self.logger.info(
                        f"Removed {outliers.sum()} outliers from column '{column}'"
                    )
    ...
    def clean_for_prediction(
        ...
        return self.clean_data(
            df, context="prediction", critical_columns=["Close"], fill_method="ffill"
        )

    def clean_for_backtest(
        self, df: pd.DataFrame
    ) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    ...
    def get_cleaning_stats(self) -> Dict[str, Any]:
        ...
        total_columns_removed = sum(
            h["total_columns_removed"] for h in self.cleaning_history
        )
        ...
        return {
            ...
            "average_rows_removed": (
                total_rows_removed / total_cleanings if total_cleanings > 0 else 0
            ),
            "average_columns_removed": (
                total_columns_removed / total_cleanings if total_cleanings > 0 else 0
            ),
        }
