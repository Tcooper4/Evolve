...
class ParameterTuner:
    ...
    def load_config_from_cli_args(self, args: Optional[List[str]] = None) -> None:
        ...
        if parsed_args.parameter_ranges:
            config["parameter_ranges"] = self._parse_parameter_ranges(
                parsed_args.parameter_ranges
            )
    ...
    def _create_argument_parser(self) -> argparse.ArgumentParser:
        ...
        parser.add_argument(
            "--max-trials", "-t", type=int, help="Maximum number of trials"
        )

        parser.add_argument("--timeout", "-o", type=int, help="Timeout in seconds")

        parser.add_argument("--n-jobs", "-j", type=int, help="Number of parallel jobs")

        parser.add_argument(
            "--random-state", "-r", type=int, help="Random state for reproducibility"
        )
    ...
    def tune_parameters(
        self, objective_function: callable, data: pd.DataFrame, **kwargs
    ) -> Dict[str, Any]:
        ...
        self.logger.info(
            f"Starting parameter tuning with {self.tuning_config.algorithm}"
        )
        self.logger.info(
            f"Parameter ranges: {list(self.tuning_config.parameter_ranges.keys())}"
        )
        ...
        if self.tuning_config.algorithm == "bayesian":
            result = self._bayesian_optimization(
                objective_function, param_space, data, **kwargs
            )
        ...
        **kwargs,
        ...
        if len(combinations) > self.tuning_config.max_trials:
            indices = np.random.choice(
                len(combinations), self.tuning_config.max_trials, replace=False
            )
    ...
    def _genetic_optimization(
        ...
        **kwargs,
        ...
        try:
            ...
            toolbox.register(
                "individual", tools.initIterate, creator.Individual, generate_param
            )
