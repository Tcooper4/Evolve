...
class MarketAnalysis:
    ...
    def _generate_signals(self, data: pd.DataFrame, indicators: Dict, regime: MarketRegime, conditions: List[MarketCondition]) -> Dict:
    ...
    def _generate_momentum_signals(self, data: pd.DataFrame, indicators: Dict, regime: MarketRegime) -> Dict:
        ...
        if signals['rsi'] == 'oversold' and signals['stochastic'] == 'oversold' and signals['cci'] == 'oversold':
            return {'signal': 'strong_buy', 'strength': 0.8}
        elif signals['rsi'] == 'overbought' and signals['stochastic'] == 'overbought' and signals['cci'] == 'overbought':
            return {'signal': 'strong_sell', 'strength': 0.8}
    ...
    def _generate_volatility_signals(self, data: pd.DataFrame, indicators: Dict, regime: MarketRegime) -> Dict:
        ...
        current_price = data['close'].iloc[-1]
        signals = {
            'bb': 'oversold' if current_price < bb['lower_band'].iloc[-1] else 'overbought' if current_price > bb['upper_band'].iloc[-1] else 'neutral',
            'kc': 'oversold' if current_price < kc['lower_band'].iloc[-1] else 'overbought' if current_price > kc['upper_band'].iloc[-1] else 'neutral',
    ...
    def _combine_signals(self, signals: Dict, regime: MarketRegime, conditions: List[MarketCondition]) -> Dict:
        ...
        buy_signals = sum(1 for signal in [trend_signal, momentum_signal, volatility_signal, volume_signal, sr_signal]
                         if signal['signal'] in ['buy', 'strong_buy'])
        sell_signals = sum(1 for signal in [trend_signal, momentum_signal, volatility_signal, volume_signal, sr_signal]
