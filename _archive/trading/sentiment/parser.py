...
class SentimentParser:
    ...
    def __init__(
        ...
        logger.info(
            f"SentimentParser initialized with financial symbol preservation: {preserve_financial_symbols}"
        )
    ...
    def _extract_financial_symbols(
        self, text: str
    ) -> Tuple[List[Dict[str, Any]], FinancialContext]:
        """
    ...
    def _clean_text_preserving_symbols(
        self, text: str, preserved_symbols: List[Dict[str, Any]]
    ) -> str:
        """
        ...
        sorted_symbols = sorted(
            preserved_symbols, key=lambda x: x["start_pos"], reverse=True
        )
        ...
        for i, symbol_info in enumerate(sorted_symbols):
            ...
            cleaned_text = (
                cleaned_text[:start_pos] + placeholder + cleaned_text[end_pos:]
            )
    ...
    def _get_context_window(
        self, text: str, start_pos: int, end_pos: int, window_size: int = 20
    ) -> str:
        """
    ...
    def get_parse_statistics(self) -> Dict[str, Any]:
        ...
        if stats["total_parses"] > 0:
            stats["avg_tickers_per_parse"] = (
                stats["tickers_found"] / stats["total_parses"]
            )
            stats["avg_hashtags_per_parse"] = (
                stats["hashtags_found"] / stats["total_parses"]
            )
            stats["avg_percentages_per_parse"] = (
                stats["percentages_found"] / stats["total_parses"]
            )
            stats["avg_currencies_per_parse"] = (
                stats["currencies_found"] / stats["total_parses"]
            )
            stats["avg_prices_per_parse"] = (
                stats["prices_found"] / stats["total_parses"]
            )
    ...
    def enable_financial_symbol_preservation(self, enable: bool = True):
        ...
        logger.info(
            f"Financial symbol preservation {'enabled' if enable else 'disabled'}"
        )
