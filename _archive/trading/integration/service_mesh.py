...
class ServiceHealthMonitor:
    ...
    async def register_service(
        ...
        self.logger.info(
            f"âœ… Service '{service_name}' registered for health monitoring"
        )
    ...
    async def unregister_service(self, service_name: str):
        ...
        self.logger.info(
            f"âœ… Service '{service_name}' unregistered from health monitoring"
        )
            ...
            try:
                async with aiohttp.ClientSession() as session:
                    async with session.get(
                        f"{service_info.endpoint}/health", timeout=5.0
                    ) as response:
                ...
                self.logger.error(
                    f"Default health check failed for '{service_name}': {e}"
                )
                service_info.status = ServiceStatus.UNHEALTHY
                service_info.error_count += 1
                service_info.last_health_check = datetime.now()
...
class LoadBalancer:
    ...
    def _least_loaded_select(self, services: List[ServiceInfo]) -> ServiceInfo:
        ...
        return min(
            services, key=lambda s: s.current_requests / s.max_concurrent_requests
        )
...
class ServiceMesh:
        ...
        **kwargs,
        ...
        try:
            ...
            await self.health_monitor.register_service(
                service_name, service_info, health_checker
            )
            ...
            if service_name in self.services:
                ...
                if self.redis_client:
                    await self._publish_service_event(
                        "service_unregistered", {"name": service_name}
                    )

                self.logger.info(
                    f"âœ… Service '{service_name}' unregistered successfully"
                )
                return True
            else:
    ...
    async def _execute_request(
        self, service: ServiceInfo, request: ServiceRequest
    ) -> ServiceResponse:
        ...
        return {
            ...
            "services": {
                name: {
                    "status": status.value,
                    "info": (
                        self.services[name].to_dict() if name in self.services else None
                    ),
                }
            ...
            "healthy_services": len(
                [s for s in health_statuses.values() if s == ServiceStatus.HEALTHY]
            ),
            "timestamp": datetime.now().isoformat(),
        }
    ...
    def get_service_info(self, service_name: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "health_status": self.health_monitor.get_all_health_statuses().get(
                service_name
            ),
...
async def register_forecasting_service(
    mesh: ServiceMesh, service_name: str, endpoint: str
) -> bool:
