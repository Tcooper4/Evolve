...
from portfolio import create_allocator, create_risk_manager
...
class MetaControllerAgent(BaseAgent):
    ...
    def _evaluate_portfolio_rebalance(
        ...
        try:
            ...
            last_rebalance = self.portfolio_state.get(
    ...
    def _evaluate_allocation_optimization(
        self, metrics: SystemMetrics
    ) -> Optional[ActionDecision]:
        ...
        try:
            ...
            last_optimization = self.portfolio_state.get(
    ...
    async def execute_action(self, decision: ActionDecision) -> ActionResult:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to execute action {decision.action_type.value}: {e}"
            )
    ...
    async def _execute_sentiment_fetch(
        ...
        try:
            if self.sentiment_fetcher:
                ...
                for ticker in tickers:
                    try:
                        sentiment_data = self.sentiment_fetcher.fetch_all_sentiment(
                            ticker, hours_back=6
                        )
                        results[ticker] = {
                        ...
                        self.logger.warning(
                            f"Failed to fetch sentiment for {ticker}: {e}"
                        )
    ...
    async def _execute_system_health_check(
        self, decision: ActionDecision
    ) -> Dict[str, Any]:
            ...
            if component == "models":
                if self.weight_registry:
                    ...
                    if weights:
                        return {
                            "status": "healthy",
                            "message": f"{len(weights)} models active",
                        }
                    else:
                    ...
                    return {
                        "status": "error",
                        "message": "Weight registry not available",
                    }
                ...
                if self.strategy_agent:
                    ...
                    return {
                        "status": "error",
                        "message": "Strategy agent not available",
                    }
                ...
                if self.sentiment_fetcher and self.sentiment_analyzer:
                    return {
                        "status": "healthy",
                        "message": "Sentiment components active",
                    }
                else:
                    return {
                        "status": "error",
                        "message": "Sentiment components not available",
                    }
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if self.system_metrics:
            ...
            if len(recent_metrics) >= 2:
                ...
                latest_perf = (
                    np.mean(list(recent_metrics[-1].model_performance.values()))
                    if recent_metrics[-1].model_performance
                    else 0
                )
                prev_perf = (
                    np.mean(list(recent_metrics[-2].model_performance.values()))
                    if recent_metrics[-2].model_performance
                    else 0
                )
        ...
        if recent_actions:
            error_rate = sum(
                1 for action in recent_actions if not action.success
            ) / len(recent_actions)
    ...
    def _update_market_state(self):
        ...
        try:
            ...
            if latest_metrics and latest_metrics.market_conditions:
                self.market_state["volatility"] = latest_metrics.market_conditions.get(
                    "volatility", 0
                )
                self.market_state["trend"] = (
                    "bullish"
                    if latest_metrics.market_conditions.get("trend_strength", 0) > 0.5
                    else "bearish"
                )
                self.market_state["volume"] = latest_metrics.market_conditions.get(
                    "volume_ratio", 1.0
                )
