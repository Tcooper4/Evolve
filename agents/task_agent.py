...
from trading.memory.agent_logger import AgentAction, AgentLogger, LogLevel
    ...
    async def execute(self, context: TaskContext, **kwargs) -> TaskResult:
        ...
        try:
            ...
            eval_request = ModelEvaluationRequest(
                ...
                test_data_path=context.parameters.get(
            ...
            if eval_result.success:
                ...
                return TaskResult(
                    ...
                    next_action=(
                        ActionType.UPDATER
                        if performance_score < context.performance_threshold
                        else ActionType.STOP_AND_REPORT
                    ),
                ...
                if update_result.success:
                    return TaskResult(
                        ...
                        data={
                            ...
                            "improvement_metrics": update_result.data.get(
                                "improvement_metrics", {}
                            ),
                        },
...
class ScorePerformanceStrategy(ActionStrategy):
    ...
    async def execute(self, context: TaskContext, **kwargs) -> TaskResult:
        ...
        try:
            ...
            current_score = (
                context.performance_history[-1]["score"]
            ...
            if context.performance_history:
                ...
                if last_score < 0.4:
                    current_params["learning_rate"] = (
                        current_params.get("learning_rate", 0.01) * 1.5
                    )
                    current_params["epochs"] = current_params.get("epochs", 100) + 50
                elif last_score < 0.6:
                    current_params["batch_size"] = max(
                        16, current_params.get("batch_size", 32) // 2
                    )
...
class TaskAgent:
    ...
    async def _execute_task_recursive(self, context: TaskContext) -> TaskResult:
        ...
        if context.depth == 0:
            ...
            action_type = context.action_history[-1].get(
                "next_action", ActionType.STOP_AND_REPORT
            )
    ...
    async def _execute_action(
        self, context: TaskContext, action_type: ActionType
    ) -> TaskResult:
        """
        ...
        try:
            ...
            await self.prompt_memory.log_prompt(
                ...
                result={
                    ...
                    "message": result.message,
                    "next_action": (
                        result.next_action.value if result.next_action else None
                    ),
                    "depth": context.depth,
    ...
    def _log_action(
        self, context: TaskContext, action_type: ActionType, result: TaskResult
    ):
    ...
    def _map_task_parameters(
        ...
        if task_type == TaskType.FORECAST:
            ...
            mapped_params.setdefault(
                "hyperparameters",
                {"epochs": 100, "batch_size": 32, "lookback_window": 60},
            )
            ...
            mapped_params.setdefault(
                "hyperparameters",
                {"n_estimators": 200, "max_depth": 8, "subsample": 0.8},
            )
...
async def execute_forecast_task(
    prompt: str, parameters: Optional[Dict[str, Any]] = None, **kwargs
) -> TaskResult:
