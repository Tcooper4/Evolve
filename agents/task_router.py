...
class TaskRouter:
    ...
    def parse_multi_intent_prompt(
        self, prompt: str, context: Optional[TaskContext] = None
    ) -> MultiIntentPrompt:
    ...
    def _create_task_definition(
        self, prompt: str, task_type: TaskType, context: TaskContext
    ) -> TaskDefinition:
    ...
    def _determine_priority(
        self, task_type: TaskType, context: TaskContext
    ) -> TaskPriority:
        ...
        for task in task_chain:
            ...
            for other_task in task_chain:
                if other_task.task_id != task.task_id:
                    if other_task.task_type in self.task_dependencies.get(
                        task.task_type, []
                    ):
    ...
    def _build_dependencies(
        self, task_chain: List[TaskDefinition]
    ) -> Dict[str, List[str]]:
    ...
    async def execute_task_chain(
        self, multi_intent: MultiIntentPrompt
    ) -> Dict[str, Any]:
        ...
        """
        self.logger.info(
            f"Executing task chain with {len(multi_intent.task_chain)} tasks"
        )
        ...
        for task_id in multi_intent.execution_order:
            ...
            if not self._check_dependencies(
                task_id, multi_intent.dependencies, results
            ):
            ...
            try:
                ...
                result = TaskResult(
                    task_id=task_id, success=False, error_message=str(e)
                )
                execution.result = result
                results[task_id] = result
    ...
    async def _execute_single_task(
        self, task: TaskDefinition, context: TaskContext
    ) -> TaskResult:
    ...
    async def _execute_forecast_task(
        self, task: TaskDefinition, context: TaskContext
    ) -> Dict[str, Any]:
    ...
    async def _execute_portfolio_task(
        self, task: TaskDefinition, context: TaskContext
    ) -> Dict[str, Any]:
    ...
    def _aggregate_results(
        self, results: Dict[str, TaskResult], multi_intent: MultiIntentPrompt
    ) -> Dict[str, Any]:
        ...
        if successful_results:
            summary["synthesized_response"] = self._synthesize_response(
                successful_results, multi_intent
            )

        return summary

    def _synthesize_response(
        self, results: Dict[str, Any], multi_intent: MultiIntentPrompt
    ) -> str:
        ...
        if "report" in results:
            ...
            response_parts.append(
                f"Report summary: {report.get('summary', 'Analysis completed')}"
            )
...
async def route_and_execute(
    prompt: str, context: Optional[TaskContext] = None
) -> Dict[str, Any]:
