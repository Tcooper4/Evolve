...
class AsyncModelLoader:
    ...
    def _load_default_configs(self) -> None:
        ...
        self.configs = {
            ...
            "lstm-forecast": ModelConfig(
                name="lstm-forecast",
                provider="pytorch",  # Changed from tensorflow - PyTorch only
                model_type="lstm",
                priority=1,
                preload=True,
                async_loading=True,
        ...
        try:
            ...
            async with self.model_locks[model_name]:
                ...
                if config.provider == "openai":
                    ...
                    raise ValueError(
                        "TensorFlow models are not supported. This codebase uses PyTorch only. "
                        "Please use 'pytorch', 'huggingface', or 'xgboost' as the provider."
                    )
    ...
    def _verify_model_sync(self, model_name: str) -> bool:
        ...
        try:
            ...
            if config.provider == "openai":
                ...
                # TensorFlow not supported - PyTorch only
                return False
    ...
    async def _load_tensorflow_model_async(self, config: ModelConfig) -> None:
        """TensorFlow model loading removed - PyTorch only.
        
        This method is kept for backward compatibility but raises an error.
        Use PyTorch models instead.
        """
        raise ValueError(
            "TensorFlow models are not supported. This codebase uses PyTorch only. "
            "Please convert your model to PyTorch format or use a PyTorch-compatible provider."
        )
    ...
    async def _check_model_health(self, model_name: str) -> bool:
        ...
        try:
            ...
            if model_info["provider"] == "huggingface":
                ...
                # TensorFlow not supported - PyTorch only
                logger.warning(f"TensorFlow model {model_name} not supported - PyTorch only")
                return False
