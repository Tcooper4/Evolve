...
class QuantGPTCommentaryAgent:
    ...
    async def generate_commentary(
        ...
        try:
            ...
            if request.request_type == CommentaryType.TRADE_EXPLANATION:
                ...
                response = await self._perform_counterfactual_analysis(
                    request, request_id
                )
    ...
    def _analyze_trade_context(
        self, trade_data: Dict[str, Any], market_data: pd.DataFrame
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if market_data is not None and not market_data.empty:
                ...
                context["trend"] = (
                    "up" if sma_short.iloc[-1] > sma_long.iloc[-1] else "down"
                )

                # Volume analysis
                if "volume" in market_data.columns:
                    context["volume_trend"] = (
                        market_data["volume"].tail(10).mean()
                        / market_data["volume"].tail(50).mean()
                    )

                # Support/resistance levels
                context["support_resistance"] = self._calculate_support_resistance(
                    market_data
                )
    ...
    async def _detect_overfitting(
        ...
        try:
            ...
            prompt = self._create_overfitting_detection_prompt(
                model_data, overfitting_analysis
            )
            ...
            overfitting_indicators = self._extract_overfitting_indicators(
                model_data, overfitting_analysis
            )
            ...
            if overfitting_score > self.overfitting_threshold:
                self._store_overfitting_alert(
                    request.symbol, overfitting_analysis, overfitting_score
                )
            ...
            return CommentaryResponse(
                ...
                - overfitting_score,  # Higher overfitting = lower confidence
                metadata={
    ...
    def _analyze_overfitting_patterns(
        ...
        try:
            ...
            if train_performance and val_performance:
                ...
                analysis["performance_gap_ratio"] = performance_gap / max(
                    val_metric, 1e-6
                )
            ...
            if num_parameters > 0 and num_samples > 0:
                ...
                analysis["complexity_risk"] = (
                    "high" if num_parameters / num_samples > 0.1 else "low"
                )
    ...
    def _calculate_oos_degradation(
        self, train_perf: Dict[str, Any], oos_perf: Dict[str, Any]
    ) -> float:
    ...
    def _perform_regime_analysis(self, market_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            analysis["trend"] = {
                ...
                "strength": abs(sma_short.iloc[-1] - sma_long.iloc[-1])
                / sma_long.iloc[-1],
            }
            ...
            if "volume" in market_data.columns:
                analysis["volume"] = {
                    "trend": market_data["volume"].tail(10).mean()
                    / market_data["volume"].tail(50).mean(),
    ...
    def _analyze_regime_implications(
        ...
        try:
            ...
            regime_analysis.get("volatility", {})
            regime_analysis.get("trend", {})

            if regime == "high_volatility":
                implications.append(
                    "High volatility suggests increased risk and potential for large price swings"
                )
                implications.append(
                    "Consider reducing position sizes and implementing tighter stop-losses"
                )
                implications.append(
                    "Volatility-based strategies may perform better in this environment"
                )

            elif regime == "low_volatility":
                implications.append(
                    "Low volatility suggests reduced risk but also limited profit potential"
                )
                implications.append(
                    "Trend-following strategies may struggle in low volatility environments"
                )
                implications.append(
                    "Consider mean-reversion strategies or options strategies"
                )

            elif regime == "trending_up":
                implications.append(
                    "Upward trend suggests momentum strategies may be effective"
                )
                implications.append(
                    "Consider trend-following indicators and breakout strategies"
                )
                implications.append("Be cautious of potential trend reversals")

            elif regime == "trending_down":
                implications.append(
                    "Downward trend suggests defensive positioning may be appropriate"
                )
                implications.append(
                    "Consider short-selling strategies or hedging positions"
                )
                implications.append(
                    "Look for oversold conditions and potential reversal signals"
                )
    ...
    async def _perform_counterfactual_analysis(
        ...
        try:
            ...
            counterfactual_results = self._analyze_counterfactual_scenarios(
                trade_data, request.market_data
            )

            # Generate counterfactual prompt
            prompt = self._create_counterfactual_analysis_prompt(
                trade_data, counterfactual_results
            )
            ...
            return CommentaryResponse(
                ...
                key_insights=key_insights,
                recommendations=recommendations,
                risk_warnings=self._extract_counterfactual_risks(
                    counterfactual_results
                ),
    ...
    def _analyze_counterfactual_scenarios(
        self, trade_data: Dict[str, Any], market_data: pd.DataFrame
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            scenarios["timing_analysis"] = self._analyze_timing_scenarios(
            ...
            scenarios["exit_analysis"] = self._analyze_exit_scenarios(
                trade_data, market_data
            )

            # Scenario 4: Alternative strategies
            scenarios["strategy_alternatives"] = self._analyze_strategy_alternatives(
                trade_data, market_data
            )
    ...
    def _parse_llm_response(self, response: str) -> Dict[str, str]:
        ...
        try:
            ...
            return {
                "success": True,
                "result": {"summary": "Analysis completed", "analysis": response},
    ...
    def _extract_trade_insights(
        ...
        try:
            ...
            if signal_strength > 0.8:
                insights.append(
                    "Strong signal strength indicates high confidence in the trade"
                )
            ...
            if volatility > 0.03:
                insights.append(
                    "High volatility environment - consider position sizing"
                )
    ...
    def _generate_trade_recommendations(
        ...
        try:
            ...
            if regime == "high_volatility":
                ...
                recommendations.append(
                    "Look for additional entry opportunities in trend"
                )
    ...
    def _store_commentary(self, response: CommentaryResponse):
        ...
        try:
            self.memory.store(
                "commentary_history",
                {"response": response.__dict__, "timestamp": datetime.now()},
            )
        except Exception as e:
            self.logger.error(f"Error storing commentary: {str(e)}")

    def _store_overfitting_alert(
        self, symbol: str, analysis: Dict[str, Any], score: float
    ):
        ...
        try:
            ...
            return {
                ...
                "commentary_types": list(
                    set([c.commentary_type.value for c in commentaries])
                ),
