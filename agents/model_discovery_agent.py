...
class ModelDiscoveryAgent(BaseAgent):
    ...
    async def execute(self, **kwargs) -> AgentResult:
        ...
        try:
            ...
            if not discovered_models:
                self.logger.warning(
                    "No models discovered, falling back to pre-defined model list"
                )
            ...
            return AgentResult(
                ...
                data={
                    ...
                    "used_fallback": len(discovered_models) > 0
                    and discovered_models[0].get("status") == "predefined",
                },
                extra_metrics={
                    "discovery_rate": len(validated_models)
                    / max(len(discovered_models), 1),
    ...
    async def _discover_models(
        ...
        for model_type in model_types:
            ...
            existing_count = len(
                [m for m in self.model_registry.values() if m.get("type") == model_type]
            )
    ...
    def _generate_model_configs(
        self, model_type: str, validation_data: Optional[pd.DataFrame] = None
    ) -> List[Dict[str, Any]]:
        ...
        for model in models:
            if self._validate_model_config(model):
                ...
                if score >= self.validation_threshold:
                    ...
                    self.logger.warning(
                        f"Model {model['id']} failed validation: score {score:.3f} < {self.validation_threshold}"
                    )
            else:
    ...
    async def _register_models(self, models: List[Dict[str, Any]]) -> int:
        ...
        for model in models:
            try:
                ...
                self.logger.error(
                    f"Failed to register model {model.get('id', 'unknown')}: {e}"
                )
