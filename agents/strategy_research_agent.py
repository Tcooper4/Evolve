...
class StrategyResearchAgent(BaseAgent):
    ...
    def _analyze_github_repo(self, repo_name: str) -> Optional[Dict]:
        ...
        try:
            ...
            for item in contents:
                ...
                elif item['type'] == 'dir' and item['name'] in ['strategies', 'models', 'algorithms']:
                    # Recursively search strategy directories
                    sub_contents = self._get_github_dir_contents(repo_name, item['path'])
        ...
        try:
            ...
            for item in contents:
                if item['type'] == 'file' and item['name'].endswith('.py'):
                    ...
                    sub_files = self._get_github_dir_contents(repo_name, f"{path}/{item['name']}")
        ...
        try:
            ...
            if not requirements:
                ...
                if content:
                    ...
                    requirements = [pkg for pkg in packages if '==' in pkg or '>=' in pkg]
    ...
    def _extract_strategy_from_text(self, text: str) -> Dict[str, Any]:
        ...
        elif any(word in text_lower for word in ['mean reversion', 'reversion', 'oscillator']):
            strategy_type = "mean_reversion"
        elif any(word in text_lower for word in ['machine learning', 'ml', 'neural', 'deep learning']):
            strategy_type = "ml"
        ...
        technical_indicators = ['sma', 'ema', 'rsi', 'macd', 'bollinger', 'stochastic', 'atr']
        ...
        # Extract tags
        tags = []
        tag_keywords = ['python', 'pandas', 'numpy', 'scikit-learn', 'tensorflow', 'pytorch', 
                       'backtesting', 'live trading', 'paper trading']
...
'''
            ...
            strategy_template = strategy_template.replace('{self._generate_strategy_logic(discovery)}', strategy_logic)
    ...
    def test_discovered_strategy(self, discovery: StrategyDiscovery) -> Dict[str, Any]:
        ...
        try:
            ...
            strategy_filename = f"discovered_{discovery.title.replace(' ', '_')[:30]}.py"
            ...
            spec = importlib.util.spec_from_file_location("discovered_strategy", str(strategy_path))
    ...
    def run_research_scan(self) -> List[StrategyDiscovery]:
        ...
        for source_name, search_func in sources:
            try:
                ...
                self.logger.info(f"Found {len(new_discoveries)} new strategies from {source_name}")
        ...
        self.logger.info(f"Research scan complete. Found {len(all_discoveries)} total new strategies.")
            ...
            try:
                ...
                for discovery in discoveries:
                    if discovery.confidence_score > 0.5:
                        self.logger.info(f"Testing high-confidence strategy: {discovery.title}")
                        test_results = self.test_discovered_strategy(discovery)
                        self.test_results[discovery.title] = test_results
                        
                        # Save test results
                        results_file = self.discovered_dir / f"test_results_{discovery.title[:30]}.json"
    ...
    def get_discovery_summary(self) -> Dict[str, Any]:
        ...
        recent = [
            discovery for discovery in self.discovered_strategies
