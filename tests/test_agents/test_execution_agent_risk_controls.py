...
class TestRiskControlsIntegration:
    ...
    def test_stop_loss_and_max_drawdown_triggers(self, execution_agent, sample_market_data):
        ...
        print(
            f"  ✅ Max drawdown triggered: current={execution_agent.portfolio_state['current_drawdown']:.1%}, max={execution_agent.portfolio_state['max_drawdown']:.1%}"
        )
        ...
        print(
            f"  ✅ Daily loss limit triggered: daily_pnl={execution_agent.portfolio_state['daily_pnl']}, max_daily_loss={execution_agent.portfolio_state['max_daily_loss']}"
        )
