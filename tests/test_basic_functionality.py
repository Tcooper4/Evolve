...
class TestBasicFunctionality(unittest.TestCase):
    """Test basic functionality of the trading system."""
    ...
    def setUp(self):
        ...
        self.sample_data['Close'] += trend
        self.sample_data['Open'] += trend
        self.sample_data['High'] += trend
        self.sample_data['Low'] += trend
    ...
    def test_data_validation(self):
        ...
        for col in price_columns:
            self.assertTrue(all(self.sample_data[col] > 0))
    ...
    def test_basic_indicators(self):
        ...
        self.assertTrue(all(pd.notna(sma_20.iloc[19:])))  # First 19 values should be NaN
        
        # Test Exponential Moving Average
        ema_20 = self.sample_data['Close'].ewm(span=20).mean()
        self.assertEqual(len(ema_20), len(self.sample_data))
        self.assertTrue(all(pd.notna(ema_20)))
    ...
    def test_signal_generation(self):
        ...
        sma_short = self.sample_data['Close'].rolling(window=10).mean()
        sma_long = self.sample_data['Close'].rolling(window=30).mean()
        
        # Generate signals
        signals = pd.Series(0, index=self.sample_data.index)
    ...
    def test_risk_metrics(self):
        ...
        self.assertLess(max_drawdown, 0)  # Max drawdown should be negative
    ...
    def test_data_preprocessing(self):
        ...
        self.assertFalse(np.isinf(clean_data).any().any())
