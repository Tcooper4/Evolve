...
class TestAlphaGenAgent:
    ...
    def agent_config(self):
        ...
        return AgentConfig(
            name="test_alphagen", custom_config={"openai_api_key": "test_key"}
        )
    ...
    async def test_execute_success(self, agent):
        ...
        with (
            patch.object(agent, "_analyze_market_context") as mock_analyze,
...
class TestSentimentIngestion:
    ...
    def agent_config(self):
        ...
        return AgentConfig(
            ...
            custom_config={
                ...
                "twitter": {"bearer_token": "test_token"},
            },
        )
    ...
    def agent(self, agent_config):
        ...
        with (
            patch("trading.agents.alpha.sentiment_ingestion.praw.Reddit"),
            patch("trading.agents.alpha.sentiment_ingestion.tweepy.Client"),
        ):
    ...
    async def test_execute_success(self, agent):
        """Test successful execution."""
        with (
            patch.object(agent, "_collect_reddit_sentiment") as mock_reddit,
...
class TestAlphaOrchestrator:
    ...
    def agent_config(self):
        ...
        return AgentConfig(
            ...
            custom_config={
                "alphagen": {"openai_api_key": "test_key"},
                "signal_tester": {
                    "test_config": {
                        "start_date": (datetime.now() - timedelta(days=30)).isoformat(),
                        "end_date": datetime.now().isoformat(),
                ...
                "alpha_registry": {"database": ":memory:"},
            },
            sentiment_ingestion={"tickers": []},
        )
    ...
    def agent(self, agent_config):
        ...
        with (
            patch("trading.agents.alpha.alpha_orchestrator.AlphaGenAgent"),
            patch("trading.agents.alpha.alpha_orchestrator.SignalTester"),
            patch("trading.agents.alpha.alpha_orchestrator.RiskValidator"),
            patch("trading.agents.alpha.alpha_orchestrator.AlphaRegistry"),
            patch("trading.agents.alpha.alpha_orchestrator.SentimentIngestion"),
        ):
    ...
    async def test_execute_full_cycle(self, agent):
        ...
        with (
            patch.object(agent.sentiment_ingestion, "execute") as mock_sentiment,
            ...
            mock_registry.return_value = AgentResult(
                success=True,
                data={"strategy_id": "strategy_001", "status": "registered"},
            )
