...
from trading.strategies.ensemble import EnsembleConfig, WeightedEnsembleStrategy
...
def test_fallback_on_all_none(ensemble):
    ...
    idx = pd.date_range("2023-01-01", periods=3)
    signals = {
        "model_a": make_signal_df(1.0, 0.9, idx),
        "model_b": make_signal_df(-1.0, 0.7, idx),
    }
...
def test_fallback_on_all_nan(ensemble):
    ...
    nan_df = pd.DataFrame(
        {"signal": [np.nan] * 3, "confidence": [np.nan] * 3}, index=idx
    )
...
def test_fallback_on_model_error(ensemble):
    ...
    try:
        ensemble.combine_signals(signals_error)
    except Exception:
