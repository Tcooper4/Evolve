...
SAMPLE_PROMPTS_PATH = os.path.join(
    os.path.dirname(__file__), "../../trading/nlp/sample_prompts.json"
)
...
class TestPromptProcessor(unittest.TestCase):
    ...
    def test_long_prompt_token_truncation(self):
        ...
        truncated_prompt, was_truncated = self.processor.truncate_prompt(
            long_prompt, max_tokens=max_tokens
        )
        ...
        truncated_exact, was_truncated_exact = self.processor.truncate_prompt(
            exact_prompt, max_tokens=max_tokens
        )
        self.assertFalse(
            was_truncated_exact, "Prompt at max_tokens should not be truncated"
        )
        ...
        truncated_below, was_truncated_below = self.processor.truncate_prompt(
            below_prompt, max_tokens=max_tokens
        )
        self.assertFalse(
        ...
        truncated_above, was_truncated_above = self.processor.truncate_prompt(
            above_prompt, max_tokens=max_tokens
        )
        self.assertTrue(
            was_truncated_above, "Prompt above max_tokens should be truncated"
        )
