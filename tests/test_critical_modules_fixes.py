...
from trading.llm.parser_engine import ParsedIntent, ParserEngine
...
class TestReportGenerator:
    ...
    def test_export_signals_defensive_checks(self):
        ...
        result = self.report_gen.export_signals(
            df, "test.csv", buy_col="Buy", sell_col="Sell"
        )
        assert result is False

        # Test with all NaN values
        df = pd.DataFrame(
            {"Buy": [np.nan, np.nan, np.nan], "Sell": [np.nan, np.nan, np.nan]}
        )
    ...
    def test_export_signals_success(self):
        ...
        df = pd.DataFrame(
            {"Buy": [1, 0, 1, 0], "Sell": [0, 1, 0, 1], "Close": [100, 101, 102, 103]}
        )
        ...
        df_custom = pd.DataFrame({"Long": [1, 0, 1, 0], "Short": [0, 1, 0, 1]})
        custom_path = os.path.join(self.temp_dir, "custom_signals.csv")
        result = self.report_gen.export_signals(
            df_custom, custom_path, buy_col="Long", sell_col="Short"
        )
        assert result is True
...
class TestBacktester:
    ...
    def test_signals_dataframe_processing(self):
        ...
        # Test forward fill
        processed = self.backtester.process_signals_dataframe(
            signals_df, fill_method="ffill"
        )
        ...
        # Test backward fill
        processed = self.backtester.process_signals_dataframe(
            signals_df, fill_method="bfill"
        )
        assert not processed.isna().any().any()

        # Test drop method
        processed = self.backtester.process_signals_dataframe(
            signals_df, fill_method="drop"
        )
        assert len(processed) < 5  # Should drop rows with NaN

        # Test zero fill
        processed = self.backtester.process_signals_dataframe(
            signals_df, fill_method="zero"
        )
        assert not processed.isna().any().any()
    ...
    def test_infinite_value_handling(self):
        ...
        signals_df = pd.DataFrame(
            {"Buy": [1, np.inf, 0, -np.inf, 1], "Sell": [0, 1, np.inf, 0, 1]}
        )
...
class TestModelLoader:
    ...
    def test_safe_from_pretrained(self):
        ...
        result = self.model_loader._safe_from_pretrained(
            mock_from_pretrained, "test_model"
        )
...
def test_production_readiness():
    ...
    try:
        ...
        assert isinstance(
            metrics, dict
        ), "Should return valid metrics even with invalid data"
