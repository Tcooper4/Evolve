...
class TestNewsSentimentCollector:
    ...
    async def collector(self):
        ...
        api_keys = {"newsapi": "test_newsapi_key", "gnews": "test_gnews_key"}
...
class TestMacroIndicatorCollector:
    ...
    async def test_get_macro_indicators_with_api_key(self, collector):
        ...
        with patch.object(collector, "_get_fred_series") as mock_fred:
            mock_fred.return_value = pd.Series(
                [100, 101, 102], index=pd.date_range("2024-01-01", periods=3)
            )
...
class TestExternalSignalsManager:
    ...
    async def test_get_all_signals_concurrent(self, manager):
        ...
        with patch.object(manager.news_collector, "get_news_sentiment") as mock_news:
            with patch.object(
                manager.twitter_collector, "get_twitter_sentiment"
            ) as mock_twitter:
                with patch.object(
                    manager.reddit_collector, "get_reddit_sentiment"
                ) as mock_reddit:
                    with patch.object(
                        manager.macro_collector, "get_macro_indicators"
                    ) as mock_macro:
                        with patch.object(
                ...
                with patch.object(
                    ...
                    with patch.object(
                        ...
                        with patch.object(
                            ...
                            assert "news_sentiment" in result
                            assert (
                                "twitter_sentiment" in result
                            )  # Should be empty list due to exception
                            assert "reddit_sentiment" in result
                            assert (
                                "macro_indicators" in result
                            )  # Should be empty dict due to exception
                            assert "options_flow" in result
    ...
    async def test_get_aggregated_sentiment(self, manager):
        ...
        with patch.object(manager, "get_all_signals") as mock_signals:
            ...
            mock_signals.return_value = {
                ...
                "twitter_sentiment": [Mock(sentiment_score=-0.2, volume=1)],
                "reddit_sentiment": [],
                "macro_indicators": {},
                "options_flow": [],
            }
...
class TestKubernetesDeployer:
    ...
    def test_create_namespace(self, mock_run):
        ...
        mock_results = [
            ...
            Mock(
                returncode=0, stdout="namespace created", stderr=""
            ),  # Creation succeeds
        ]
    ...
    def test_update_deployment_config(self, mock_exists, mock_open):
        ...
        deployer = KubernetesDeployer(
            "test-namespace", "test-image", registry="test-registry.com"
        )
...
class TestDeploymentScriptIntegration:
    ...
    def test_main_function_success(self, mock_deployer_class):
        ...
        with patch(
            "sys.argv",
            ["deploy_to_kube_batch.py", "--namespace", "test", "--image", "test"],
        ):
...
def run_performance_benchmark():
    ...
    print(
        f"Async strategy benchmark: {execution_time:.2f}s for 100 concurrent strategies"
    )
