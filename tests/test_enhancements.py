...
try:
    ...
    from trading.memory.state_manager import (
        StateManager,
        StateVersion,
    )
    from trading.signals.strategy_executor import (
        StrategyExecutor,
        TaskStatus,
    )
    ...
    async def test_strategy_executor_enhancements(self):
        ...
        try:
            ...
            executor = StrategyExecutor(
                max_queue_size=5, max_concurrent_tasks=2, default_timeout=5.0
            )
            ...
            for i in range(3):
                task_id = await executor.submit_task(
                    f"success_{i}", successful_strategy
                )
                if task_id:
            ...
            for i in range(10):
                task_id = await executor.submit_task(
                    f"overflow_{i}", successful_strategy
                )
                if task_id is None:
    ...
    async def test_integration_workflow(self):
        ...
        try:
            ...
            state_manager = StateManager(
                os.path.join(self.temp_dir, "integration_state.pkl")
            )
