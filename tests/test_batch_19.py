...
from trading.context_manager.trading_context import (
    StrategyType,
    TradingContextManager,
)
from trading.meta_learning.model_swapper import (
    ModelMetrics,
    ModelSwapper,
    ValidationResult,
)
...
from trading.nlp.prompt_bridge import IntentType, PromptBridge
from trading.signal_score.evaluator import SignalScoreEvaluator, SignalType
...
class TestTradingContextManager(unittest.TestCase):
    ...
    def test_session_creation(self):
        ...
        session_id = self.context_manager.create_session(
            user_id="test_user", max_age=timedelta(hours=2)
        )
    ...
    def test_session_expiration(self):
        ...
        session_id = self.context_manager.create_session(
            user_id="test_user", max_age=timedelta(seconds=1)
        )
...
class TestSignalScoreEvaluator(unittest.TestCase):
    ...
    def test_nan_protection(self):
        ...
        signal_data = {"rsi": np.nan, "current_price": 100.0, "sma_short": np.inf}

        score = self.evaluator.evaluate_signal(
            signal_data=signal_data, strategy_type="RSI"
        )
    ...
    def test_sma_signal_evaluation(self):
        ...
        score = self.evaluator.evaluate_signal(
            signal_data=signal_data, strategy_type="SMA"
        )
    ...
    def test_bollinger_signal_evaluation(self):
        ...
        score = self.evaluator.evaluate_signal(
            signal_data=signal_data, strategy_type="BB"
        )

        self.assertIsNotNone(score)
        self.assertLess(
            score.score, 0
        )  # Should be buy signal (negative score for strong buy)
    ...
    def test_custom_strategy_registration(self):
        ...
        success = self.evaluator.register_custom_strategy(
            "CUSTOM_RSI", custom_evaluator
        )
        self.assertTrue(success)

        # Test custom strategy evaluation
        score = self.evaluator.evaluate_signal(
            signal_data={"test": 1.0}, strategy_type="CUSTOM_RSI"
        )
...
class TestBatch19Integration(unittest.TestCase):
    ...
    def test_end_to_end_workflow(self):
        ...
        compound_prompt = (
            "analyze market data and apply RSI strategy with MACD confirmation"
        )
        ...
        success = model_swapper.add_candidate_model(
            model_id="test_candidate", model_type="LSTM", metrics=candidate_metrics
        )
        self.assertTrue(success)
