...
class TestProphetModel:
    ...
    def test_trend_detection(self, model):
        ...
        trend_data = pd.Series(np.linspace(100, 200, 100), index=dates)
    ...
    def test_missing_values_handling(self, model):
        ...
        try:
            ...
            assert (
                "NaN" in str(e) or "missing" in str(e).lower()
            ), f"Model should handle missing values gracefully, got: {e}"
    ...
    def test_multiple_seasonality(self, model):
        ...
        try:
            ...
            assert (
                "seasonality" in str(e).lower() or "pattern" in str(e).lower()
            ), f"Model should handle multiple seasonality, got: {e}"
    ...
    def test_holiday_integration(self, model):
        ...
        try:
            ...
            assert (
                "holiday" in str(e).lower() or "country" in str(e).lower()
            ), f"Holiday integration should work, got: {e}"
