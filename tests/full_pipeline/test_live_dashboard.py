...
class StreamlitOutputCapture:
    ...
    def capture_output(self, process):
        ...
        stdout_thread = threading.Thread(
            target=capture_stream, args=(process.stdout, "info")
        )
        stderr_thread = threading.Thread(
            target=capture_stream, args=(process.stderr, "error")
        )
...
class TestLiveDashboard:
    ...
    async def test_dashboard_port_conflict(self, free_port):
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            with patch("subprocess.Popen") as mock_popen:
                ...
                ), "Process should fail on port conflict"
    ...
    async def test_dashboard_logging(self, free_port):
        ...
        with patch("subprocess.Popen") as mock_popen:
            ...
            assert any(
                "Dashboard" in log for log in output_capture.logs
            ), "Should capture dashboard logs"
        ...
        with patch("requests.get") as mock_get:
            ...
            try:
                response = requests.get(
                    f"http://localhost:{free_port}/_stcore/health", timeout=5
                )
