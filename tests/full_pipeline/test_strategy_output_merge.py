...
class StrategyOutputMerger:
    ...
    def merge_strategy_outputs(
        ...
        for strategy_name, df in strategy_outputs.items():
            if df is not None and not df.empty:
                ...
                self.logger.warning(
                    f"Strategy {strategy_name} returned None or empty DataFrame"
                )
    ...
    def _merge_dataframes(
        self, strategy_outputs: Dict[str, pd.DataFrame], timestamp_column: str
    ) -> pd.DataFrame:
        ...
        for strategy_name, df in strategy_outputs.items():
            if timestamp_column not in df.columns:
                self.logger.warning(
                    f"Strategy {strategy_name} missing timestamp column"
                )
                continue
    ...
    def _resolve_conflicts(
        self, merged_df: pd.DataFrame, signal_column: str
    ) -> pd.DataFrame:
...
class TestStrategyOutputMerge:
    ...
    def sample_data(self):
        ...
        return {"RSI": rsi_data, "MACD": macd_data, "Bollinger": bollinger_data}
    ...
    def test_different_signal_values(self, merger):
        ...
        strategy_outputs = {
            "Strategy1": pd.DataFrame(
                {"timestamp": [base_time], "signal": [1]}  # Numeric signal
            ),
            "Strategy2": pd.DataFrame(
                {"timestamp": [base_time], "signal": ["BUY"]}  # String signal
            ),
        }
...
class MockStrategy:
    ...
    def generate_signals(self, timestamps: List[datetime]) -> pd.DataFrame:
        ...
        return pd.DataFrame(
            {"timestamp": timestamps, "signal": self.signals[: len(timestamps)]}
        )
...
class TestStrategyIntegration:
    ...
    def test_integration_with_mock_strategies(self):
        ...
        assert len(result) == 5
        assert all(
            col in result.columns for col in ["timestamp", "signal", "confidence"]
        )
