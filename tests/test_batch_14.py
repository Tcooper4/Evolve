...
class TestCompositeStrategy(unittest.TestCase):
    ...
    def test_priority_override_below_threshold(self):
        ...
        self.assertIn(
            resolution.resolution_method, ["majority_voting", "priority_override"]
        )
    ...
    def test_config_export_import(self):
        ...
        self.assertTrue(result)
        self.assertEqual(
            new_composite.strategy_priority, self.composite.strategy_priority
        )
        self.assertEqual(
            new_composite.min_confidence_threshold,
            self.composite.min_confidence_threshold,
        )
...
class TestConfidencePlotting(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.confidence_intervals = pd.DataFrame(
            {"lower": self.data - 2, "upper": self.data + 2}, index=dates
        )
    ...
    def test_plot_with_confidence_basic(self):
        ...
        fig = self.plotter.plot_with_confidence(
            self.data, self.confidence_intervals, show=False
        )
    ...
    def test_plot_with_confidence_empty_signals(self):
        ...
        empty_signals = {"buy": pd.Series(dtype=float), "sell": pd.Series(dtype=float)}
    ...
    def test_plot_with_confidence_no_signals(self):
        ...
        fig = self.plotter.plot_with_confidence(
            self.data, self.confidence_intervals, signal_markers=None, show=False
        )
...
class TestBatch14Integration(unittest.TestCase):
    ...
    def test_full_workflow(self):
        ...
        confidence_intervals = pd.DataFrame(
            {"lower": self.data["close"] - 2, "upper": self.data["close"] + 2},
    ...
    def test_error_handling(self):
        ...
        with self.assertRaises(Exception):
            ...
            self.plotter.plot_with_confidence(
                pd.Series([1, 2, 3, 4, 5]), None, show=False  # This will cause an error
            )
