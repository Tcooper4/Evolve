...
class TestAsyncStrategyRunner:
    ...
    async def test_parallel_execution_with_failures(self, runner, sample_data):
        ...
        strategies = [MockStrategy("GoodStrategy1"), MockStrategy("GoodStrategy2")]
        ...
        result = await runner.run_strategies_parallel(strategies, sample_data, {})
    ...
    async def test_concurrent_limit(self, runner, sample_data):
        ...
        with patch.object(
            runner, "_execute_strategy_core", side_effect=tracked_execution
        ):
            result = await runner.run_strategies_parallel(
