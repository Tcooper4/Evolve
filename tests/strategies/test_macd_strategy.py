...
class TestMACDStrategy:
    ...
    def test_golden_cross_trend_confirmation(self, strategy_config, sample_data):
        ...
        for scenario in cross_scenarios:
            ...
            declining_prices = 100 - np.linspace(0, scenario['decline_magnitude'], scenario['decline_period']) + np.random.normal(0, 1, scenario['decline_period'])
            
            # Generate reversal and uptrend
            uptrend_prices = (100 - scenario['decline_magnitude']) + np.linspace(0, scenario['uptrend_magnitude'], len(dates) - scenario['decline_period']) + np.random.normal(0, 1, len(dates) - scenario['decline_period'])
            ...
            self.assertGreaterEqual(len(golden_cross_points), scenario['expected_crosses'],
            ...
            for cross_point in golden_cross_points:
                if cross_point + 15 < len(golden_cross_data):
                    ...
                    print(f"    Cross at index {cross_point}: trend_confirmed={trend_confirmed}, histogram_positive={histogram_positive}, strength={trend_strength}")
        ...
        declining_prices = 100 - np.linspace(0, strong_scenario['decline_magnitude'], strong_scenario['decline_period']) + np.random.normal(0, 1, strong_scenario['decline_period'])
        uptrend_prices = (100 - strong_scenario['decline_magnitude']) + np.linspace(0, strong_scenario['uptrend_magnitude'], len(dates) - strong_scenario['decline_period']) + np.random.normal(0, 1, len(dates) - strong_scenario['decline_period'])
        ...
        false_cross_data = pd.DataFrame({
            "close": choppy_prices,
            "volume": choppy_volumes
        }, index=false_cross_dates)
