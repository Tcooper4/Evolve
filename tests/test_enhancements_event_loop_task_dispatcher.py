    ...
    async def test_dashboard_port_conflict_handling(self):
        ...
        with patch("subprocess.Popen") as mock_popen:
            ...
            process = subprocess.Popen(
                ["streamlit", "run", "test.py", "--server.port", str(occupied_port)]
            )
            await asyncio.sleep(0.1)
...
class TestTaskDispatcherEnhancements:
    ...
    async def test_local_task_queue(self):
        ...
        task3 = Task(
            id="task3", func=test_func, args=(3,), priority=TaskPriority.NORMAL
        )
...
def create_test_config():
    ...
    return {
        "event_loop": {"max_retries": 3, "base_delay": 0.1},
        "task_dispatcher": {"max_workers": 5, "enable_redis": False},
        "dashboard": {"port_range": (8501, 8600), "timeout": 30},
    }
