...
from trading.ensemble.integrity_checker import (
    DistanceMetric,
    DriftType,
    EnsembleIntegrityChecker,
)
from trading.risk_model.compliance_flags import (
    ComplianceFlagType,
    RiskLevel,
...
from trading.sentiment.parser import FinancialSymbol, SentimentParser
...
class TestSentimentParser(unittest.TestCase):
    ...
    def test_financial_symbol_preservation(self):
        ...
        ticker_symbol = next(
            (
                s
                for s in parsed.preserved_symbols
                if s["type"] == FinancialSymbol.TICKER.value
            ),
            None,
        )
        ...
        percentage_symbol = next(
            (
                s
                for s in parsed.preserved_symbols
                if s["type"] == FinancialSymbol.PERCENTAGE.value
            ),
            None,
        )
    ...
    def test_currency_preservation(self):
        ...
        currency_symbols = [
            s
            for s in parsed.preserved_symbols
            if s["type"] == FinancialSymbol.CURRENCY.value
        ]
...
class TestRiskModelComplianceFlags(unittest.TestCase):
    ...
    def test_compliance_flag_triggering(self):
        ...
        vol_flag = next(
            (f for f in flags if f.flag_type == ComplianceFlagType.VOLATILITY), None
        )
...
class TestBatch20Integration(unittest.TestCase):
    ...
    def test_end_to_end_workflow(self):
        ...
        for _ in range(10):
            integrity_checker.add_model_predictions(
                "test_model", np.random.normal(0, 1, 10)
            )

        current_predictions = np.random.normal(2, 1, 50)
        drift_result = integrity_checker.distribution_drift_check(
            "test_model", current_predictions
        )
