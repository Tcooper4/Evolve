...
from trading.optimization.backtest_optimizer import (
    RegimeDetector, WalkForwardOptimizer, BacktestOptimizer,
    WalkForwardResult, RegimeInfo
)
...
class TestRegimeDetector:
    ...
    def sample_data(self):
        ...
        data.loc['2020-07-01':'2020-12-31', 'close'] = data.loc['2020-06-30', 'close'] + np.cumsum(np.random.normal(-0.002, 0.03, 184))
        data.loc['2020-07-01':'2020-12-31', 'volume'] = np.random.randint(2000000, 8000000, 184)
        
        # Third regime: Moderate volatility sideways
        data.loc['2021-01-01':, 'close'] = data.loc['2020-12-31', 'close'] + np.cumsum(np.random.normal(0.0005, 0.02, 135))
    ...
    def test_calculate_regime_features(self, sample_data):
        ...
        expected_features = ['volatility', 'returns', 'skewness', 'kurtosis', 
                           'trend_strength', 'momentum', 'rsi', 'volume_ratio', 
