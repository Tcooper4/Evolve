...
class MockAgent(BaseAgent):
    ...
    async def execute(self, **kwargs) -> AgentResult:
        ...
        if self.should_fail and self.execution_count <= self.fail_count:
            raise Exception(
                f"Mock agent {self.config.name} failed on attempt {self.execution_count}"
            )
...
class TestEnhancedAgentManager:
    ...
    async def test_execute_agent_success(self, agent_manager):
        ...
        # Execute agent
        result = await agent_manager.execute_agent_with_retry(
            "success_agent", test_param="value"
        )

        assert result.success is True
        assert "message" in result.data
        assert (
            result.data["message"] == "Mock agent success_agent executed successfully"
        )
    ...
    async def test_execute_agent_failure_no_retry(self, agent_manager):
        ...
        result = await agent_manager.execute_agent_with_retry(
            "fail_agent", retry_config=retry_config
        )
    ...
    async def test_execute_agent_with_retry_success(self, agent_manager, retry_config):
        ...
        result = await agent_manager.execute_agent_with_retry(
            "retry_agent", retry_config=retry_config
        )
    ...
    async def test_execute_agent_with_retry_failure(self, agent_manager, retry_config):
        ...
        start_time = time.time()
        result = await agent_manager.execute_agent_with_retry(
            "always_fail_agent", retry_config=retry_config
        )
        execution_time = time.time() - start_time
    ...
    async def test_execute_agent_timeout(self, agent_manager):
        ...
        result = await agent_manager.execute_agent_with_retry(
            "slow_agent", retry_config=retry_config
        )
    ...
    def test_calculate_backoff_delay(self, agent_manager, retry_config):
        ...
        delay_no_jitter = agent_manager._calculate_backoff_delay(
            1, retry_config_no_jitter
        )
    ...
    def test_log_error_with_traceback(self, agent_manager):
        ...
        agent_manager._log_error_with_traceback(
            "test_agent", error, "task1", 1, context
        )
    ...
    async def test_concurrent_agent_execution(self, agent_manager):
        ...
        tasks = []
        for i in range(3):
            task = agent_manager.execute_agent_with_retry(
                f"agent_{i}", param=f"value_{i}"
            )
            tasks.append(task)
