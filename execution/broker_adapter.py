...
class BaseBrokerAdapter(ABC):
    ...
    def _check_rate_limit(self, endpoint: str) -> bool:
        ...
        self.last_request_time[endpoint] = [
            t for t in self.last_request_time[endpoint] if t > window_start
        ]
...
class AlpacaBrokerAdapter(BaseBrokerAdapter):
    ...
    async def submit_order(self, order: OrderRequest) -> OrderExecution:
        ...
        try:
            ...
            execution = OrderExecution(
                ...
                average_price=(
                    float(order_details.filled_avg_price)
            ...
            self.logger.error(f"Failed to submit order to Alpaca: {_unused_var}")
            raise
    ...
    async def cancel_order(self, order_id: str) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Failed to cancel order on Alpaca: {_unused_var}")
            return False
    ...
    async def get_order_status(self, order_id: str) -> Optional[OrderExecution]:
        ...
        try:
            ...
            self.logger.error(f"Failed to get order status from Alpaca: {_unused_var}")
            return None
    ...
    async def get_position(self, ticker: str) -> Optional[Position]:
        ...
        try:
            ...
            self.logger.debug(f"Position not found: {e}")
            return None
...
class IBKRBrokerAdapter(BaseBrokerAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Failed to connect to IBKR: {_unused_var}")
            self.is_connected = False
            return False
    ...
    async def submit_order(self, order: OrderRequest) -> OrderExecution:
        ...
        try:
            ...
            if order.order_type == OrderType.MARKET:
                ...
                ib_order = LimitOrder(
                    order.side.value.upper(), order.quantity, order.price
                )
            elif order.order_type == OrderType.STOP:
                ib_order = StopOrder(
                    order.side.value.upper(), order.quantity, order.stop_price
                )
            else:
                raise ValueError(f"Unsupported order type: {order.order_type}")
            ...
            self.logger.error(f"Failed to submit order to IBKR: {_unused_var}")
            raise
    ...
    async def cancel_order(self, order_id: str) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Failed to cancel order on IBKR: {_unused_var}")
            return False
    ...
    async def get_position(self, ticker: str) -> Optional[Position]:
        ...
        try:
            ...
            self.logger.error(f"Failed to get position from IBKR: {_unused_var}")
            return None
    ...
    async def get_all_positions(self) -> Dict[str, Position]:
        ...
        try:
            ...
            self.logger.error(f"Failed to get positions from IBKR: {_unused_var}")
            return {}
    ...
    async def get_account_info(self) -> AccountInfo:
        ...
        try:
            ...
            self.logger.error(f"Failed to get account info from IBKR: {_unused_var}")
            raise
    ...
    async def get_market_data(self, ticker: str) -> MarketData:
        ...
        try:
            ...
            self.logger.error(f"Failed to get market data from IBKR: {_unused_var}")
            raise
...
class PolygonBrokerAdapter(BaseBrokerAdapter):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # Rate limits
        self.rate_limits = {"market_data": {"max_requests": 1000, "window": 60}}
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(url, headers=headers) as response:
                    if response.status == 200:
                        ...
                        self.logger.error(
                            f"Failed to connect to Polygon: {response.status}"
                        )
                        return False

        except Exception as _unused_var:  # Placeholder, flake8 ignore: F841
            self.logger.error(f"Failed to connect to Polygon: {_unused_var}")
            self.is_connected = False
            return False
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(url, headers=headers) as response:
                    if response.status == 200:
                        ...
                        raise Exception(
                            f"Failed to get data for {ticker}: {response.status}"
                        )

        except Exception as _unused_var:  # Placeholder, flake8 ignore: F841
            self.logger.error(f"Failed to get market data from Polygon: {_unused_var}")
            raise
    ...
    async def get_position(self, ticker: str) -> Optional[Position]:
        raise NotImplementedError(
            "Polygon adapter does not support position management"
        )
...
def create_broker_adapter(
    broker_type: str = "simulation", config: Dict[str, Any] = None
) -> BrokerAdapter:
