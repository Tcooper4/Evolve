...
class ExecutionAgent:
    ...
    def __init__(self, config_path: str = "config/app_config.yaml"):
        ...
        self.execution_mode = ExecutionMode(
            self.execution_config.get("mode", "simulation")
        )
        ...
        self.price_impact_factor = self.execution_config.get(
    ...
    def _initialize_broker_adapter(self):
        ...
        if self.execution_mode in [ExecutionMode.LIVE, ExecutionMode.PAPER]:
            try:
                ...
                self.logger.info(
                    f"Broker adapter initialized: {broker_config.get('type', 'alpaca')}"
                )
    ...
    def _simulate_order_execution(self, order: OrderRequest) -> OrderExecution:
        ...
        price_impact = (
            order_value / 1000000
        ) * self.price_impact_factor  # Impact per $1M
        ...
        if execution.side == OrderSide.BUY:
            ...
            if position.quantity <= 0:
                ...
                if position.quantity <= execution.executed_quantity:
                    # Full sell
                    realized_pnl = (
                        execution.average_price - position.average_price
                    ) * position.quantity
                    position.realized_pnl += realized_pnl
                    position.quantity -= execution.executed_quantity
                    if position.quantity < 0:
    ...
    def _check_risk_limits(self, order: OrderRequest) -> Tuple[bool, str]:
        ...
        if order_value > self.max_order_size:
            return (
                False,
                f"Order size limit exceeded: ${order_value:.2f} > ${self.max_order_size:.2f}",
            )
        ...
        total_portfolio_value = (
            sum(pos.market_value for pos in self.positions.values()) + order_value
        )

        if (
            total_portfolio_value > 0
            and new_position_value / total_portfolio_value > self.max_position_size
        ):
            return (
                False,
    ...
    async def submit_order(
        ...
        self.logger.info(
            f"Order submitted: {order_id} - {side.value} {quantity} {ticker} @ {price or 'MARKET'}"
        )
    ...
    async def _execute_order(self, order: OrderRequest) -> OrderExecution:
        ...
        try:
            ...
            self.logger.info(
                f"Order executed: {order.order_id} - {execution.executed_quantity} @ {execution.average_price:.2f}"
            )
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        )  # This should track commission separately
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        order_id1 = await agent.submit_order(
            ticker="AAPL", side=OrderSide.BUY, order_type=OrderType.MARKET, quantity=100
        )
