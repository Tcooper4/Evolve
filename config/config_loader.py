...
class ConfigLoader:
        ...
        for env_var, (section, key) in self.env_mappings.items():
            ...
            if env_value is not None:
                if section in self.sections:
                    ...
                    self._set_nested_value(self.sections[section].data, key, converted_value)
                    self.logger.info(f"Applied environment override: {env_var} -> {section}.{key}")
        ...
        for section_name, section in self.sections.items():
            if section_name in self.schemas:
                try:
                    ...
                    self.logger.info(f"Configuration section '{section_name}' validated successfully")
                    ...
                    self.logger.error(f"Configuration validation error in '{section_name}': {e}")
                except Exception as e:
                    error = ConfigValidationError(
                        field="validation",
                    ...
                    section.validation_errors.append(error)
                    self.logger.error(f"Configuration validation failed for '{section_name}': {e}")
            ...
            if section:
                ...
                if section in self.sections:
                    ...
                    if file_path.exists():
                        ...
                        self.sections[section].last_modified = datetime.fromtimestamp(file_path.stat().st_mtime)
                        self.sections[section].validation_errors.clear()
                        
                        # Re-validate
                        if section in self.schemas:
                            validate(instance=section_data, schema=self.schemas[section])
    ...
    def export_config(self, format: str = "yaml") -> str:
        ...
        try:
            if format.lower() == "json":
                ...
                return yaml.dump(self.get_all_configs(), default_flow_style=False, indent=2)
        except Exception as e:
    ...
    def get_config_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "environment_overrides": len([k for k, v in self.env_mappings.items() if os.getenv(k)])
        }
        ...
        for section_name, section in self.sections.items():
            summary["sections"][section_name] = {
                ...
                "error_count": len([e for e in section.validation_errors if e.severity == "error"]),
                "warning_count": len([e for e in section.validation_errors if e.severity == "warning"]),
