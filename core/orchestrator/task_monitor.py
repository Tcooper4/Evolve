...
class TaskMonitor:
    ...
    def update_task_performance(self, task_name: str, execution: TaskExecution) -> None:
        ...
        if execution.duration_seconds:
            ...
            if len(self.performance_metrics[task_name]) > 100:
                self.performance_metrics[task_name] = self.performance_metrics[task_name][-50:]
        ...
        success_count = sum(1 for e in recent_executions if e.status == TaskStatus.COMPLETED)
        self.success_rates[task_name] = success_count / len(recent_executions) if recent_executions else 0.0
    ...
    async def check_system_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            healthy_agents = sum(1 for agent in self.agent_status.values() if agent.health_score > 0.7)
            ...
            for agent_name, agent in self.agent_status.items():
                if agent.health_score < 0.3:
                    critical_issues.append(f"{agent_name}: Low health score ({agent.health_score:.2f})")
                if agent.failure_count > self.error_threshold:
                    critical_issues.append(f"{agent_name}: High failure count ({agent.failure_count})")
            ...
            for task_name, metrics in self.performance_metrics.items():
                if metrics:
                    ...
                    if avg_duration > 600:  # 10 minutes
                        performance_issues.append(f"{task_name}: Slow performance ({avg_duration:.1f}s avg)")
        ...
        try:
            ...
            for task_name in self.success_rates:
                ...
                if recent_executions:
                    success_count = sum(1 for e in recent_executions if e.status == TaskStatus.COMPLETED)
                    self.success_rates[task_name] = success_count / len(recent_executions)
            
        except Exception as e:
            self.logger.error(f"Failed to update performance metrics: {e}")

    def _get_recent_executions(self, task_name: str, hours: int = 24) -> List[TaskExecution]:
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "average_success_rate": sum(self.success_rates.values()) / len(self.success_rates) if self.success_rates else 0.0,
