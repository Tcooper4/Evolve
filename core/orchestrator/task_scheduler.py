...
from .task_models import TaskConfig, TaskStatus, TaskType
    ...
    def _schedule_task(self, task_config: TaskConfig) -> None:
        ...
        if task_config.interval_minutes <= 0:
            ...
            job = schedule.every().day.at("00:00").do(
                self._task_wrapper, task_config.name
            ).tag(task_config.name)
        else:
            # Recurring task
            job = schedule.every(task_config.interval_minutes).minutes.do(
                self._task_wrapper, task_config.name
            ).tag(task_config.name)
        
        self.scheduled_tasks[task_config.name] = job
        self.logger.info(f"Scheduled task: {task_config.name} (every {task_config.interval_minutes} minutes)")
