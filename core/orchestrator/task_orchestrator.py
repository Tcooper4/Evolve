...
class TaskOrchestrator:
    ...
    def _create_default_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "scheduler_config": {"max_workers": 10},
            "executor_config": {"max_workers": 10},
            "monitor_config": {
                "health_check_interval_minutes": 5,
    ...
    def _initialize_task_providers(self):
        ...
        try:
            ...
            agent_provider = create_task_provider(
                "agent", self.config.get("agent_provider_config", {})
            )
            self.task_providers["agent"] = agent_provider

            # Initialize system task provider
            system_provider = create_task_provider(
                "system", self.config.get("system_provider_config", {})
            )
    ...
    def _load_tasks(self):
        ...
        try:
            ...
            self.logger.info(
                f"Loaded {len(tasks_config)} task configurations with dynamic DAG"
            )
        except Exception as e:
    ...
    def _build_execution_order(
        self, task_graph: Dict[str, TaskConfig]
    ) -> List[List[str]]:
    ...
    async def _health_monitor_loop(self):
        ...
        while self.is_running:
            try:
                ...
                await asyncio.sleep(
                    self.config.get("health_check_interval_minutes", 5) * 60
                )
            except Exception as e:
