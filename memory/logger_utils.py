...
class UnifiedLogger:
    ...
    def _setup_daily_logger(self, name: str, log_file: Path,
    ...
    def archive_logs(self, archive_dir: Union[str, Path]) -> None:
        ...
        for log_file in self.log_dir.glob("performance.log.*"):
            if log_file.suffix.isdigit():
                archive_path = archive_dir / f"performance_{datetime.now().strftime('%Y%m%d')}_{log_file.suffix[1:]}.log"
        ...
        for log_file in self.log_dir.glob("daily_*.log.*"):
            if log_file.suffix.isdigit():
                archive_path = archive_dir / f"{log_file.stem}_{datetime.now().strftime('%Y%m%d')}_{log_file.suffix[1:]}.log"
                log_file.rename(archive_path)
