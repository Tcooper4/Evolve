...
class PromptMemoryBackend(ABC):
    ...
    async def get_prompt_history(
        self, user_id: str = "default", n: int = 10
    ) -> List[PromptEntry]:
...
class PromptMemory:
    ...
    async def get_statistics(self, user_id: str = "default") -> Dict[str, Any]:
        ...
        try:
            ...
            if not history:
                # Return empty statistics if no history
                return {}

            total_prompts = len(history)
            successful_prompts = sum(1 for entry in history if entry.success)
            avg_execution_time = (
                sum(entry.execution_time for entry in history) / total_prompts
            )

            # Count agent types
            agent_counts = {}
            for entry in history:
                agent_counts[entry.agent_type] = (
                    agent_counts.get(entry.agent_type, 0) + 1
                )

            most_common_agent = (
                max(agent_counts.items(), key=lambda x: x[1])[0]
                if agent_counts
                else None
            )
...
async def log_prompt(prompt: str, result: Dict[str, Any], **kwargs) -> bool:
