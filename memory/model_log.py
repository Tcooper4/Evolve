...
def _update_best_models(ticker: str, model_name: str, performance_data: Dict[str, Any]):
    ...
    metrics_to_track = [
        (
            "sharpe",
            "best_sharpe",
            lambda x, y: x > y if x is not None and y is not None else x is not None,
        ),
        (
            "mse",
            "best_mse",
...
def render_model_performance_dashboard():
    ...
    if not tickers:
        st.warning(
            "No model performance data available. Run some models first to see performance metrics."
        )
        return
    ...
    selected_models = st.sidebar.multiselect(
        "Select Models", models, default=models[:5] if len(models) > 5 else models
    )

    days_back = st.sidebar.slider("Days Back", min_value=1, max_value=365, value=30)

    # Get performance data
    df = get_model_performance_history(ticker=selected_ticker, days_back=days_back)
    ...
    with tab1:
        ...
        # Initialize chart figures dictionary
        chart_figures = {}
        ...
        for metric in metrics_to_plot:
            if metric in df.columns and not df[metric].isna().all():
                ...
                # Store figure for potential future use
                chart_figures[metric] = fig
    ...
    with tab2:
        ...
        if not df.empty:
            ...
            avg_metrics = df.groupby("model_name")[
                ["sharpe", "mse", "total_return", "win_rate", "accuracy"]
            ].mean()
        ...
        for metric_name, metric_data in metrics.items():
            if metric_data.get("model"):
                summary_data.append(
                    {
                        "Ticker": ticker,
                        "Metric": metric_name.replace("best_", "")
                        .replace("_", " ")
                        .title(),
                        "Model": metric_data["model"],
                        "Value": metric_data["value"],
                        "Date": (
                            metric_data["timestamp"][:10]
