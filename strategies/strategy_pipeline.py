...
class CombinationConfig:
    ...
    mode: str = 'intersection'  # 'intersection', 'union', 'weighted', 'voting', 'confidence'
    ...
    conflict_resolution: str = 'majority'  # 'majority', 'highest_confidence', 'weighted'
...
class StrategyPipeline:
    ...
    def __init__(
        ...
        logger.info(f"StrategyPipeline initialized with {len(self.strategies)} strategies")
    ...
    def combine_signals(
        ...
        **kwargs
        ...
        if self.combination_config.smoothing_window:
            combined = self._apply_smoothing(combined, self.combination_config.smoothing_window)
    ...
    def _combine_voting(
        ...
        result = np.where(weighted_votes > threshold, 1,
                         np.where(weighted_votes < -threshold, -1, 0))
    ...
    def generate_combined_signals(
        ...
        for strategy_name in strategy_names:
            if strategy_name in self.strategy_functions:
                try:
                    strategy_config = next(s for s in self.strategies if s.name == strategy_name)
