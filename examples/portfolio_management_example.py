...
from portfolio.risk_manager import create_risk_manager
...
class PortfolioManagementExample:
        ...
        for asset in self.assets:
            ...
            if asset.ticker == "AAPL":
                ...
                noise = np.random.normal(
                    0, base_volatility * np.sqrt(1 - correlation**2), days
                )
    ...
    def compare_allocation_strategies(self):
        ...
        print(
            f"Best Strategy by Return: {comparison_df.loc[comparison_df['Expected Return'].idxmax(), 'Strategy']}"
        )
        print(
            f"Best Strategy by Volatility: {comparison_df.loc[comparison_df['Expected Volatility'].idxmin(), 'Strategy']}"
        )
        print(
            f"Best Strategy by Diversification: {comparison_df.loc[comparison_df['Diversification Ratio'].idxmax(), 'Strategy']}"
        )
    ...
    def run_risk_analysis(self, strategy_name="maximum_sharpe"):
        ...
        risk_attribution = self.risk_manager.calculate_risk_attribution(
        ...
        for ticker, attribution in risk_attribution.items():
            ...
            print(
                f"    Volatility Contribution: {attribution['volatility_contribution']:.4f}"
            )
            print(f"    VaR Contribution: {attribution['var_contribution']:.4f}")

        # Generate risk report
        risk_report = self.risk_manager.get_risk_report(
            positions, risk_metrics, violations
        )
    ...
    def run_portfolio_simulation(
        self, strategy_name="maximum_sharpe", rebalancing_freq="monthly"
    ):
        ...
        simulation = self.risk_manager.simulate_portfolio_returns(
            initial_positions, self.market_data, rebalancing_freq
        )
        ...
        sharpe_ratio = (
            (simulation["daily_return"].mean() * 252) / volatility
            if volatility > 0
            else 0
        )
        ...
        rolling_sharpe = (
            simulation["daily_return"].rolling(30).mean() * 252
        ) / rolling_vol
    ...
    def run_stress_testing(self, strategy_name="maximum_sharpe"):
        ...
        stress_results = self.risk_manager.stress_test_portfolio(
            positions, self.market_data, scenarios
        )
    ...
    def create_visualizations(self):
        ...
        if self.allocation_results:
            ...
            volatilities = [
                self.allocation_results[s].expected_volatility for s in strategies
            ]
        ...
        if self.allocation_results:
            ...
            for i, (strategy, name) in enumerate(
                zip(selected_strategies, strategy_names)
            ):
        ...
        if self.risk_analysis:
            ...
            if strategy_name in self.risk_analysis:
                ...
                volatility_contrib = [
                    risk_attribution[t]["volatility_contribution"] for t in tickers
                ]
...
def main():
    ...
    print("\nSUMMARY:")
    print(
        f"- Analyzed {len(example.assets)} assets across {len(example.allocation_results)} strategies"
    )
    print(
        f"- Best Sharpe Ratio: {max(r.sharpe_ratio for r in example.allocation_results.values()):.3f}"
    )
    print(
        f"- Best Expected Return: {max(r.expected_return for r in example.allocation_results.values()):.3f}"
    )
    print(
        f"- Lowest Volatility: {min(r.expected_volatility for r in example.allocation_results.values()):.3f}"
    )
