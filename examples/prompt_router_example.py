...
async def example_strategy_prompt():
    ...
    prompt = (
        "Create a momentum-based trading strategy for SPY ETF using ensemble models"
    )
...
async def example_full_pipeline():
    ...
    prompt = (
        "Run the complete pipeline: build, evaluate, and update a model for TSLA stock"
    )
...
async def main():
    ...
    try:
        ...
        logger.info("=" * 60)
        logger.info(
            f"Forecast Prompt: {'âœ“' if forecast_result['success'] else 'âœ—'}"
        )
        logger.info(
            f"Strategy Prompt: {'âœ“' if strategy_result['success'] else 'âœ—'}"
        )
        logger.info(
            f"Backtest Prompt: {'âœ“' if backtest_result['success'] else 'âœ—'}"
        )
        logger.info(
            f"Builder Workflow: {'âœ“' if builder_result['success'] else 'âœ—'}"
        )
        logger.info(
            f"Evaluator Workflow: {'âœ“' if evaluator_result['success'] else 'âœ—'}"
        )
        logger.info(
            f"Updater Workflow: {'âœ“' if updater_result['success'] else 'âœ—'}"
        )
