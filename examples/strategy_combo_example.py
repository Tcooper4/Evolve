...
def demonstrate_basic_combination():
    ...
    try:
        ...
        combination_config = CombinationConfig(
            mode="intersection", min_agreement=0.5, confidence_threshold=0.6
        )
        ...
        logger.info(
            f"Combined signal generated with {len(combined_signal[combined_signal != 0])} signals"
        )
...
def demonstrate_different_modes():
    ...
    try:
        ...
        for mode in modes:
            ...
            logger.info(
                f"{mode}: {signal_count} signals ({buy_signals} buy, {sell_signals} sell)"
            )
...
def demonstrate_weighted_combination():
    ...
    try:
        ...
        for i, weights in enumerate(weight_combinations):
            ...
            logger.info(
                f"Weights {i + 1} - Sharpe: {performance['sharpe_ratio']:.3f}, "
...
def calculate_performance_metrics(
    data: pd.DataFrame, signal: pd.Series
) -> Dict[str, float]:
...
def demonstrate_strategy_pipeline_integration():
    ...
    try:
        ...
        pipeline = create_strategy_combo(
            strategy_names=strategy_names, mode="weighted", weights=[0.4, 0.4, 0.2]
        )
...
def demonstrate_backward_compatibility():
    ...
    try:
        ...
        logger.info(
            f"Combined {len(combined[combined != 0])} signals using intersection mode"
        )
...
def main():
    ...
    try:
        ...
        logger.info("=== SUMMARY ===")
        logger.info(
            f"Basic combination: {len(basic_signal[basic_signal != 0]) if basic_signal is not None else 0} signals"
        )
        logger.info(f"Different modes tested: {len(modes_results)}")
        logger.info(f"Weighted combinations tested: {len(weighted_results)}")
        logger.info(
            f"Pipeline integration: {'Success' if integration_results else 'Failed'}"
        )
        logger.info(
            f"Backward compatibility: {'Success' if backward_signal is not None else 'Failed'}"
        )
...
if __name__ == "__main__":
    results = main()
    print(
        "Strategy combination completed. Multiple strategies have been "
