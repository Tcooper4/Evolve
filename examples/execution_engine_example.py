...
class ExecutionEngineExample:
    ...
    async def run_basic_execution_demo(self):
        ...
        for order_id in order_ids:
            ...
            if execution:
                print(
                    f"  {order_id}: {execution.status.value} @ ${execution.average_price:.2f}"
                )
        ...
        for ticker, position in positions.items():
            print(
                f"  {ticker}: {position.quantity} shares @ ${position.average_price:.2f}"
            )
    ...
    async def run_portfolio_rebalancing_demo(self):
        ...
        for ticker in self.tickers:
            if ticker in positions:
                ...
                if (
                    abs(current_value - target_value) > total_portfolio_value * 0.02
                ):  # 2% threshold
                    if current_value > target_value:
                        ...
                        if sell_quantity > 0:
                            print(
                                f"  Rebalancing {ticker}: Selling {sell_quantity} shares"
                            )
                            await self.agent.submit_order(
                                ticker=ticker,
                                side=OrderSide.SELL,
                        ...
                        if buy_quantity > 0:
                            print(
                                f"  Rebalancing {ticker}: Buying {buy_quantity} shares"
                            )
                            await self.agent.submit_order(
                                ticker=ticker,
                                side=OrderSide.BUY,
        ...
        print(
            f"  Portfolio Return: {((final_total_value - total_portfolio_value) / total_portfolio_value):.2%}"
        )
            ...
            try:
                ...
                if connected:
                    ...
                    try:
                        ...
                        print(
                            f"  Market Data: âœ… {market_data.ticker} @ ${market_data.last:.2f}"
                        )
    ...
    async def run_performance_analysis_demo(self):
        ...
        for i, trade in enumerate(trades):
            print(
                f"  Trade {i + 1}: {trade['side'].value} {trade['quantity']} {trade['ticker']}"
            )
