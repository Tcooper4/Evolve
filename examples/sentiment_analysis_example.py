...
def main():
    ...
    if not features_df.empty:
        ...
        dates = pd.date_range(datetime.now() - timedelta(hours=24), periods=10, freq='2H')
    ...
    multi_features = analyzer.generate_multi_ticker_features(list(multi_sentiment.keys()), hours_back=6)
    ...
    if multi_features:
        ...
        print(f"Total data points: {summary['overall_stats'].get('total_data_points', 0)}")
    ...
    def monitor_sentiment(ticker: str, interval_minutes: int = 30):
        ...
        print(f"Starting sentiment monitoring for {ticker} (every {interval_minutes} minutes)")
        ...
        while True:
            try:
                ...
                if not features.empty:
                    ...
                    print(f"[{timestamp}] {ticker} sentiment signal: {latest_signal:.3f}")
    ...
    def sentiment_trading_strategy(ticker: str, lookback_hours: int = 24):
        ...
        sentiment_momentum = features.get('sentiment_momentum', pd.Series()).iloc[-1] if 'sentiment_momentum' in features.columns else 0
        sentiment_volatility = features.get('sentiment_volatility', pd.Series()).iloc[-1] if 'sentiment_volatility' in features.columns else 0
        engagement_rate = features.get('engagement_rate', pd.Series()).iloc[-1] if 'engagement_rate' in features.columns else 0
...
def example_social_sentiment():
    ...
    print(f"Fetched {len(reddit_data)} Reddit posts and {len(twitter_data)} tweets for {ticker}")
