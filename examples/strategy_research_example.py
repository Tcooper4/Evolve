...
def main():
    ...
    if results["status"] == "success":
        ...
        if summary["recent_discoveries"]:
            ...
            for discovery in summary["recent_discoveries"][:5]:
                ...
                print(
                    f"    Type: {discovery['type']}, Confidence: {discovery['confidence']:.2f}"
                )
    ...
    for discovery in arxiv_discoveries[:3]:
        ...
        print(
            f"    Type: {discovery.strategy_type}, Confidence: {discovery.confidence_score:.2f}"
        )
    ...
    if arxiv_discoveries:
        ...
        if strategy_code:
            ...
            strategy_filename = (
                f"discovered_{test_discovery.title.replace(' ', '_')[:30]}.py"
            )
            strategy_path = (
                Path("strategies/discovered")
    ...
    high_confidence = [
        d for d in agent.discovered_strategies if d.confidence_score > 0.7
    ]
    medium_confidence = [
        d for d in agent.discovered_strategies if 0.4 <= d.confidence_score <= 0.7
    ]
    low_confidence = [
        d for d in agent.discovered_strategies if d.confidence_score < 0.4
    ]
    ...
    momentum_strategies = [
        d for d in agent.discovered_strategies if d.strategy_type == "momentum"
    ]
    mean_reversion_strategies = [
        d for d in agent.discovered_strategies if d.strategy_type == "mean_reversion"
    ]
        ...
        try:
            ...
            if command == "quit":
                ...
                for i, discovery in enumerate(agent.discovered_strategies):
                    print(
                        f"  {i}: {discovery.title} ({discovery.source}) - {discovery.strategy_type}"
                    )
    ...
    print(
        "Strategy research completed. New strategies have been discovered "
        "and analyzed."
    )
...
def example_strategy_generation():
    ...
    if strategy_code:
        ...
        print("-" * 40)
        print(
            strategy_code[:500] + "..." if len(strategy_code) > 500 else strategy_code
        )
        print("-" * 40)
