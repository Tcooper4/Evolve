...
def demo_prompt_examples():
    ...
    if "error" in stats:
        ...
        print(
            f"   🔍 Average performance score: {stats['average_performance_score']:.3f}"
        )
        print(f"   🔗 Embeddings available: {stats['embeddings_available']}")
        print(
            f"   📊 Sentence transformer available: {stats['sentence_transformer_available']}"
        )
    ...
    for i, prompt in enumerate(test_prompts, 1):
        ...
        if similar_examples:
            ...
            for j, example_data in enumerate(similar_examples, 1):
                ...
                print(
                    f"         Performance: {example.get('performance_score', 0.0):.2f}"
                )
        else:
    ...
    if similar_examples:
        ...
        print("   ⚠️ No examples available for few-shot prompt creation")
    ...
    try:
        ...
        agent._save_successful_example(
            "Forecast DEMO stock for next week", new_parsed_output, "demo_test", 0.95
        )
...
def main():
    ...
    try:
        ...
        print(f"⚠️ Error running demos: {e}")
        import traceback
