...
def main():
    ...
    if summary:
        ...
        if "model_performance" in summary and summary["model_performance"]:
            print(
                f"  Model Performance: {summary['model_performance'].get('average', 0):.3f}"
            )

        if "strategy_performance" in summary and summary["strategy_performance"]:
            print(
                f"  Strategy Performance: {summary['strategy_performance'].get('average', 0):.3f}"
            )
    ...
    if health_decision:
        ...
        print(
            f"  Health Check Result: {'✅ Healthy' if health_result.success else '❌ Issues Found'}"
        )
...
def example_performance_monitoring():
    ...
    for i in range(10):
        ...
        if metrics:
            ...
            avg_model_perf = (
                np.mean(list(metrics.model_performance.values()))
                if metrics.model_performance
                else 0
            )
            avg_strategy_perf = (
                np.mean(list(metrics.strategy_performance.values()))
    ...
    if performance_data:
        ...
        print(
            f"  Model Performance: {df['model_performance'].mean():.3f} ± {df['model_performance'].std():.3f}"
        )
        print(
            f"  Strategy Performance: {df['strategy_performance'].mean():.3f} ± {df['strategy_performance'].std():.3f}"
        )
        ...
        model_trend = (
            "📈 Improving"
            if df["model_performance"].iloc[-1] > df["model_performance"].iloc[0]
            else "📉 Declining"
        )
        strategy_trend = (
            "📈 Improving"
            if df["strategy_performance"].iloc[-1] > df["strategy_performance"].iloc[0]
            else "📉 Declining"
        )
...
def example_threshold_adjustment():
    ...
    model_thresholds["performance_threshold"] = (
        original_threshold * 0.8
    )  # More sensitive

    print(
        f"Adjusted performance threshold: {original_threshold:.3f} → {model_thresholds['performance_threshold']:.3f}"
    )
    ...
    if metrics:
        ...
        print(
            f"Trigger evaluation with adjusted thresholds: {len(decisions)} decisions"
        )
    ...
    print(
        f"Restored performance threshold: {model_thresholds['performance_threshold']:.3f}"
    )
...
def example_error_handling():
    ...
    for decision in decisions:
        ...
        if decision.priority >= 4:
            ...
            if result.recommendations:
                print(
                    f"    Recovery Recommendations: {', '.join(result.recommendations)}"
                )
