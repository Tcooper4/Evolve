...
# NOTE: Flake8 compliance changes applied. Non-ASCII print statements and unnecessary f-strings fixed.
"""
...
def demonstrate_basic_simulation():
    ...
    returns = generate_realistic_returns(
        n_days=252, mean_return=0.0005, volatility=0.02
    )
...
def demonstrate_bootstrap_methods():
    ...
    print("\n🛠️ Bootstrap Methods Comparison")
    print("=" * 50)
...
def demonstrate_confidence_intervals():
    ...
    config = MonteCarloConfig(n_simulations=1000, confidence_levels=confidence_levels)
...
def demonstrate_risk_analysis():
    ...
    for scenario_name, mean_ret, vol in scenarios:
        ...
        returns = generate_realistic_returns(
            n_days=252, mean_return=mean_ret, volatility=vol
        )
...
def demonstrate_performance_comparison():
    ...
    strategies = {
        ...
        "Conservative": generate_realistic_returns(
            mean_return=0.0003, volatility=0.015
        ),
    ...
    for strategy_name, returns in strategies.items():
        ...
        print(f"  Volatility: {stats['std_total_return']:.2%}")
        print(
            f"  Sharpe Ratio: {stats['mean_total_return'] / stats['std_total_return']:.2f}"
        )
        print(f"  95% VaR: {stats['var_95']:.2%}")
    ...
    comparison_df["Sharpe_Ratio"] = (
        comparison_df["mean_total_return"] / comparison_df["std_total_return"]
    )
...
def demonstrate_parameter_sensitivity():
    ...
    for n_sim in n_simulations_list:
        ...
        stats = results["summary_statistics"]
        print(
            f"  {n_sim:>4} simulations: Mean=${stats['mean_final_value']:>8,.0f}, VaR={stats['var_95']:>6.2%}"
        )
    ...
    for capital in initial_capitals:
        ...
        print(
            f"  ${capital:>6,.0f}: Final=${stats['mean_final_value']:>8,.0f}, Return={total_return:>6.2%}"
        )
...
def main():
    ...
    try:
        ...
        print(
            "Monte Carlo simulation completed. Check the results above for "
