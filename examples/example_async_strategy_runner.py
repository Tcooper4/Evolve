...
class AsyncStrategyExecutor:
    ...
    async def run_bollinger_bands(self) -> dict:
        ...
        logger.info(
            f"Bollinger Bands strategy completed: {result.get('success', False)}"
        )
        return result
    ...
    async def run_mean_reversion(self) -> dict:
        ...
        logger.info(
            f"Mean Reversion strategy completed: {result.get('success', False)}"
        )
        return result
...
async def example_sequential_vs_parallel():
    ...
    parallel_results = await asyncio.gather(
        executor.run_rsi(), executor.run_macd(), executor.run_bollinger_bands()
    )
    ...
    try:
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                ...
                logger.warning(
                    f"Strategy {i} completed but failed: {result.get('error')}"
                )
            else:
