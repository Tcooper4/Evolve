...
def demonstrate_all_models_optimization():
    ...
    result = tuner.optimize_all_models(
        data=data, target_column="close", model_types=["lstm", "xgboost", "transformer"]
    )
...
def demonstrate_forecasting_integration():
    ...
    result = optimizer.optimize_for_forecasting(
        data=data, target_column="close", forecast_horizon=30
    )

    if result["success"]:
        logger.info(f"Forecasting optimization completed!")
        logger.info(
            f"Recommended model: {result['recommendation']['recommended_model']}"
        )
        logger.info(
            f"Expected Sharpe ratio: {result['recommendation']['expected_sharpe']:.4f}"
        )
...
def demonstrate_pipeline_integration():
    ...
    from trading.optimization.forecasting_integration import (
        integrate_with_forecasting_pipeline,
    )

    # Integrate with pipeline
    logger.info("Integrating optimization with forecasting pipeline...")
    result = integrate_with_forecasting_pipeline(
        data=data, target_column="close", forecast_horizon=30, auto_optimize=True
    )
    ...
    if result["success"]:
        ...
        if result["optimization_result"]:
            logger.info(
                f"Optimization Sharpe ratio: {result['optimization_result']['recommendation']['expected_sharpe']:.4f}"
            )
    else:
...
def demonstrate_results_analysis():
    ...
    # Run optimization
    result = tuner.optimize_all_models(
        data=data, target_column="close", model_types=["lstm", "xgboost"]
    )
...
def main():
    ...
    try:
        ...
        demonstrate_forecasting_integration()
        logger.info("-" * 30)
