...
class ExplainabilityExample:
    ...
    def run_signal_generation_example(self):
        ...
        for ticker in self.tickers:
            ...
            print(f"  Signal: {signals['momentum_value']:.3f} (confidence: {signals['confidence']:.1%})")
    ...
    def run_model_selection_example(self):
        ...
        for ticker in self.tickers:
            ...
            model_event_id = self.audit_logger.log_model_selected(
                ...
                alternative_models=[m['name'] for m in models if m['name'] != selected_model['name']],
            ...
            print(f"  Selected: {selected_model['name']} (accuracy: {selected_model['performance']['accuracy']:.1%})")
    ...
    def run_forecast_generation_example(self):
        ...
        for ticker in self.tickers:
            ...
            print(f"  Forecast: {forecast['value']:.2f} (confidence: {forecast['confidence']:.1%})")
    ...
    def run_performance_analysis_example(self):
        ...
        performance_metrics = {
            'total_trades': len([e for e in self.audit_events if e['type'] == 'trade_decision']),
            'successful_trades': len([e for e in self.audit_events if e['type'] == 'trade_decision' and e['expected_return'] > 0]),
            'average_confidence': np.mean([e['confidence'] for e in self.audit_events if 'confidence' in e]),
            'total_expected_return': sum([e['expected_return'] for e in self.audit_events if e['type'] == 'trade_decision'])
        }
        
        print(f"\nPerformance Summary:")
        print(f"  Total trades: {performance_metrics['total_trades']}")
        print(f"  Successful trades: {performance_metrics['successful_trades']}")
        print(f"  Success rate: {performance_metrics['successful_trades']/performance_metrics['total_trades']:.1%}")
        print(f"  Average confidence: {performance_metrics['average_confidence']:.1%}")
        print(f"  Total expected return: {performance_metrics['total_expected_return']:.1%}")
    ...
    def generate_comprehensive_reports(self):
        ...
        print(f"\nExplanation Summary:")
        print(f"  Total explanations: {explanation_summary['summary']['total_explanations']}")
        print(f"  Tickers covered: {explanation_summary['summary']['tickers_covered']}")
        print(f"  Explanations by type: {explanation_summary['metrics']['explanations_by_type']}")
    ...
    def _generate_integrated_report(self, audit_summary: Dict, explanation_summary: Dict):
        ...
        integrated_report = {
            ...
            'trading_analysis': {
                'tickers_traded': list(set(e['ticker'] for e in self.audit_events if 'ticker' in e)),
                'decision_types': list(set(e['type'] for e in self.audit_events)),
                'average_confidence': self.performance_metrics.get('average_confidence', 0),
                'total_expected_return': self.performance_metrics.get('total_expected_return', 0)
            },
        ...
        models = [
            {
                ...
                'performance': {'accuracy': 0.85, 'sharpe_ratio': 1.2, 'stability': 0.9},
            ...
            },
            {
                ...
                'performance': {'accuracy': 0.82, 'sharpe_ratio': 1.1, 'stability': 0.85},
            ...
            },
            {
                ...
                'performance': {'accuracy': 0.84, 'sharpe_ratio': 1.15, 'stability': 0.88},
    ...
    def _generate_forecast(self, ticker: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'risk_factors': ['Market volatility', 'Earnings uncertainty', 'Macro risks'],
    ...
    def _generate_trade_decision(self, ticker: str) -> Dict[str, Any]:
        ...
        stop_loss = entry_price * (1 - expected_risk) if trade_type == "buy" else entry_price * (1 + expected_risk)
        take_profit = entry_price * (1 + expected_return) if trade_type == "buy" else entry_price * (1 - expected_return)
        ...
        return {
            ...
            'technical_signals': ['RSI momentum', 'MACD crossover', 'Volume confirmation'],
