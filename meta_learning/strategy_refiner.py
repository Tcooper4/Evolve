...
class StrategyRefiner(BaseAgent):
    ...
    def _calculate_recency_weights(self) -> None:
        ...
        for performance in self.performance_history:
            ...
            if days_old > self.max_history_days:
                ...
                # Exponential decay
                performance.recency_weight = math.exp(
                    -self.recency_decay_rate * days_old
                )
    ...
    def get_top_strategies(
        self, scoring_method: str = "composite", limit: int = None
        ...
        recent_performances = [
            p for p in self.performance_history if p.recency_weight > 0
        ]
        ...
        if not scoring_func:
            self.logger.warning(
                f"Unknown scoring method: {scoring_method}, using composite"
            )
    ...
    def refine_strategies(
        self, strategy_configs: List[Dict[str, Any]], scoring_method: str = "composite"
    ) -> List[Dict[str, Any]]:
        ...
        top_strategies = self.get_top_strategies(
            scoring_method, self.top_strategies_count
        )
        ...
        for config in strategy_configs:
            ...
            if is_top_performer:
                ...
                self.logger.info(
                    f"Applied minor refinement to top performer: {strategy_name}"
                )
            else:
                # Major refinements for underperformers
                refined_config = self._major_refinement(config, top_strategies)
                self.logger.info(
                    f"Applied major refinement to underperformer: {strategy_name}"
                )
    ...
    def _major_refinement(
        self, config: Dict[str, Any], top_strategies: List[Tuple[str, float]]
    ) -> Dict[str, Any]:
    ...
    def _apply_parameter_mutations(
        self, config: Dict[str, Any], best_performance: StrategyPerformance
    ) -> Dict[str, Any]:
    ...
    async def execute(self, **kwargs) -> AgentResult:
        ...
        try:
            ...
            refined_configs = []
            if strategy_configs:
                refined_configs = self.refine_strategies(
                    strategy_configs, scoring_method
                )
    ...
    def _calculate_performance_statistics(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "best_strategy": max(
                recent_performances, key=lambda p: p.sharpe_ratio
            ).strategy_name,
            "worst_strategy": min(
                recent_performances, key=lambda p: p.sharpe_ratio
            ).strategy_name,
    ...
    def register_scoring_function(
        self, name: str, scoring_func: ScoringFunction
    ) -> None:
    ...
    def get_performance_history(
        ...
        if strategy_name:
            return [
                p for p in self.performance_history if p.strategy_name == strategy_name
            ]
