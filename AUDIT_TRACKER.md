...
**Session 3 (continued) — nlp / report / memory / agents tiers:**
- **NLP tier**: ticker extraction treated every short word as a ticker (re.IGNORECASE nullified the uppercase pattern's entire signal — one sentence yielded 15 "tickers", 13 noise); now case-sensitive + stoplist, cashtags bypass. sentiment_processor (already deep-upgraded earlier: VADER blend, negation, inflection) sanity-verified by execution.
- **Report tier**: profit-factor/win-loss ratios emitted `Infinity` (invalid JSON, corrupts exported reports) on zero-loss runs at 4 sites — capped at 999.0; metrics math hand-verified; numpy-typed hyperparameter export verified end-to-end.
- **Memory tier — the worst multi-user leak**: LONG_TERM and PREFERENCE entries were global (single-user-era design for restart persistence) — Bob's chat context included Alice's trades; one user switching the active LLM switched everyone's; chat preference learning wrote into a shared pot. Now durably per-user (resolved per call — the store is a singleton), legacy NULL rows readable with user rows shadowing them, personal mode still persists across restarts. Proven through one shared store instance.
- **Agents tier**: market_regime_agent's RandomForest was trained on synthetic random noise (incl. sideways=high-vol, backwards) — output on real features essentially arbitrary; replaced with transparent finance-consistent rules, all five regimes verified. model_selector_agent's regime detection had TRENDING_DOWN unreachable, a 70%-per-DAY trend threshold (directional selection had never fired), and abs(autocorr) classifying trending series as mean-reverting — all fixed and verified on characteristic series. agent_leaderboard sort directions (incl. drawdown sign) verified clean.
- Session battery: 60+ tests across auth/data/nlp/report/agents, all passing; all pages AppTest-clean.
- **docs/NEXT_SESSIONS.md** written: precise live-data mandate for the fresh chat, then remaining depth (agent bodies line-by-line, wire-or-archive five, stale legacy suites), then the React rewrite.
