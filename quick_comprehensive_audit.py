...
def quick_audit():
    ...
    for root, dirs, files in os.walk('.'):
        ...
        dirs[:] = [d for d in dirs if not any(exclude in d for exclude in ['__pycache__', '.git', '.venv', 'venv', 'env'])]
            ...
            for func_name in functions:
                ...
                if match:
                    ...
                    if has_return:
                        ...
                        side_effects = any(pattern in func_body.lower() for pattern in [
                            'print(', 'logging', 'logger', 'st.', 'plt.', 'open(', 'write(', 
                            'save(', 'load(', 'return', 'pass', 'raise'
                        ])
                        
                        if side_effects and not func_name.startswith('test_'):
                            # Determine priority
                            file_str = str(file_path).lower()
                            if any(keyword in file_str for keyword in ['agent', 'model', 'strategy', 'service', 'core']):
                                priority = 'critical'
