...
class KubernetesDeployer:
    ...
    def build_and_push_image(self) -> bool:
        ...
        try:
            ...
            build_result = self.run_command([
                "docker", "build", "-t", self.full_image_name, "."
            ...
            push_result = self.run_command([
                "docker", "push", self.full_image_name
    ...
    def apply_kubernetes_configs(self) -> bool:
        ...
        try:
            ...
            apply_result = self.run_command([
                "kubectl", "apply", "-f", "kubernetes/deployment.yaml", "-n", self.namespace
            ], timeout=60)
    ...
    def wait_for_deployment(self) -> bool:
        ...
        try:
            ...
            rollout_result = self.run_command([
                "kubectl", "rollout", "status", f"deployment/{self.deployment_name}",
                "-n", self.namespace
    ...
    def check_service_health(self) -> bool:
        ...
        try:
            ...
            service_url_result = self.run_command([
                ...
                "-n", self.namespace, "-o", "jsonpath='{.spec.rules[0].host}'"
            ], timeout=30)
    ...
    def show_logs(self) -> None:
        ...
        try:
            ...
            self.run_command([
                "kubectl", "logs", "-f", f"deployment/{self.deployment_name}",
                "-n", self.namespace
            ], timeout=60, capture_output=False)
...
Examples:
  ...
  %(prog)s --show-logs  # Show logs after deployment
    ...
    config = {
        ...
        "health_check_timeout": args.health_timeout or int(env_config["health_check_timeout"])
    }
    ...
    if args.dry_run:
        ...
        logger.info(f"Would deploy {config['registry']}/{config['image']}:{config['image_tag']}")
