    ...
    def validate_config(self, config_path: str):
        ...
        try:
            ...
            if not isinstance(config["app"]["name"], str):
                validation_results["issues"].append("app.name must be a string")

            if not isinstance(config["app"]["version"], str):
                validation_results["issues"].append("app.version must be a string")

            # Validate server section
            if not isinstance(config["server"]["host"], str):
                validation_results["issues"].append("server.host must be a string")

            if not isinstance(config["server"]["port"], int):
                validation_results["issues"].append("server.port must be an integer")

            # Validate database section
            if not isinstance(config["database"]["host"], str):
                validation_results["issues"].append("database.host must be a string")

            if not isinstance(config["database"]["port"], int):
                validation_results["issues"].append("database.port must be an integer")
                ...
                for key, value in config.items():
                    if isinstance(value, (str, int, float, bool)):
                        client.secrets.kv.v2.create_or_update_secret(
                            path=f"trading/{key}", secret=dict(value=str(value))
                        )
                    elif isinstance(value, dict):
                        for subkey, subvalue in value.items():
                            client.secrets.kv.v2.create_or_update_secret(
