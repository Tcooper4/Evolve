...
class EvolveOrchestratorIntegration:
        ...
        for agent_name, agent in self.orchestrator.agents.items():
            if agent_name in method_mappings:
                ...
                if available_methods:
                    self.logger.info(
                        f"Agent {agent_name} has methods: {available_methods}"
                    )
                else:
    ...
    def get_integration_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "enabled_tasks": len(
                [t for t in self.orchestrator.tasks.values() if t.enabled]
            ),
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Integrate Task Orchestrator with Evolve Platform"
    )
    parser.add_argument(
        "--config",
        type=str,
    ...
    parser.add_argument(
        "--start", action="store_true", help="Start orchestrator after integration"
    )
    ...
    try:
        ...
        if success:
            print(
                "Orchestrator integration completed. System components have "
                ...
                if args.monitor:
                    ...
                    try:
                        while True:
                            ...
                            status = integration.get_integration_status()
                            print(
                                f"Health: {status['overall_health']:.2f} | "
