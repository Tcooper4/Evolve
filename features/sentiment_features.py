...
class SentimentAnalyzer:
    ...
    def _initialize_analyzers(self):
        ...
        if VADER_AVAILABLE:
            ...
            self.logger.warning("VADER not available - sentiment scoring will be limited")
        
        # Initialize BERT
        if BERT_AVAILABLE:
            try:
                # Use a pre-trained sentiment analysis model
                model_name = self.sentiment_config.get('bert_model', 'cardiffnlp/twitter-roberta-base-sentiment-latest')
                self.bert_analyzer = pipeline(
                    "sentiment-analysis",
                    model=model_name,
                    device=0 if torch.cuda.is_available() else -1
                )
                self.logger.info(f"BERT sentiment analyzer initialized with {model_name}")
    ...
    def analyze_text_sentiment(self, text: str) -> Dict[str, float]:
        ...
        if self.bert_analyzer and cleaned_text:
            try:
                bert_result = self.bert_analyzer(cleaned_text[:512])[0]  # Limit text length
    ...
    def process_sentiment_data(self, sentiment_data: List[SentimentData]) -> List[SentimentFeatures]:
        """
        ...
        for data in sentiment_data:
            ...
            weighted_score = sentiment_scores['vader_compound'] * weight * engagement_score
    ...
    def _calculate_engagement_score(self, data: SentimentData) -> float:
        ...
        if data.source == 'reddit':
            ...
            upvote_ratio = data.metadata.get('upvote_ratio', 0.5) if data.metadata else 0.5
    ...
    def aggregate_sentiment_features(self, features: List[SentimentFeatures], 
                                   time_window: int = 24) -> pd.DataFrame:
        """
        ...
        resampled = df.resample(f'{time_window}H').agg({
            'ticker': 'first',
        ...
        resampled['engagement_rate'] = resampled['weighted_score_sum'] / resampled['volume_sum'].replace(0, 1)
        
        # Add source-specific features
        for source in ['news', 'reddit', 'twitter']:
            source_df = df[df['source'] == source]
            if not source_df.empty:
                source_resampled = source_df.resample(f'{time_window}H').agg({
                    'vader_compound': 'mean',
                    'weighted_score': 'sum',
                    'volume': 'sum'
                }).fillna(0)
                ...
                resampled[f'{source}_weighted_score'] = source_resampled['weighted_score']
        
        return resampled
    
    def generate_sentiment_features(self, ticker: str, hours_back: int = 24) -> pd.DataFrame:
        """
    ...
    def generate_multi_ticker_features(self, tickers: List[str], 
        ...
        for ticker in tickers:
            try:
                ...
                if not features.empty:
                    ...
                    self.logger.info(f"Generated features for {ticker}: {len(features)} rows")
                else:
    ...
    def create_sentiment_signal(self, features_df: pd.DataFrame, 
        ...
        sentiment_momentum = features_df['vader_compound_mean'].rolling(lookback_periods).mean()
        
        # Calculate sentiment strength
        sentiment_strength = features_df['vader_compound_std'].rolling(lookback_periods).mean()
        
        # Calculate volume-weighted sentiment
        volume_sentiment = (features_df['weighted_score_sum'] / 
                          features_df['volume_sum'].replace(0, 1)).rolling(lookback_periods).mean()
        
        # Combine signals
        signal = (sentiment_momentum * 0.4 + 
    ...
    def get_sentiment_correlation(self, features_dict: Dict[str, pd.DataFrame]) -> pd.DataFrame:
        """
    ...
    def save_features(self, features_dict: Dict[str, pd.DataFrame], 
    ...
    def get_sentiment_summary(self, features_dict: Dict[str, pd.DataFrame]) -> Dict[str, Any]:
        """
...
def create_sentiment_analyzer(config_path: str = "config/app_config.yaml") -> SentimentAnalyzer:
