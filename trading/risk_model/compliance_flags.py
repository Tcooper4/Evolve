...
class RiskModelComplianceFlags:
    ...
    def __init__(
        ...
        logger.info(
            f"RiskModelComplianceFlags initialized with rolling window: {rolling_window}"
        )
    ...
    def add_risk_data(
        ...
        # Create data row
        data_row = {"timestamp": timestamp, **risk_metrics}
        ...
        if len(self.risk_data[portfolio_id]) > self.rolling_window:
            self.risk_data[portfolio_id] = self.risk_data[portfolio_id].tail(
                self.rolling_window
            )
    ...
    def _check_volatility_compliance(
        self,
        current_vol: float,
    ...
    def _check_drawdown_compliance(
        self,
        current_dd: float,
        thresholds: RiskThresholds,
        portfolio_id: str,
    ...
    def _check_concentration_compliance(
        self,
        current_conc: float,
        thresholds: RiskThresholds,
        portfolio_id: str,
    ...
    def _check_correlation_compliance(
        self,
        current_corr: float,
        thresholds: RiskThresholds,
        portfolio_id: str,
    ...
    def _check_leverage_compliance(
        self,
        current_lev: float,
        thresholds: RiskThresholds,
        portfolio_id: str,
    ...
    def _update_thresholds(self, portfolio_id: str) -> None:
        ...
        try:
            ...
            thresholds = RiskThresholds(
                ...
                drawdown_95th=(
                    df["drawdown"].quantile(0.95) if "drawdown" in df.columns else 0.0
                ),
                drawdown_99th=(
                    df["drawdown"].quantile(0.99) if "drawdown" in df.columns else 0.0
                ),
                concentration_95th=(
                    df["concentration"].quantile(0.95)
                    if "concentration" in df.columns
                    else 0.0
                ),
                concentration_99th=(
                    df["concentration"].quantile(0.99)
                    if "concentration" in df.columns
                    else 0.0
                ),
                correlation_95th=(
                    df["correlation"].quantile(0.95)
                    if "correlation" in df.columns
                    else 0.0
                ),
                correlation_99th=(
                    df["correlation"].quantile(0.99)
                    if "correlation" in df.columns
                    else 0.0
                ),
            ...
            logger.info(
                f"Updated thresholds for portfolio {portfolio_id} with {len(df)} samples"
            )
    ...
    def clear_resolved_flags(
        self, portfolio_id: str, flag_types: Optional[List[ComplianceFlagType]] = None
    ) -> int:
        """
        ...
        logger.info(
            f"Cleared {len(flags_to_remove)} flags for portfolio {portfolio_id}"
        )
    ...
    def get_compliance_summary(self, portfolio_id: str) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "critical_flags": len(
                [f for f in active_flags if f.risk_level == RiskLevel.CRITICAL]
            ),
            "high_flags": len(
                [f for f in active_flags if f.risk_level == RiskLevel.HIGH]
            ),
            "medium_flags": len(
                [f for f in active_flags if f.risk_level == RiskLevel.MEDIUM]
            ),
            "low_flags": len(
                [f for f in active_flags if f.risk_level == RiskLevel.LOW]
            ),
            "has_thresholds": thresholds is not None,
            "last_threshold_update": (
                thresholds.updated_at.isoformat() if thresholds else None
            ),
    ...
    def get_risk_statistics(self) -> Dict[str, Any]:
        ...
        stats["flag_type_distribution"] = {
            flag_type: flag_types.count(flag_type) for flag_type in set(flag_types)
        }
...
def create_risk_model_compliance_flags(
    rolling_window: int = 252,
) -> RiskModelComplianceFlags:
