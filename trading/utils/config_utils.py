...
class ConfigManager:
        ...
        try:
            with self._lock:
                if filename not in self.configs:
                    return {'success': False, 'error': f'Config {filename} not loaded', 'timestamp': datetime.now().isoformat()}
                
                return {'success': True, 'result': self.configs[filename], 'message': 'Config retrieved successfully', 'timestamp': datetime.now().isoformat()}
    ...
    def stop_hot_reload(self, filename: str) -> None:
        ...
        try:
            ...
            return {'success': True, 'message': f'Hot-reload stopped for {filename}', 'timestamp': datetime.now().isoformat()}
    ...
    def stop_all_hot_reload(self) -> None:
        ...
        try:
            ...
            return {'success': True, 'message': 'All hot-reload observers stopped', 'timestamp': datetime.now().isoformat()}
...
def hot_reload_config(func: Callable) -> Callable:
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            return {'success': True, 'result': result, 'message': 'Function executed successfully', 'timestamp': datetime.now().isoformat()}
        except Exception as e:
            return {'success': False, 'error': str(e), 'timestamp': datetime.now().isoformat()}
    ...
    try:
        ...
        with open(config_path) as f:
            if suffix == '.json':
                ...
                return {'success': False, 'error': f'Unsupported config format: {suffix}', 'timestamp': datetime.now().isoformat()}
        
        logger.info(f"Config loaded from {config_file}")
        
        return {'success': True, 'result': config, 'message': 'Config loaded successfully', 'timestamp': datetime.now().isoformat()}
...
def update_config(config: TradingConfig, updates: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        return {'success': True, 'result': updated_config, 'message': 'Config updated successfully', 'timestamp': datetime.now().isoformat()}
...
def get_env_config() -> Dict[str, Any]:
    ...
    try:
        ...
        return {'success': True, 'result': config, 'message': 'Environment config loaded successfully', 'timestamp': datetime.now().isoformat()}
...
def create_default_config(config_file: str) -> Dict:
    ...
    try:
        ...
        return {'success': True, 'message': f'Default config created at {config_file}', 'timestamp': datetime.now().isoformat()}
...
def validate_config(config: TradingConfig) -> Dict:
    ...
    try:
        ...
        if errors:
            return {'success': False, 'error': f'Config validation failed: {"; ".join(errors)}', 'timestamp': datetime.now().isoformat()}
