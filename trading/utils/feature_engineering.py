...
class FeatureEngineer:
    ...
    def create_technical_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if 'macd' in data.columns:
                result['macd_signal_cross'] = ((data['macd'] > data['macd_signal']) & 
                                             (data['macd'].shift(1) <= data['macd_signal'].shift(1))).astype(int)
                result['macd_negative_cross'] = ((data['macd'] < data['macd_signal']) & 
                                               (data['macd'].shift(1) >= data['macd_signal'].shift(1))).astype(int)
                    ...
                    for window in windows:
                        for func in functions:
                            if func == 'mean':
                                ...
                                result[f'{column}_rolling_median_{window}'] = data[column].rolling(window=window).median()
    ...
    def select_features(self, X: pd.DataFrame, y: pd.Series, 
                       method: str = 'mutual_info', k: int = 10) -> pd.DataFrame:
