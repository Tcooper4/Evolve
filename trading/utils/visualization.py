...
def plot_forecast(data: pd.DataFrame, 
                 ...
                 confidence_intervals: Optional[Tuple[np.ndarray, np.ndarray]] = None) -> Union[go.Figure, Figure]:
...
def _plot_forecast_plotly(data: pd.DataFrame, 
                         predictions: np.ndarray,
                         show_confidence: bool = False,
...
def _plot_forecast_matplotlib(data: pd.DataFrame, 
                             predictions: np.ndarray,
                             show_confidence: bool = False,
                             confidence_intervals: Optional[Tuple[np.ndarray, np.ndarray]] = None) -> Figure:
    ...
    if show_confidence and confidence_intervals is not None:
        lower, upper = confidence_intervals
        ax.fill_between(data.index[-len(predictions):], lower, upper, 
...
def _plot_shap_values_matplotlib(shap_values: np.ndarray, data: pd.DataFrame) -> Figure:
    ...
    # SHAP summary plot
    for i, feature in enumerate(data.columns):
        ax1.scatter(shap_values[:, i], [feature] * len(shap_values), 
                   c=data[feature], cmap='viridis', alpha=0.6)
...
def _plot_backtest_results_matplotlib(results: pd.DataFrame) -> Figure:
    ...
    if 'benchmark_returns' in results.columns:
        ax1.plot(results.index, results['benchmark_returns'], label='Benchmark Returns', 
