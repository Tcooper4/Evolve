...
class ModelRegistry:
            ...
            if metadata_path.exists():
                with open(metadata_path, "r") as f:
                    ...
                    for name, meta_dict in metadata_dict.items():
                        ...
                        meta_dict["updated_at"] = datetime.fromisoformat(
                            meta_dict["updated_at"]
                        )
                ...
                with open(performance_path, "r") as f:
                    ...
                    for model_name, metrics_list in performance_dict.items():
                        ...
                        for metrics_dict in metrics_list:
                            metrics_dict["timestamp"] = datetime.fromisoformat(
                                metrics_dict["timestamp"]
                            )
                            metrics_dict["task_type"] = TaskType(
                                metrics_dict["task_type"]
                            )
                            self.performance_history[model_name].append(
                                PerformanceMetrics(**metrics_dict)
                            )
    ...
    def _save_registry(self):
        ...
        try:
            ...
            metadata_path = Path(self.registry_path) / "metadata.json"
            metadata_dict = {
                name: meta.to_dict() for name, meta in self.model_metadata.items()
            }
    ...
    def get_best_model(
        ...
        try:
            ...
            for name, metadata in self.model_metadata.items():
                ...
                if (
                    name not in self.performance_history
                    or not self.performance_history[name]
                ):
                    continue

                # Get latest performance metrics
                latest_metrics = max(
                    self.performance_history[name], key=lambda x: x.timestamp
                )
            ...
            if best_model:
                self.logger.info(
                    f"âœ… Best model for {task_type.value}: {best_model} (score: {best_score:.4f})"
                )
            else:
                self.logger.warning(
                    f"âš ï¸ No suitable model found for {task_type.value}"
                )
    ...
    def track_performance(
        self, model_name: str, task_type: TaskType, **metrics
    ) -> bool:
        ...
        try:
            ...
            self.logger.error(
                f"âŒ Failed to track performance for model '{model_name}': {e}"
            )
            return False

    def get_performance_history(
        self, model_name: str, days: Optional[int] = None
        ...
        try:
            ...
            if days:
                ...
                history = [
                    metrics for metrics in history if metrics.timestamp >= cutoff_date
                ]
            ...
            self.logger.error(
                f"âŒ Failed to get performance history for model '{model_name}': {e}"
            )
            return []
        ...
        try:
            ...
            if models_without_performance:
                health_status["issues"].append(
                    f"Models without performance data: {models_without_performance}"
                )
                health_status["status"] = "degraded"
