...
class SignalTracer:
    ...
    def __init__(self, 
        ...
        logger.info(f"SignalTracer initialized with safe evaluation: {enable_safe_eval}")
    
    def safe_eval(self, expression: str, context: Optional[Dict[str, Any]] = None) -> Tuple[Any, Optional[str]]:
        """
        ...
        if not self.enable_safe_eval:
            logger.warning("Safe evaluation disabled - using ast.literal_eval() anyway for security")
    ...
    def _is_safe_ast(self, node: ast.AST) -> bool:
        ...
        if isinstance(node, ast.Constant):
            ...
            if isinstance(node, ast.List):
                ...
                return all(self._is_safe_ast(k) for k in node.keys) and \
                       all(self._is_safe_ast(v) for v in node.values)
            elif isinstance(node, ast.Set):
                return all(self._is_safe_ast(elt) for elt in node.elts)
        elif isinstance(node, ast.UnaryOp):
            return isinstance(node.op, (ast.UAdd, ast.USub, ast.Not)) and \
                   self._is_safe_ast(node.operand)
        elif isinstance(node, ast.BinOp):
            return isinstance(node.op, (ast.Add, ast.Sub, ast.Mult, ast.Div, ast.Mod, 
                                      ast.Pow, ast.LShift, ast.RShift, ast.BitOr, 
                                      ast.BitXor, ast.BitAnd, ast.FloorDiv)) and \
                   self._is_safe_ast(node.left) and self._is_safe_ast(node.right)
        elif isinstance(node, ast.Compare):
            return all(isinstance(op, (ast.Eq, ast.NotEq, ast.Lt, ast.LtE, 
                                     ast.Gt, ast.GtE, ast.Is, ast.IsNot, ast.In, ast.NotIn)) 
                      for op in node.ops) and \
                   self._is_safe_ast(node.left) and \
                   all(self._is_safe_ast(comp) for comp in node.comparators)
        else:
            return False
    ...
    def trace_signal(self, 
                    ...
                    level: TraceLevel = TraceLevel.INFO) -> SignalTrace:
        """
    ...
    def trace_evaluation(self, 
                        signal_id: str,
                        expression: str,
                        context: Optional[Dict[str, Any]] = None) -> SignalTrace:
        """
    ...
    def trace_signal_processing(self, 
                              signal_id: str,
                              processing_steps: List[Dict[str, Any]]) -> List[SignalTrace]:
        """
        ...
        for i, step in enumerate(processing_steps):
            ...
            if 'expression' in step_data:
                trace = self.trace_evaluation(signal_id, step_data['expression'], step_data.get('context'))
            else:
    ...
    def get_signal_traces(self, 
                         ...
                         limit: Optional[int] = None) -> List[SignalTrace]:
        """
    ...
    def get_trace_statistics(self) -> Dict[str, Any]:
        ...
        level_counts = {}
        for level in TraceLevel:
            level_counts[level.value] = len([t for t in self.trace_history if t.level == level])
    ...
    def clear_trace_history(self, signal_id: Optional[str] = None):
        ...
        """
        if signal_id:
            self.trace_history = [t for t in self.trace_history if t.signal_id != signal_id]
