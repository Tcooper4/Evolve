...
class ParserEngine:
    ...
    def __init__(
        ...
        logger.info(
            f"ParserEngine initialized with providers: regex={self.use_regex_first}, "
    ...
    def _init_huggingface(self) -> None:
        ...
        try:
            ...
            model_name = (
                "distilgpt2"
                if self.huggingface_model == "gpt2"
                else self.huggingface_model
            )
        ...
        try:
            ...
            if best_match:
                ...
                time_match = re.search(
                    r"(\d+)\s*(days?|weeks?|months?)", prompt, re.IGNORECASE
                )
                if time_match:
                ...
                action_match = re.search(
                    r"\b(buy|sell|hold|long|short)\b", prompt, re.IGNORECASE
                )
                if action_match:
    ...
    def _calculate_route_score(
        self, route: StrategyRoute, intent: str, args: Dict[str, Any]
    ) -> float:
    ...
    def get_registry_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "strategies": list(
                set(route.strategy_name for route in self.strategy_registry.values())
            ),
            "last_updated": datetime.now().isoformat(),
        }
