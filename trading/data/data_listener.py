...
class DataListener:
                ...
                if news.get("success"):
                    ...
                    for item in news.get("news_items", []):
                        ...
                        if self._is_significant_news(item):
                            self.paused = True
                            logger.warning(
                                f"Significant news detected: {item.get('title', 'Unknown')}. Pausing trading."
                            )
    ...
    def set_volatility_threshold(self, threshold: float) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "success": True,
                "message": f"Volatility threshold updated to {threshold}",
                "timestamp": time.time(),
            }
    ...
    def get_stream_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if status.get("success"):
                ...
                return {"success": True, "result": info, "timestamp": time.time()}
            return status
