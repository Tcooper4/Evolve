...
class MacroDataIntegration:
    ...
    def _get_fallback_fred_data(self, series_id: str) -> pd.DataFrame:
        ...
        try:
            ...
            if series_id in ['DGS10', 'DGS2', 'DGS3MO']:
                ...
                values = base_rate + 0.5 * np.sin(np.linspace(0, 4*np.pi, len(dates))) + np.random.normal(0, 0.1, len(dates))
                ...
                values = 4.0 + 0.5 * np.sin(np.linspace(0, 2*np.pi, len(dates))) + np.random.normal(0, 0.1, len(dates))
    ...
    def _create_fallback_yield_curve(self, date: datetime) -> YieldCurveData:
        ...
        rates = {
            '1M': 4.8, '3M': 5.0, '6M': 5.1, '1Y': 4.9, '2Y': 4.5,
