...
class ParallelProcessor:
    ...
    def load_tickers_parallel(self, requests: List[DataLoadRequest], loader_instance) -> Dict[str, DataLoadResponse]:
        ...
        logger.info(f"Completed parallel load: {len([r for r in results.values() if r.success])}/{len(results)} successful")
        return results
    ...
    def load_tickers_in_chunks(self, requests: List[DataLoadRequest], loader_instance) -> Dict[str, DataLoadResponse]:
        ...
        chunks = [requests[i:i + self.config.chunk_size] 
...
class DataLoader:
    ...
    def load_multiple_tickers(self, requests: List[DataLoadRequest], 
                            use_parallel: bool = True,
                            progress_callback: Optional[callable] = None) -> Dict[str, DataLoadResponse]:
