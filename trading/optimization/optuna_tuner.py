...
class SharpeOptunaTuner:
    ...
    def optimize_lstm(
        ...
        def objective(trial: optuna.Trial) -> float:
            ...
            try:
                ...
                # Prepare sequences
                X_train_seq = self._prepare_sequences(
                    X_train.values, params["sequence_length"]
                )
        ...
        logger.info(
            f"LSTM optimization completed. Best Sharpe ratio: {self.best_scores['lstm']:.4f}"
        )
    ...
    def optimize_xgboost(
        ...
        logger.info(
            f"XGBoost optimization completed. Best Sharpe ratio: {self.best_scores['xgboost']:.4f}"
        )
    ...
    def optimize_transformer(
        ...
        logger.info(
            f"Transformer optimization completed. Best Sharpe ratio: {self.best_scores['transformer']:.4f}"
        )
    ...
    def optimize_all_models(
        ...
        # Find best model
        best_model = max(
            results.keys(), key=lambda k: results[k].get("best_score", -1.0)
        )
    ...
    def get_model_recommendation(
        ...
        return {
            ...
            "confidence": (
                "high" if best_score > 0.5 else "medium" if best_score > 0.2 else "low"
            ),
...
def get_sharpe_optuna_tuner(
    study_name: str = "trading_optimization", n_trials: int = 100, timeout: int = 3600
) -> SharpeOptunaTuner:
    """
