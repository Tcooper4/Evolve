...
class RSIOptimizer(BaseOptimizer):
    ...
    def optimize(self,
                 ...
                 n_trials: int = 20) -> RSIOptimizationResult:
                ...
                for oversold in oversolds:
                    ...
                    try:
                        ...
                        if sharpe > best_score:
                            best_score = sharpe
                            best_result = result
                        logger.debug(f"Tested period={period}, overbought={overbought}, oversold={oversold}, Sharpe={sharpe:.3f}, WinRate={win_rate:.3f}")
                    except Exception as e:
                        logger.warning(f"Failed for period={period}, overbought={overbought}, oversold={oversold}: {e}")
        logger.info(f"Best Sharpe: {best_result.sharpe_ratio:.3f} (period={best_result.period}, overbought={best_result.overbought}, oversold={best_result.oversold})")
        return best_result
    ...
    def plot_signals(self, result: OptimizationResult, title: str = "RSI Signals") -> go.Figure:
        ...
        fig.add_trace(
            go.Scatter(
                ...
                name='Signals',
                marker=dict(
                    color=['red' if s == -1 else 'green' if s == 1 else 'gray' for s in result.signals.values],
                    size=8
                )
            ),
            row=2, col=1
