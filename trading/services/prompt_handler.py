...
class PromptHandler:
    ...
    def handle_prompt(self, 
        ...
        if self.log_prompt_content:
            logger.info(f"Processing prompt {prompt_id} (hash: {prompt_hash}): {prompt_content[:100]}...")
        else:
            logger.info(f"Processing prompt {prompt_id} (hash: {prompt_hash})")
        
        return prompt_id
    
    def log_routing_failure(self, 
                           prompt_id: str, 
                           error: Exception,
        ...
        if prompt_id not in self.prompt_history:
            logger.warning(f"Attempted to log routing failure for unknown prompt: {prompt_id}")
            return
    ...
    def log_processing_failure(self, 
                              prompt_id: str, 
                              error: Exception,
        ...
        if prompt_id not in self.prompt_history:
            logger.warning(f"Attempted to log processing failure for unknown prompt: {prompt_id}")
            return
    ...
    def log_gpt_fallback_failure(self, 
                                prompt_id: str, 
                                error: Exception,
        ...
        if prompt_id not in self.prompt_history:
            logger.warning(f"Attempted to log GPT fallback failure for unknown prompt: {prompt_id}")
            return
    ...
    def log_successful_routing(self, 
                              prompt_id: str, 
        ...
        """
        if prompt_id not in self.prompt_history:
            logger.warning(f"Attempted to log successful routing for unknown prompt: {prompt_id}")
            return
        ...
        logger.info(f"Successful routing for prompt {prompt_id} (hash: {record.prompt_hash})")
    
    def log_successful_processing(self, 
                                 prompt_id: str, 
        ...
        """
        if prompt_id not in self.prompt_history:
            logger.warning(f"Attempted to log successful processing for unknown prompt: {prompt_id}")
            return
        ...
        logger.info(f"Successful processing for prompt {prompt_id} (hash: {record.prompt_hash})")
    ...
    def get_prompts_by_hash(self, prompt_hash: str) -> List[PromptRecord]:
        ...
        """
        return [
            record for record in self.prompt_history.values()
    ...
    def get_failed_prompts(self, 
                          error_type: Optional[str] = None) -> List[PromptRecord]:
        """
        Get failed prompts.
        ...
        for record in self.prompt_history.values():
            if record.status in [PromptStatus.FAILED, PromptStatus.FALLBACK_FAILED]:
                if error_type is None or any(e.error_type == error_type for e in record.errors):
