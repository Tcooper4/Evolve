...
class APILoggingMiddleware:
    ...
    async def _log_execution(self, func: Callable, endpoint: str, method: str, *args, **kwargs):
        ...
        try:
            ...
            self.logger.info(f"✅ API Request Completed | ID: {request_id} | {method} {endpoint} | Time: {execution_time:.3f}s")
            ...
            # Log error
            self.logger.error(f"❌ API Request Failed | ID: {request_id} | {method} {endpoint} | Time: {execution_time:.3f}s | Error: {str(e)}")
    ...
    def log_stats_summary(self):
        ...
        for endpoint, stats in summary['endpoints'].items():
            ...
            self.logger.info(f"  Response Time Range: {stats['min_response_time']:.3f}s - {stats['max_response_time']:.3f}s")
