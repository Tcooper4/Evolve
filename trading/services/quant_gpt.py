...
class QuantGPT:
    ...
    def __init__(self, config: Optional[QuantGPTConfig] = None, 
                 openai_api_key: str = None, redis_host: str = 'localhost', 
        ...
        for attempt in range(self.config.max_retries):
            try:
                ...
                if attempt == self.config.max_retries - 1:
                    raise QueryParsingError(f"Failed to parse query after {self.config.max_retries} attempts: {e}", query)
    ...
    def _execute_action_with_retry(self, parsed: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for attempt in range(self.config.max_retries):
            try:
                ...
                if attempt == self.config.max_retries - 1:
                    raise ActionExecutionError(f"Failed to execute action after {self.config.max_retries} attempts: {e}", parsed.get('intent'))
                time.sleep(self.config.retry_delay)
    ...
    def _generate_commentary_with_retry(self, query: str, parsed: Dict[str, Any], result: Dict[str, Any]) -> str:
        ...
        for attempt in range(self.config.max_retries):
            try:
                ...
                if attempt == self.config.max_retries - 1:
                    raise CommentaryGenerationError(f"Failed to generate commentary after {self.config.max_retries} attempts: {e}")
                time.sleep(self.config.retry_delay)
