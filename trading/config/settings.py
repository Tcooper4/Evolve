...
def validate_config() -> bool:
    ...
    # Validate ports
    for port_name, port_value in [('WEB_PORT', WEB_PORT), ('PROMETHEUS_PORT', PROMETHEUS_PORT), ('GRAFANA_PORT', GRAFANA_PORT)]:
    ...
    # Validate timeouts
    for timeout_name, timeout_value in [('AGENT_TIMEOUT', AGENT_TIMEOUT), ('EXECUTION_TIMEOUT', EXECUTION_TIMEOUT), ('LLM_TIMEOUT', LLM_TIMEOUT)]:
        if timeout_value <= 0:
            errors.append(f"Invalid {timeout_name}: {timeout_value}")
    
    # Validate intervals
    for interval_name, interval_value in [('AGENT_HEARTBEAT_INTERVAL', AGENT_HEARTBEAT_INTERVAL), ('MEMORY_CLEANUP_INTERVAL', MEMORY_CLEANUP_INTERVAL)]:
        if interval_value <= 0:
