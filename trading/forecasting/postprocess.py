...
class ForecastPostprocessor:
    ...
    def __init__(self, config: Optional[PostprocessConfig] = None):
        ...
        logger.info(
            f"ForecastPostprocessor initialized with {self.config.alpha_method.value} alpha method"
        )

    def calculate_dynamic_alpha(
        self,
        forecast: Union[np.ndarray, pd.Series],
        method: Optional[AlphaMethod] = None,
    ) -> float:
        """
    ...
    def _calculate_std_based_alpha(
        self, forecast: Union[np.ndarray, pd.Series]
    ) -> float:
        """
        ...
        logger.debug(
            f"STD-based alpha: {alpha:.4f} (std: {forecast_std:.4f}, max: {forecast_max:.4f})"
        )
        return alpha

    def _calculate_volatility_based_alpha(
        self, forecast: Union[np.ndarray, pd.Series]
    ) -> float:
        """
    ...
    def _calculate_hybrid_alpha(self, forecast: Union[np.ndarray, pd.Series]) -> float:
        ...
        logger.debug(
            f"Hybrid alpha: {alpha:.4f} (std: {std_alpha:.4f}, vol: {vol_alpha:.4f})"
        )
        return alpha

    def apply_ewma_smoothing(
        self, forecast: Union[np.ndarray, pd.Series], alpha: Optional[float] = None
    ) -> np.ndarray:
        """
    ...
    def postprocess_forecast(
        ...
        logger.info(
            f"Original std: {original_stats['std']:.4f}, Smoothed std: {smoothed_stats['std']:.4f}"
        )
    ...
    def get_processing_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "mean_smoothing_ratio": (
                float(np.mean(smoothing_ratios)) if smoothing_ratios else 0.0
            ),
    ...
    def update_config(self, new_config: PostprocessConfig):
        ...
        logger.info(
            f"Updated config: {new_config.alpha_method.value}, dynamic_alpha: {new_config.enable_dynamic_alpha}"
        )


def create_forecast_postprocessor(
