    ...
    def calculate_hybrid_volatility(self, returns: pd.Series) -> pd.Series:
        ...
        if GARCH_AVAILABLE:
            try:
                ...
                hybrid_vol[recent_mask] = (
                    0.4 * rolling_vol[recent_mask]
                    + 0.3 * ewma_vol[recent_mask]
                    + 0.3 * garch_vol[recent_mask]
                )
            except Exception as e:
                self.logger.warning(
                    f"GARCH calculation failed in hybrid volatility: {e}"
                )
        ...
        try:
            if self.model_type == VolatilityModel.ROLLING_STD:
                ...
                if GARCH_AVAILABLE:
                    ...
                    current_vol = (
                        results.conditional_volatility.iloc[-1] / 100 * np.sqrt(252)
                    )
                    forecast_vol = (
                        forecast.variance.iloc[-1] ** 0.5 / 100 * np.sqrt(252)
                    )
                else:
                    current_vol = self.calculate_rolling_volatility(returns).iloc[-1]
...
class RiskManager:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.position_config = self.config.get(
            "position_sizing",
            {
    ...
    def export_risk_report(self, filepath: str) -> Dict[str, Any]:
        ...
        try:
            ...
            report_data = {
                ...
                "data_summary": {
                    "total_observations": (
                        len(self.returns) if self.returns is not None else 0
                    ),
                    "date_range": {
                        "start": (
                            self.returns.index[0].isoformat()
                            if self.returns is not None and len(self.returns) > 0
                            else None
                        ),
                        "end": (
                            self.returns.index[-1].isoformat()
                            if self.returns is not None and len(self.returns) > 0
                            else None
                        ),
                    },
    ...
    def calculate_dynamic_position_size(
        ...
        try:
            ...
            volatility_forecast = self.volatility_model.forecast_volatility(
                self.returns
            )
            ...
            if self.position_config.get("risk_adjustment", True):
                ...
                kelly_fraction = (
                    self.current_metrics.kelly_fraction if self.current_metrics else 0.0
                )
                risk_adjusted_size = volatility_adjusted_size * max(0, kelly_fraction)

                # Confidence adjustment
                confidence_threshold = self.position_config.get(
                    "confidence_threshold", 0.7
                )
            ...
            self.logger.info(
                f"Position size calculated for {symbol}: {final_recommendation:.4f}"
            )
    ...
    def update_volatility_model(self, model_type: VolatilityModel, **kwargs) -> bool:
