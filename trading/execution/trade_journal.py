...
class TradeJournal:
    ...
    def add_trade(
        ...
        **kwargs
        ...
        try:
            ...
            self.logger.info(f"Added trade {trade_id}: {side} {quantity} {symbol} @ {entry_price}")
    ...
    def _calculate_trade_pnl(self, trade: Trade) -> None:
        ...
        # Subtract costs
        total_costs = trade.commission + (trade.slippage * trade.quantity * trade.entry_price)
    ...
    def _update_daily_pnl(self, trade: Trade) -> None:
        ...
        if trade.entry_time:
            ...
            self.daily_pnl[date_key] = self.daily_pnl.get(date_key, 0) + (trade.pnl or 0)
    ...
    def summarize(self, start_date: Optional[datetime] = None, end_date: Optional[datetime] = None) -> TradeSummary:
        """
        ...
        try:
            ...
            average_return_per_trade = total_pnl / total_trades if total_trades > 0 else 0.0
            ...
            average_holding_period = np.mean(holding_periods) if holding_periods else timedelta(0)
            
            # Volume and cost statistics
            total_volume = sum(trade.quantity * trade.entry_price for trade in closed_trades)
            total_commission = sum(trade.commission for trade in closed_trades)
            total_slippage = sum(trade.slippage * trade.quantity * trade.entry_price for trade in closed_trades)
            ...
            strategies_used = list(set(trade.strategy for trade in closed_trades if trade.strategy))
