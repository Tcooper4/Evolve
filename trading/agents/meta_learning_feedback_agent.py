...
class MetaLearningFeedbackAgent(BaseAgent):
    ...
    async def process_model_feedback(self, feedback: ModelFeedback):
        ...
        try:
            ...
            # Log feedback
            self.logger.info(
                f"Processed feedback for {feedback.model_name}: "
    ...
    def _store_feedback(self, feedback: ModelFeedback):
        ...
        try:
            ...
            key_metrics = [
                feedback.mse,
                feedback.sharpe_ratio,
                feedback.win_rate,
                feedback.max_drawdown,
                feedback.actual_return,
                feedback.predicted_return,
            ]
            if any(
                isinstance(x, float) and (np.isnan(x) or x is None) for x in key_metrics
            ):
                # Fallback: use last valid feedback if available
                if (
                    feedback.model_name in self.feedback_history
                    and self.feedback_history[feedback.model_name]
                ):
                    last_valid = next(
                        (
                            f
                    ...
                    if last_valid:
                        self.logger.warning(
                            f"NaN detected in feedback for {feedback.model_name}, using last valid feedback instead."
                        )
                        feedback = last_valid
                    else:
                        self.logger.warning(
    ...
    async def _update_ensemble_weights(self):
        ...
        try:
            ...
            if last_update and (datetime.now() - last_update["timestamp"]).days < 7:
                return
    ...
    def get_model_performance_summary(
        ...
        try:
            ...
            return {
                ...
                "last_updated": (
                    recent_feedback[-1].timestamp if recent_feedback else None
                ),
            }
