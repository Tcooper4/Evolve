...
class PromptResponseValidator:
    ...
    def __init__(self, 
        ...
        logger.info(f"PromptResponseValidator initialized with level: {validation_level.value}")
    ...
    def _create_strategy_schema(self) -> StrategySchema:
        ...
        schema.optional_fields = ['confidence', 'timestamp', 'strategy_name', 'metadata']
        
        return schema
    
    def validate_strategy_result(self, 
                               data: Dict[str, Any],
                               schema: Optional[StrategySchema] = None) -> ValidationResult:
        """
        ...
        if result.is_valid:
            logger.debug(f"Strategy result validation passed with {len(result.warnings)} warnings")
        else:
            logger.warning(f"Strategy result validation failed: {result.errors}")
        
        return result
    
    def _check_required_fields(self, data: Dict[str, Any], schema: StrategySchema) -> List[str]:
    ...
    def _validate_field(self, 
                       field_name: str, 
                       field_value: Any, 
                       schema: StrategySchema) -> List[str]:
        ...
        if field_name in schema.field_types:
            ...
            if not isinstance(field_value, expected_type):
                errors.append(f"Field '{field_name}' has wrong type. Expected {expected_type.__name__}, got {type(field_value).__name__}")
        ...
        if field_name in schema.field_validators:
            try:
                ...
                if validator_errors:
                    errors.extend([f"Field '{field_name}': {error}" for error in validator_errors])
            except Exception as e:
    ...
    def _check_warnings(self, data: Dict[str, Any], schema: StrategySchema) -> List[str]:
    ...
    def validate_prompt_response(self, 
                               response: str,
                               expected_format: str = "json") -> ValidationResult:
        """
    ...
    def get_validation_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            'avg_validation_time': np.mean([r.validation_time for r in self.validation_history]),
