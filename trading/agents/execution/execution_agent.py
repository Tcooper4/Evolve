...
# Modular imports
from .risk_controls import RiskControls, RiskThreshold, RiskThresholdType, ExitReason, create_default_risk_controls
from .trade_signals import TradeSignal, ExecutionRequest, ExecutionResult
from .execution_providers import ExecutionMode, ExecutionProvider, create_execution_provider
...
class ExecutionAgent(BaseAgent):
    ...
    def _initialize_execution_providers(self) -> None:
        ...
        if self.config.get("enable_ib", False):
            ...
            self.execution_providers[ExecutionMode.INTERACTIVE_BROKERS] = create_execution_provider(
                ExecutionMode.INTERACTIVE_BROKERS, ib_config
            )
        
        if self.config.get("enable_robinhood", False):
            rh_config = provider_configs.get("robinhood", {})
            self.execution_providers[ExecutionMode.ROBINHOOD] = create_execution_provider(
                ExecutionMode.ROBINHOOD, rh_config
            )
    ...
    def _check_position_limits(self, signal: TradeSignal) -> bool:
        ...
        existing_positions = self.position_manager.get_positions_by_symbol(signal.symbol)
    ...
    async def _execute_real_trade(self, signal: TradeSignal, execution_price: float) -> Position:
        ...
        if not provider:
            raise RuntimeError(f"No execution provider available for mode: {self.execution_mode}")
        
        # Execute via provider
        result = await provider.execute_trade(signal)
        
        if not result.get("success", False):
            raise RuntimeError(f"Trade execution failed: {result.get('error', 'Unknown error')}")
        
        # Create position from execution result
        position = Position(
            position_id=result.get("order_id", f"real_{len(self.position_manager.positions) + 1}"),
            symbol=signal.symbol,
            direction=signal.direction,
