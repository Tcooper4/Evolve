...
from .risk_controls import ExitEvent
...
class PositionManager:
    ...
    def calculate_position_risk_metrics(
        ...
        max_price = (
            position.max_price if hasattr(position, "max_price") else entry_price
        )
        drawdown = (
            ((max_price - current_price) / max_price) * 100
            if position.direction.value == "long"
            else ((current_price - max_price) / max_price) * 100
        )
    ...
    def calculate_portfolio_risk_metrics(
        ...
        for position in self.positions.values():
            current_price = market_data.get(
                f"{position.symbol}_price", position.entry_price
            )
