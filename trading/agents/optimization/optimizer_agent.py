...
class OptimizerAgent(BaseAgent):
        ...
        try:
            ...
            return AgentResult(
                ...
                data={
                    ...
                    "best_score": (
                        max([r.optimization_score for r in results]) if results else 0.0
                    ),
                },
            )
    ...
    async def _run_optimization(
        ...
        try:
            if config.optimization_type == OptimizationType.STRATEGY_COMBINATION:
                results = await self.strategy_optimizer.optimize_strategy_combinations(
                    config
                )
    ...
    async def _update_agent_configs(
        ...
        try:
            ...
            self.logger.info(
                f"Updating agent configs with best parameters: {best_result.parameter_combination}"
            )
    ...
    async def _update_strategy_configs(
        ...
        try:
            ...
            for strategy_config in config.strategy_configs:
                ...
                strategy_params = {
                    k: v
                    for k, v in best_result.parameter_combination.items()
    ...
    async def _save_strategy_config(
        ...
        try:
            ...
            self.logger.error(
                f"Failed to save strategy config for {strategy_name}: {e}"
            )
...
def create_optimizer_agent(config: Optional[Dict[str, Any]] = None) -> OptimizerAgent:
    ...
    agent_config = AgentConfig(
        name="OptimizerAgent", agent_type="optimization", config=config
    )
