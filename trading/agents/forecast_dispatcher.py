...
class ForecastDispatcher(BaseAgent):
    ...
    async def execute(self, **kwargs) -> AgentResult:
        ...
        try:
            ...
            if data is None or data.empty:
                return AgentResult(
                    success=False,
                    error_message="No data provided for forecasting"
                )
            ...
            if enable_consensus and len(forecast_results) >= self.consensus_config['min_models_for_consensus']:
                consensus_result = self._check_consensus(forecast_results)
            
            # Select best forecast
            best_forecast = self._select_best_forecast(forecast_results, consensus_result)
            ...
            return AgentResult(
                ...
                extra_metrics={
                    ...
                    'consensus_agreement': consensus_result.agreement_level if consensus_result else None,
        ...
        for model_name in models_to_use:
            ...
            try:
                ...
                if forecast_result.success:
                    ...
                    nan_fraction = np.isnan(forecast_values).sum() / len(forecast_values)
                    ...
                    if nan_fraction <= self.fallback_config['nan_threshold']:
                        ...
                        result = ForecastResult(
                            ...
                            confidence_intervals=forecast_result.data.get('confidence_intervals'),
    ...
    def _check_consensus(self, forecast_results: List[ForecastResult]) -> ConsensusResult:
        ...
        model_names = [result.model_name for result in forecast_results]
        ...
        consensus_confidence = agreement_level * self.consensus_config['confidence_weight'] + \
    ...
    def _calculate_model_weights(self, forecast_results: List[ForecastResult]) -> Dict[str, float]:
        ...
        for result in forecast_results:
            # Performance weight
            performance_score = self.model_performance.get(result.model_name, {}).get('score', 0.5)
            ...
            if result.confidence_intervals:
                ...
                confidence_score = 1.0 / (1.0 + avg_width)  # Narrower intervals = higher confidence
            
            # Combined weight
            weight = (performance_score * self.consensus_config['performance_weight'] +
    ...
    def _select_best_forecast(
        ...
        if (consensus_result and 
            consensus_result.agreement_level > self.consensus_config['agreement_threshold']):
        ...
        best_result = max(forecast_results, key=lambda r: 
            self.model_performance.get(r.model_name, {}).get('score', 0.5))
        
        self.logger.info(f"Selected best individual model: {best_result.model_name}")
        
        return best_result
    
    def _update_performance_tracking(self, forecast_results: List[ForecastResult]) -> None:
        ...
        for result in forecast_results:
            ...
            perf['avg_execution_time'] = (
                (perf['avg_execution_time'] * (perf['total_runs'] - 1) + result.execution_time) /
                perf['total_runs']
            )
