...
class RegimeDetectionAgent(BaseAgent):
    ...
    def __init__(self, config: Optional[AgentConfig] = None):
        ...
        # Configuration
        self.lookback_periods = self.config.custom_config.get('lookback_periods', {
            'short': 20,
            'medium': 50,
            'long': 200
        })
        
        self.volatility_thresholds = self.config.custom_config.get('volatility_thresholds', {
            'low': 0.15,
            'high': 0.35
        })
        
        self.trend_thresholds = self.config.custom_config.get('trend_thresholds', {
            'weak': 0.1,
            'strong': 0.3
        })
    ...
    def detect_regime(self, data: pd.DataFrame, symbol: str = "UNKNOWN") -> RegimeResult:
        ...
        try:
            ...
            self.logger.info(f"Detected {final_regime.value} regime for {symbol} with confidence {final_confidence:.2f}")
    ...
    def _calculate_regime_metrics(self, data: pd.DataFrame) -> RegimeMetrics:
        ...
        try:
            ...
            momentum = (close_prices[-1] - close_prices[-self.lookback_periods['short']]) / close_prices[-self.lookback_periods['short']]
            ...
            if 'volume' in data.columns:
                ...
                if len(volume) >= self.lookback_periods['short']:
                    ...
                    historical_volume = np.mean(volume[-self.lookback_periods['medium']:-self.lookback_periods['short']])
