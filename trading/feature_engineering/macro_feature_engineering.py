    ...
    def _add_rolling_features(self, df: pd.DataFrame, macro_columns: List[str]) -> pd.DataFrame:
        ...
        for col in macro_columns:
            if col in df.columns:
                ...
                df[f'{col}_zscore_30'] = (df[col] - df[col].rolling(window=30).mean()) / df[col].rolling(window=30).std()
    ...
    def get_feature_importance(self, df: pd.DataFrame, target_col: str = 'returns') -> Dict[str, float]:
        ...
        try:
            ...
            feature_cols = [col for col in df.columns if any(
                macro in col for macro in ['CPIAUCSL', 'GDP', 'UNRATE', 'FEDFUNDS', 'GS10', 'VIXCLS']
            )]
