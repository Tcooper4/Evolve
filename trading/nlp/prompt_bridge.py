...
class PromptBridge:
    ...
    def __init__(self, 
        ...
        logger.info(f"PromptBridge initialized with regex fallback: {enable_regex_fallback}")
    ...
    def _initialize_intent_patterns(self) -> Dict[str, List[Tuple[str, IntentType]]]:
        ...
        patterns = {
            'forecast': [
                ...
                (r'\b(price|market|trend)\s+(prediction|forecast)\b', IntentType.FORECAST),
                (r'\b(future|upcoming)\s+(price|value)\b', IntentType.FORECAST)
            ],
            'strategy': [
                (r'\b(apply|use|run|execute)\s+(strategy|strategy)\b', IntentType.STRATEGY),
                (r'\b(RSI|MACD|SMA|BB|Bollinger|Moving\s+Average)\b', IntentType.STRATEGY),
                (r'\b(trading|investment)\s+(strategy|approach)\b', IntentType.STRATEGY)
            ],
            'analysis': [
            ...
            'optimization': [
                ...
                (r'\b(parameter|hyperparameter)\s+(optimization|tuning)\b', IntentType.OPTIMIZATION),
                (r'\b(improve|enhance|boost)\s+(performance|returns)\b', IntentType.OPTIMIZATION)
            ],
            'monitoring': [
                (r'\b(monitor|watch|track|observe)\b', IntentType.MONITORING),
                (r'\b(alert|notify|report)\b', IntentType.MONITORING),
                (r'\b(dashboard|status|health)\s+(check|monitor)\b', IntentType.MONITORING)
            ]
        }
        return patterns
        ...
        try:
            ...
            for intent in intents:
                if intent.confidence >= self.min_confidence_threshold:
                    ...
                    logger.warning(f"Low confidence intent filtered: {intent.action} (confidence: {intent.confidence})")
            ...
            if result.success:
                logger.info(f"Successfully parsed {len(valid_intents)} intents from prompt")
            else:
    ...
    def _is_compound_prompt(self, prompt_text: str) -> bool:
        ...
        action_verbs = ['forecast', 'apply', 'run', 'analyze', 'execute', 'optimize', 'monitor']
        verb_count = sum(1 for verb in action_verbs if verb.lower() in prompt_text.lower())
    ...
    def split_intents(self, prompt_text: str) -> List[ParsedIntent]:
        ...
        for i, text in enumerate(split_texts):
            if len(intents) >= self.max_compound_intents:
                logger.warning(f"Maximum compound intents ({self.max_compound_intents}) reached")
                break
    ...
    def _parse_single_intent(self, text: str) -> ParsedIntent:
        ...
        if intent.confidence < self.min_confidence_threshold and self.enable_regex_fallback:
    ...
    def _extract_parameters(self, text: str, intent_type: IntentType) -> Dict[str, Any]:
        ...
        if intent_type == IntentType.STRATEGY:
            ...
            # Extract quantities
            quantity_match = re.search(r'\b(\d+)\s*(shares|units|contracts)\b', text, re.IGNORECASE)
            if quantity_match:
    ...
    def resolve_dependencies(self, intents: List[ParsedIntent]) -> List[ParsedIntent]:
        ...
        optimization_intents = [i for i in intents if i.intent_type == IntentType.OPTIMIZATION]
        ordered_intents.extend(optimization_intents)
        
        # Execution intents last
        execution_intents = [i for i in intents if i.intent_type == IntentType.EXECUTION]
        ordered_intents.extend(execution_intents)
        
        # Monitoring intents
        monitoring_intents = [i for i in intents if i.intent_type == IntentType.MONITORING]
    ...
    def get_parse_statistics(self) -> Dict[str, Any]:
        ...
        compound_parses = sum(1 for result in self.parse_history if result.compound_detected)
        ...
        stats = {
            ...
            'avg_parse_time': sum(r.parse_time for r in self.parse_history) / total_parses,
