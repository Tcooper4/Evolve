...
class SentimentBridge:
        ...
        try:
            for lexicon_name, lexicon in self.sentiment_lexicons.items():
                ...
                if words:
                    ...
                    logger.info(
                        f"Pre-computed embeddings for {len(words)} words in {lexicon_name} lexicon"
                    )
        except Exception as e:
    ...
    def score_text(
        self, text: str, lexicon_name: str = "basic", use_soft_matching: bool = None
    ) -> LexiconScore:
        """
        ...
        confidence = min(
            1.0, (len(matched_words) + len(soft_matches)) / max(word_count, 1)
        )
    ...
    def _find_soft_matches(
        self, words: List[str], lexicon_name: str
    ) -> List[SoftMatchResult]:
        """
            ...
            try:
                ...
                for lexicon_word, lexicon_embedding in lexicon_embeddings.items():
                    similarity = cosine_similarity(
                        word_embedding, lexicon_embedding.reshape(1, -1)
                    )[0][0]
    ...
    def score_batch(
        self, texts: List[str], lexicon_name: str = "basic"
    ) -> List[LexiconScore]:
        """
    ...
    def add_custom_lexicon(
        self, name: str, lexicon: Dict[str, float], compute_embeddings: bool = True
    ):
        """
        ...
        if compute_embeddings and self.enable_soft_matching:
            try:
                ...
                if words:
                    ...
                    logger.info(
                        f"Added custom lexicon '{name}' with {len(words)} words"
                    )
            except Exception as e:
                logger.error(
                    f"Failed to compute embeddings for custom lexicon '{name}': {e}"
                )
