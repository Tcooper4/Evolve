...
class HybridEngine:
    ...
    def generate_signals(self, data: pd.DataFrame, symbol: str) -> HybridSignal:
        ...
        try:
            ...
            logger.info(f"Generated hybrid signal for {symbol}: {hybrid_signal.signal_type.value} "
    ...
    def get_performance_metrics(self) -> Dict[str, float]:
        ...
        metrics = {
            ...
            'buy_signals': len([s for s in self.signal_history if s.signal_type in [SignalType.BUY, SignalType.STRONG_BUY]]),
            'sell_signals': len([s for s in self.signal_history if s.signal_type in [SignalType.SELL, SignalType.STRONG_SELL]]),
