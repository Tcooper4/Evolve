...
class RegimeClassifier:
    ...
    def _determine_regime(self, volatility: float, momentum: float, 
                         trend_strength: float, volume_profile: str) -> Tuple[MarketRegime, float]:
...
class StrategyGatekeeper:
    ...
    def evaluate_strategy(self, strategy_name: str, data: pd.DataFrame, 
                         strategy_signals: pd.DataFrame) -> GatekeeperDecision:
    ...
    def _calculate_strategy_performance(self, strategy_name: str, data: pd.DataFrame,
                                      signals: pd.DataFrame, regime_metrics: RegimeMetrics) -> StrategyPerformance:
    ...
    def _calculate_regime_performance(self, returns: pd.Series, 
    ...
    def _make_decision(self, strategy_name: str, performance: StrategyPerformance,
                      regime_metrics: RegimeMetrics) -> GatekeeperDecision:
        ...
        if performance.max_drawdown > self.max_drawdown_threshold:
            reasoning.append(f"Max drawdown {performance.max_drawdown:.3f} above threshold {self.max_drawdown_threshold}")
            risk_score += 0.4
    ...
    def get_active_strategies(self) -> Dict[str, StrategyPerformance]:
        ...
        return {name: perf for name, perf in self.strategy_performance.items()
    ...
    def get_strategy_recommendations(self, regime: MarketRegime) -> List[Tuple[str, float]]:
        ...
        for name, performance in active_strategies.items():
            ...
            score = (performance.sharpe_ratio * 0.4 + 
