...
class IntelligentForecastExplainability:
    ...
    def _bootstrap_confidence_interval(
        self, predictions: np.ndarray, confidence_level: float, n_bootstrap: int = 1000
    ) -> ConfidenceInterval:
    ...
    def calculate_feature_importance(
        self, model: Any, X: pd.DataFrame, method: str = "shap"
        ...
        try:
            if method == "shap" and SHAP_AVAILABLE:
                ...
                return {
                    "success": True,
                    "result": self._calculate_model_importance(model, X),
                    "message": "Operation completed successfully",
                    "timestamp": datetime.now().isoformat(),
                }
    ...
    def _identify_risk_factors(
        ...
        confidence_intervals: List[ConfidenceInterval],
        feature_importance: List[FeatureImportance],
        actual_values: Optional[pd.Series],
    ) -> List[str]:
