                ...
                for existing_id in existing_ids:
                    if existing_id in self._tasks:
                        ...
                        if existing_task.status in [
                            ...
                            logger.warning(
                                f"âš ï¸ Duplicate task detected: {task.id} matches {existing_id}"
                            )
                            return False
...
class LocalTaskQueue:
    ...
    def put(self, task: Task) -> bool:
        ...
        with self._lock:
            ...
            self._size += 1
            logger.debug(
                f"ðŸ“¥ Added task {task.id} to queue (priority: {task.priority.name})"
            )
            return True
        ...
        with self._lock:
            ...
            for priority in reversed(list(TaskPriority)):
                if self._queues[priority]:
                    ...
                    self._size -= 1
                    logger.debug(
                        f"ðŸ“¤ Retrieved task {task.id} from queue (priority: {priority.name})"
                    )
                    return task
            return None
...
class RedisTaskQueue:
    ...
    def __init__(
        self, redis_url: str = "redis://localhost:6379", queue_name: str = "task_queue"
    ):
...
class TaskDispatcher:
    ...
    def __init__(
        ...
        self.redis_queue = (
            RedisTaskQueue(redis_url) if enable_redis and redis_url else None
        )
        ...
        **kwargs,
        ...
        if self.redis_queue:
            ...
            if not enqueued:
                ...
                logger.warning(
                    f"âš ï¸ Redis failed, falling back to local queue for task {task_id}"
                )
    ...
    async def _handle_task_failure(self, task: Task, worker_name: str) -> None:
        ...
        if task.retry_count <= task.max_retries:
            ...
            logger.warning(
                f"ðŸ”„ Retrying task {task.id} (attempt {task.retry_count}/{task.max_retries})"
            )
            ...
            logger.error(
                f"âŒ Task {task.id} failed permanently after {task.max_retries} retries"
            )
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        return {
            **self._metrics,
            ...
            + (self.redis_queue.size() if self.redis_queue else 0),
