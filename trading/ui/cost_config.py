...
def render_cost_config_sidebar(
        ...
        if sample_trade_value > 0:
            _show_cost_breakdown_preview(
                ...
                (
                    min_commission
                    if config_ui.show_advanced
                    else cost_params.min_commission
                ),
                (
                    max_commission
                    if config_ui.show_advanced
                    else cost_params.max_commission
                ),
            )
    ...
    return CostParameters(
        ...
        cash_drag_rate=(
            cash_drag_rate if config_ui.show_advanced else cost_params.cash_drag_rate
        ),
        min_commission=(
            min_commission if config_ui.show_advanced else cost_params.min_commission
        ),
...
def render_cost_summary(metrics: Dict[str, Any]) -> None:
    ...
    with col1:
        if "total_trading_costs" in metrics:
            st.metric("Total Trading Costs", f"${metrics['total_trading_costs']:,.2f}")

        if "cost_per_trade" in metrics:
            st.metric("Cost per Trade", f"${metrics['cost_per_trade']:.2f}")
    ...
    with col2:
        ...
        if "cost_percentage" in metrics:
            st.metric("Cost % of Volume", f"{metrics['cost_percentage']:.3f}%")
    ...
    with col3:
        ...
        if "turnover_ratio" in metrics and not pd.isna(metrics["turnover_ratio"]):
            st.metric("Turnover Ratio", f"{metrics['turnover_ratio']:.2f}")

    # Cost breakdown chart
    if all(
        metric in metrics
        for metric in ["total_commission", "total_slippage", "total_spread"]
    ):
    ...
    if any(
        metric in metrics
        for metric in ["avg_cash_utilization", "cash_drag_cost", "cash_drag_percentage"]
    ):
        ...
        with col1:
            if "avg_cash_utilization" in metrics and not pd.isna(
                metrics["avg_cash_utilization"]
            ):
                st.metric(
        ...
        with col2:
            if "cash_drag_cost" in metrics:
                st.metric("Cash Drag Cost", f"${metrics['cash_drag_cost']:,.2f}")

        with col3:
            if "cash_drag_percentage" in metrics:
                st.metric("Cash Drag %", f"{metrics['cash_drag_percentage']:.2f}%")
