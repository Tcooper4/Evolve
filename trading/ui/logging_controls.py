...
def render_logging_controls() -> None:
    ...
    new_metrics = st.sidebar.checkbox("Enable Metric Logging", value=current_metrics)
    ...
    new_path = st.sidebar.text_input("Metrics File Path", value=current_path)
...
def render_log_viewer() -> None:
    ...
    if Path(log_file).exists():
        ...
        if logs:
            ...
            filtered_logs = logs
            if log_level != "ALL":
                filtered_logs = [log for log in logs if f"[{log_level}]" in log]
