...
def to_device(data: Union[torch.Tensor, Dict[str, torch.Tensor]], device: torch.device) -> Union[torch.Tensor, Dict[str, torch.Tensor]]:
...
def get_model_metadata(model_type: str, config: dict, device: torch.device, 
                      best_val_loss: float, train_losses: list) -> Dict[str, Any]:
    """Get model metadata.
