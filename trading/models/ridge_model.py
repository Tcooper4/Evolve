...
from .base_model import BaseModel, ModelRegistry, ValidationError, ModelError
...
class RidgeModel(BaseModel):
    ...
    def train(self, data: pd.DataFrame, target_col: str = 'target', 
              feature_cols: Optional[List[str]] = None, **kwargs) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if features.shape[1] != expected_features:
                ...
                if self.config.get('use_lags', True):
                    ...
                    features_orig = data.select_dtypes(include=[np.number]).drop(columns=['target'], errors='ignore').values
                    ...
                    if features_lagged.shape[1] == expected_features:
                        ...
                        raise ModelError(f"Feature mismatch: model expects {expected_features} features, but got {features.shape[1]}. Please provide input data with the same lag structure as used in training.")
                else:
                    raise ModelError(f"Feature mismatch: model expects {expected_features} features, but got {features.shape[1]}. Please provide input data with the same feature structure as used in training.")
            predictions = self.model.predict(features)
...
def create_ridge_model(alpha: float = 1.0, max_iter: int = 1000, 
                      solver: str = 'auto', polynomial_degree: int = 1,
                      use_lags: bool = True, lag_periods: int = 5) -> RidgeModel:
    """
