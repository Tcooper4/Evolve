...
class ForecastNormalizer:
    ...
    def _ensure_dataframe(self, data: Union[pd.DataFrame, pd.Series, np.ndarray]) -> pd.DataFrame:
    ...
    def _validate_forecast(self, forecast_df: pd.DataFrame) -> None:
        ...
        if len(forecast_df) < self.min_forecast_length:
            raise ValueError(f"Forecast too short: {len(forecast_df)} < {self.min_forecast_length}")
        
        if len(forecast_df) > self.max_forecast_length:
            raise ValueError(f"Forecast too long: {len(forecast_df)} > {self.max_forecast_length}")
        
        # Check for excessive NaN values
        nan_ratio = forecast_df.isna().sum().sum() / (forecast_df.shape[0] * forecast_df.shape[1])
        if nan_ratio > 0.5:
    ...
    def _align_to_datetime(
        ...
        **kwargs
        ...
        if reference is None:
            ...
            raise ValueError(f"Invalid reference type for datetime alignment: {type(reference)}")
        ...
        aligned_df = forecast_df.reindex(reference_index, method=self.interpolation_method)
    ...
    def _align_to_reference(
        ...
        **kwargs
        ...
        if not isinstance(forecast_df.index, pd.DatetimeIndex):
            # Create datetime index for forecast
            forecast_start = reference_index[0] if len(reference_index) > 0 else datetime.now()
            forecast_index = pd.date_range(
                start=forecast_start,
                periods=len(forecast_df),
                freq=self.default_freq
