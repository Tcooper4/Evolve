...
class PerformanceSummarizer:
    ...
    def __init__(self, risk_free_rate: float = 0.01, trading_days_per_year: int = 252):
        ...
        logger.info(
            f"PerformanceSummarizer initialized with risk_free_rate={risk_free_rate:.4f}"
        )
    ...
    def calculate_performance_metrics(
        ...
        try:
            ...
            alpha = annualized_return - (
                rf_rate + beta * (0.08 - rf_rate)
            )  # Assuming 8% market return

            # Information ratio
            information_ratio = (
                excess_returns.mean() / returns.std() if returns.std() > 0 else 0
            )
            ...
            logger.info(
                f"Performance metrics calculated successfully for {len(returns)} data points"
            )
            return metrics
    ...
    def compare_strategies(
        ...
        try:
            ...
            logger.info(
                f"Strategy comparison completed for {len(strategy_returns)} strategies"
            )
    ...
    def generate_performance_report(
        ...
        try:
            ...
            report = {
                ...
                "metrics": metrics,
                "rolling_metrics": (
                    rolling_metrics.to_dict() if not rolling_metrics.empty else {}
                ),
    ...
    def _generate_recommendations(self, metrics: Dict[str, float]) -> List[str]:
        ...
        if profit_factor < 1.5:
            recommendations.append(
                "Focus on improving profit factor through better position sizing"
            )
