...
class PortfolioManager:
                ...
                for position in positions:
                    ...
                    if symbol in prices:
                        ...
                        self.redis_client.hset(
                            "positions", f"{symbol}_{position['entry_time']}", json.dumps(position)
                        )
            else:
    ...
    def calculate_portfolio_pnl(self) -> float:
        ...
        try:
            ...
            for position in positions:
                if "current_price" in position:
                    total_pnl += (position["current_price"] - position["entry_price"]) * position[
                        "quantity"
                    ]
    ...
    def update_positions(self, prices: Dict[str, float]) -> None:
        ...
        for position in self.get_positions():
            ...
            if position["stop_loss"] is not None:
                if (
                    position["type"] == PositionType.LONG and current_price <= position["stop_loss"]
                ) or (
    ...
    def rebalance_portfolio(
        ...
        for symbol in list(self.get_positions()):
            if symbol not in target_weights:
                rebalance_trades.append(
                    (symbol, -self.get_position(symbol)["quantity"], prices[symbol])
                )
        ...
        for symbol, target_size in target_positions.items():
            current_size = (
                self.get_position(symbol)["quantity"] if symbol in self.get_positions() else 0
            )
