...
class ModelSwapper:
    ...
    def __init__(
        ...
        logger.info(
            f"ModelSwapper initialized with min Sharpe: {min_sharpe_ratio}, max MSE increase: {max_mse_increase}"
        )
    ...
    def add_candidate_model(
        ...
        try:
            ...
            if validation_result != ValidationResult.PASSED:
                ...
                logger.warning(
                    f"Candidate model {model_id} failed validation: {validation_result.value}"
                )
                return False
            ...
            logger.info(
                f"Added candidate model {model_id} with Sharpe: {model_metrics.sharpe_ratio:.3f}"
            )
            return True
    ...
    def _validate_candidate_model(self, metrics: ModelMetrics) -> ValidationResult:
        ...
        if metrics.sharpe_ratio < self.min_sharpe_ratio:
            logger.debug(
                f"Sharpe ratio too low: {metrics.sharpe_ratio:.3f} < {self.min_sharpe_ratio}"
            )
        ...
        if self.active_models:
            ...
            if current_mse is not None and current_mse > 1e-10:
                ...
                if mse_increase > self.max_mse_increase:
                    logger.debug(
                        f"MSE increase too high: {mse_increase:.3f} > {self.max_mse_increase}"
                    )
    ...
    def _passes_safety_checks(self, metrics: ModelMetrics) -> bool:
        ...
        if (
            np.isnan(metrics.sharpe_ratio)
            or np.isinf(metrics.sharpe_ratio)
            or np.isnan(metrics.mse)
            or np.isinf(metrics.mse)
        ):
    ...
    def find_best_candidate(
        self, target_model_type: Optional[str] = None, min_confidence: float = 0.5
        ...
        def candidate_score(candidate: SwapCandidate) -> float:
            ...
            normalized_sharpe = min(
                candidate.metrics.sharpe_ratio / 2.0, 1.0
            )  # Normalize to 0-1
            return (
        ...
        logger.info(
            f"Selected best candidate: {best_candidate.model_id} (score: {candidate_score(best_candidate):.3f})"
        )

        return best_candidate

    def swap_model(
        self, candidate_id: str, target_model_id: Optional[str] = None
    ) -> SwapResult:
        """
        ...
        try:
            ...
            logger.info(
                f"Successfully swapped model {target_model_id} -> {candidate_id} "
    ...
    def _select_target_model(self, candidate: SwapCandidate) -> Optional[str]:
        ...
        same_type_models = [
            model_id
            for model_id, metrics in self.active_models.items()
        ...
        if same_type_models:
            ...
            worst_model = min(
                same_type_models, key=lambda mid: self.active_models[mid].sharpe_ratio
            )
            return worst_model
    ...
    def _calculate_performance_improvement(
        self, old_metrics: ModelMetrics, new_metrics: ModelMetrics
    ) -> float:
        """
    ...
    def get_swap_statistics(self) -> Dict[str, Any]:
        ...
        stats["avg_active_sharpe"] = (
            np.mean([m.sharpe_ratio for m in self.active_models.values()])
            if self.active_models
            else 0.0
        )
        ...
        for result in reversed(self.swap_history):
            if result.success and result.old_model_id and result.new_model_id:
                ...
                if result.new_model_id in self.active_models:
                    ...
                    logger.warning(
                        f"Rolling back swap: {result.old_model_id} <- {result.new_model_id}"
                    )
                    return True
    ...
    def set_validation_criteria(
        self, min_sharpe_ratio: float = None, max_mse_increase: float = None
    ):
