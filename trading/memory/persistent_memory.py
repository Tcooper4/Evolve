    ...
    async def search_similar_interactions(
        ...
        try:
            ...
            similarities, indices = self.index.search(
                query_embedding.astype("float32"), min(limit * 2, self.index.ntotal)
            )
...
class SQLiteMemoryStore:
    ...
    def _init_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_interaction_type ON interactions(interaction_type)"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_outcome ON interactions(outcome)"
            )
            conn.execute(
            ...
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_model_name ON interactions(model_name)"
            )
...
class PersistentMemory:
    ...
    def __init__(
        ...
        if memory_type in [MemoryType.REDIS, MemoryType.HYBRID]:
            if redis_url and REDIS_AVAILABLE:
                ...
                raise ImportError(
                    "Redis not available but required for REDIS memory type"
                )
        ...
        if memory_type in [MemoryType.VECTOR_STORE, MemoryType.HYBRID]:
            if VECTOR_STORE_AVAILABLE:
                ...
                raise ImportError(
                    "Vector store not available but required for VECTOR_STORE memory type"
                )
    ...
    async def search_similar_interactions(
        ...
        if "vector" in self.stores:
            return await self.stores["vector"].search_similar_interactions(
...
def get_persistent_memory(
    memory_type: MemoryType = MemoryType.HYBRID, redis_url: Optional[str] = None
) -> PersistentMemory:
