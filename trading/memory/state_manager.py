            ...
            with self._file_lock:
                ...
                if isinstance(data, dict) and "version" in data:
                    ...
                    if self._version.version not in StateVersion.COMPATIBLE_VERSIONS:
                        logger.warning(
                            f"State version {self._version.version} may not be compatible"
                        )

                    # Load state data
                    self._state = data.get("state", {})
                    self._last_save = datetime.fromisoformat(
                        data.get("last_save", datetime.now().isoformat())
                    )

                    logger.info(
                        f"State loaded successfully: version {self._version.version}"
                    )
                else:
                    ...
                    if value["timestamp"] < cutoff_time:
                        ...
                        if "data" in value and not value.get("compressed", False):
                            try:
                                ...
                                logger.warning(
                                    f"Failed to compress data for {key}: {e}"
                                )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            return {
                ...
                "version": self._version.to_dict(),
                "file_size_mb": (
                    self.state_file.stat().st_size / (1024 * 1024)
                    if self.state_file.exists()
                    else 0
                ),
            }
        ...
        with self._lock:
            ...
            for key, value in list(self._state.items()):
                if (
                    ...
                    and "data" in value
                    and not value.get("compressed", False)
                ):
                    try:
