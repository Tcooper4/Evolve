...
class ModelMonitor:
    ...
    def _load_monitoring_data(self):
        ...
        try:
            ...
            if data_file.exists():
                ...
                logger.info(f"Loaded monitoring data: {len(self.parameter_history)} models, {len(self.drift_alerts)} drift alerts")
    ...
    def record_model_parameters(self, model_name: str, parameters: Dict[str, Any], 
        ...
        try:
            with self._lock:
                ...
                if param_record.drift_score > self.drift_threshold:
                    self._create_drift_alert(model_name, 'parameter', param_record.drift_score, 
    ...
    def _create_drift_alert(self, model_name: str, drift_type: str, drift_score: float, 
                           affected_parameters: List[str]):
    ...
    def _check_performance_drops(self, model_name: str, current_metrics: Dict[str, float], 
                                threshold_percent: float = 20.0, window_size: int = 10):
    ...
    def get_performance_alerts(self, model_name: Optional[str] = None, 
    ...
    def get_drift_alerts(self, model_name: Optional[str] = None, 
    ...
    def get_rolling_performance_averages(self, model_name: str, 
...
def _calculate_chi_square_drift(current_data: pd.DataFrame, historical_data: pd.DataFrame) -> float:
    ...
    try:
        ...
        bins = np.linspace(min(current_data.min(), historical_data.min()),
                          max(current_data.max(), historical_data.max()), 10)
