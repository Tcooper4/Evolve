...
class MemoryChunk:
    ...
    def __init__(self, data: Any, ttl_seconds: Optional[int] = None, 
                 max_size_bytes: Optional[int] = None, priority: int = 1):
        self.data = data
...
class MemoryManager:
    ...
    def add(self, key: str, data: Any, ttl_seconds: Optional[int] = None,
            max_size_bytes: Optional[int] = None, priority: int = 1) -> bool:
        """Add a memory chunk."""
        with self.lock:
    ...
    def _can_fit_chunk(self, chunk_size: int) -> bool:
        ...
        return (self.total_memory_usage + chunk_size <= self.max_memory_bytes and
...
class AgentMemory:
    ...
    def __init__(self, path: str = "agent_memory.json", 
                 max_memory_mb: int = 100, max_entries: int = 10000,
                 cleanup_interval_seconds: int = 300):
        self.path = Path(path)
    ...
    def log_outcome(self, agent: str, run_type: str, outcome: Dict[str, Any], 
                   memory_type: str = 'medium_term') -> Dict[str, Any]:
        """
    ...
    def get_history(self, agent: str, run_type: Optional[str] = None, 
                   model_id: Optional[str] = None, memory_type: Optional[str] = None) -> Dict[str, Any]:
        """
    ...
    def get_recent_performance(self, agent: str, run_type: str, metric: str, 
                             window: int = 10, memory_type: Optional[str] = None) -> dict:
        """
    ...
    def is_improving(self, agent: str, run_type: str, metric: str, 
                    window: int = 10, memory_type: Optional[str] = None) -> dict:
        """
        ...
        try:
            ...
            higher_is_better = any(keyword in metric.lower() for keyword in 
                                 ['accuracy', 'precision', 'recall', 'f1', 'sharpe', 'return', 'profit'])
                    ...
                    for agent in data:
                        for run_type in data[agent]:
                            if isinstance(data[agent][run_type], list):
                                ...
                                data[agent][run_type] = [
                                    entry for entry in data[agent][run_type] 
