...
class EnsembleIntegrityChecker:
    ...
    def __init__(
        ...
        logger.info(
            f"EnsembleIntegrityChecker initialized with reference window: {reference_window}"
        )
        ...
        try:
            ...
            if features and "features" in reference_dist:
                for feature_name, feature_values in features.items():
                    ref_feature = reference_dist["features"].get(
            ...
            confidence = self._calculate_drift_confidence(
                max_distance, len(current_predictions)
            )
            ...
            logger.info(
                f"Drift check for {model_id}: {drift_type.value} (distance: {max_distance:.4f})"
            )
            return result
    ...
    def _update_reference_distributions(self):
        ...
        logger.debug(
            f"Updated reference distributions for {len(self.reference_distributions)} models"
        )
    ...
    def check_ensemble_health(self) -> EnsembleHealth:
        ...
        if len(drift_alerts) > 3:
            recommendations.append(
                "High drift frequency detected - review model stability"
            )
    ...
    def _create_no_drift_result(
        self, model_id: str, distance_metric: DistanceMetric
    ) -> DriftResult:
    ...
    def _create_error_result(
        self, model_id: str, distance_metric: DistanceMetric, error: str
    ) -> DriftResult:
    ...
    def _update_drift_stats(self, result: DriftResult):
        ...
        if self.stats["total_checks"] > 0:
            total_score = (
                self.stats["avg_drift_score"] * (self.stats["total_checks"] - 1)
    ...
    def get_drift_statistics(self) -> Dict[str, Any]:
        ...
        drift_types = [
            d.drift_type.value for d in self.drift_history[-100:]
        ]  # Last 100 drifts
        stats["drift_type_distribution"] = {
            drift_type: drift_types.count(drift_type) for drift_type in set(drift_types)
        }
...
def create_ensemble_integrity_checker(
    reference_window: int = 1000,
) -> EnsembleIntegrityChecker:
