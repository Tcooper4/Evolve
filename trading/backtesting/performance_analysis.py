...
class PerformanceAnalyzer:
    ...
    def _calculate_cost_adjusted_metrics(
        ...
        if "equity_curve" in df.columns:
            ...
            cost_adjusted_return = (
                (cost_adjusted_equity.iloc[-1] / cost_adjusted_equity.iloc[0]) - 1
            )
            ...
            metrics.update({
                ...
                "cost_per_trade": total_costs / len(trade_log) if len(trade_log) > 0 else 0.0,
                "cost_impact": (metrics.get("total_return", 0) - cost_adjusted_return) * 100
            })
    ...
    def _calculate_trade_metrics(self, trade_log: pd.DataFrame) -> Dict[str, float]:
        ...
        if "pnl" in trade_log.columns:
            ...
            if len(pnl_series) > 0:
                ...
                metrics["avg_win"] = pnl_series[pnl_series > 0].mean() if (pnl_series > 0).any() else np.nan
                metrics["avg_loss"] = pnl_series[pnl_series < 0].mean() if (pnl_series < 0).any() else np.nan
                metrics["largest_win"] = pnl_series.max() if len(pnl_series) > 0 else np.nan
                metrics["largest_loss"] = pnl_series.min() if len(pnl_series) > 0 else np.nan
            ...
            try:
                ...
                for _, trade in trade_log.iterrows():
                    if pd.notna(trade["timestamp"]) and pd.notna(trade["exit_time"]):
                        duration = (trade["exit_time"] - trade["timestamp"]).total_seconds() / 3600  # hours
                        durations.append(duration)
                
                metrics["avg_trade_duration"] = np.mean(durations) if durations else np.nan
            except Exception as e:
    ...
    def _calculate_cost_breakdown(self, trade_log: pd.DataFrame) -> Dict[str, float]:
        ...
        return {
            ...
            "cost_percentage": (total_costs / total_trade_value * 100) if total_trade_value > 0 else 0.0,
            "commission_percentage": (total_commission / total_trade_value * 100) if total_trade_value > 0 else 0.0,
            "slippage_percentage": (total_slippage / total_trade_value * 100) if total_trade_value > 0 else 0.0,
            "spread_percentage": (total_spread / total_trade_value * 100) if total_trade_value > 0 else 0.0
        }
    ...
    def _calculate_cash_efficiency_metrics(
        ...
        if "equity_curve" in df.columns and "cash" in df.columns:
            # Cash utilization
            avg_cash_utilization = (df["equity_curve"] - df["cash"]) / df["equity_curve"]
            ...
            metrics["cash_drag_percentage"] = (cash_drag_cost / initial_cash * 100) if initial_cash > 0 else 0.0
            ...
            if not trade_log.empty and "quantity" in trade_log.columns:
                ...
                metrics["turnover_ratio"] = total_volume / avg_portfolio_value if avg_portfolio_value > 0 else 0.0
            else:
