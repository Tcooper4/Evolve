...
class BacktestCompiler:
    ...
    def __init__(self, deduplication_enabled: bool = True):
        ...
        logger.info(f"BacktestCompiler initialized with deduplication={deduplication_enabled}")
    ...
    def compile_signals(
        ...
        try:
            ...
            if self.deduplication_enabled:
                ...
                logger.info(f"Signal deduplication applied: {stats['duplicates_removed']} duplicates removed")
            
            # Store compilation record
            self._store_compilation_record(strategy_signals, compiled_df, timeframe)
            
            logger.info(f"Signals compiled successfully: {len(compiled_df)} signals from {len(strategy_signals)} strategies")
            return compiled_df
    ...
    def _prepare_signals(
        ...
        for strategy_name, signals_df in strategy_signals.items():
            try:
                ...
                if missing_columns:
                    logger.warning(f"Strategy {strategy_name} missing columns: {missing_columns}")
                    continue
                ...
                logger.error(f"Error preparing signals for strategy {strategy_name}: {e}")
                continue
    ...
    def _compile_raw_signals(
        self, 
        prepared_signals: Dict[str, pd.DataFrame]
    ) -> pd.DataFrame:
    ...
    def _deduplicate_signals(
        ...
        try:
            ...
            if method == "priority":
                ...
                logger.warning(f"Unknown deduplication method: {method}, using priority")
                deduplicated_df = self._deduplicate_by_priority(compiled_df)
            ...
            stats = {
                ...
                "deduplication_rate": duplicates_removed / initial_count if initial_count > 0 else 0
            }
    ...
    def _deduplicate_by_priority(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            # Add priority column
            df['priority'] = df['strategy'].map(strategy_priorities).fillna(strategy_priorities['default'])
            
            # Group by timestamp and symbol, keep highest priority signal
            deduplicated = df.loc[df.groupby(['timestamp', 'symbol'])['priority'].idxmax()]
    ...
    def _deduplicate_by_confidence(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            deduplicated = df.loc[df.groupby(['timestamp', 'symbol'])['confidence'].idxmax()]
    ...
    def _deduplicate_by_timestamp(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            deduplicated = df.groupby(['timestamp', 'symbol']).tail(1).reset_index(drop=True)
                ...
                if len(group) > 1:
                    ...
                    for i, strategy1 in enumerate(strategies):
                        for j, strategy2 in enumerate(strategies[i+1:], i+1):
                            ...
                            analysis["strategy_overlaps"][pair] = analysis["strategy_overlaps"].get(pair, 0) + 1
            
            # Calculate overlap percentage
            analysis["overlap_percentage"] = (
                analysis["overlapping_periods"] / len(grouped) * 100 
                if len(grouped) > 0 else 0
            )
    ...
    def get_compilation_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_deduplication_rate": total_duplicates_removed / total_compilations if total_compilations > 0 else 0,
            "recent_compilations": self.compilation_history[-5:] if len(self.compilation_history) > 5 else self.compilation_history
        }
