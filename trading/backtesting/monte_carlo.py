...
class MonteCarloSimulator:
    ...
    def simulate_portfolio_paths(
        ...
        if self.config.bootstrap_method == "historical":
            paths = self._bootstrap_historical_returns(returns, n_simulations, n_periods)
            ...
            paths = self._parametric_bootstrap_returns(returns, n_simulations, n_periods)
        else:
    ...
    def calculate_percentiles(self, confidence_levels: Optional[List[float]] = None) -> pd.DataFrame:
        """
        ...
        if self.simulated_paths is None:
            raise ValueError("Must run simulation first. Call simulate_portfolio_paths()")
        ...
        for level in confidence_levels:
            percentile_values = np.percentile(self.simulated_paths.values, level * 100, axis=1)
        ...
        percentile_df = pd.DataFrame(
            percentiles,
            index=self.simulated_paths.index
        )
    ...
    def get_summary_statistics(self) -> Dict[str, float]:
        ...
        stats = {
            ...
            'cvar_99': total_returns[total_returns <= np.percentile(total_returns, 1)].mean(),
        ...
        if self.percentiles is not None:
            for col in self.percentiles.columns:
                if col.startswith('P'):
                    ...
                    stats[f'return_{col.lower()}'] = (final_percentile - initial_capital) / initial_capital
    ...
    def plot_simulation_results(
        ...
        ax1.set_title('Monte Carlo Simulation: Portfolio Value Paths', fontsize=14, fontweight='bold')
        ax1.set_ylabel('Portfolio Value ($)', fontsize=12)
        ax1.legend()
        ax1.grid(True, alpha=0.3)
        
        # Plot 2: Return Distribution at Final Period
        final_returns = (self.simulated_paths.iloc[-1] - self.results['initial_capital']) / self.results['initial_capital']
        
        ax2.hist(final_returns, bins=50, alpha=0.7, color='skyblue', edgecolor='black')
        ax2.axvline(final_returns.mean(), color='red', linestyle='--', label=f'Mean: {final_returns.mean():.2%}')
        ax2.axvline(np.percentile(final_returns, 5), color='orange', linestyle='--', label=f'5th percentile: {np.percentile(final_returns, 5):.2%}')
        ax2.axvline(np.percentile(final_returns, 95), color='orange', linestyle='--', label=f'95th percentile: {np.percentile(final_returns, 95):.2%}')
...
if __name__ == "__main__":
    ...
    print(f"5th Percentile Final Value: ${results['summary_statistics']['final_p5']:,.2f}")
