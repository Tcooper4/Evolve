...
class BacktestUtils:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.required_columns = self.config.get("required_columns", ["Buy", "Sell", "Close"])
    ...
    def generate_backtest_report(self, df: pd.DataFrame) -> Optional[BacktestReport]:
        ...
        if not is_valid:
            ...
            return self._generate_empty_report(f"Validation failed: {', '.join(errors)}")
    ...
    def _create_trade_history(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for i in range(min(len(buy_indices), len(sell_indices))):
            trade = {
                ...
                "return": (df.loc[sell_indices[i], "Close"] - df.loc[buy_indices[i], "Close"]) / df.loc[buy_indices[i], "Close"]
            }
            trades.append(trade)
