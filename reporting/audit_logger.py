...
# Local imports
from utils.common_helpers import load_config, safe_json_saver
...
class AuditLogger:
    ...
    def __init__(self, config_path: str = "config/app_config.yaml"):
        ...
        self.enabled_event_types = set(
            self.audit_config.get(
                "enabled_event_types", [e.value for e in AuditEventType]
            )
        )
        self.min_confidence_threshold = self.audit_config.get(
    ...
    def _initialize_output_files(self):
        ...
        if "json" in self.output_formats:
            ...
            with open(json_file, "w") as f:
                json.dump(
                    {"session_metadata": self.session_metadata, "events": []},
                    f,
                    indent=2,
                )
    ...
    def _add_event(self, event: AuditEvent) -> str:
        ...
        if (
            event.confidence_score is not None
            and event.confidence_score < self.min_confidence_threshold
        ):
            return event.event_id
    ...
    def flush_events(self):
        ...
        try:
            ...
            self.logger.debug(
                f"Flushed {len(self.events[-self.batch_size:])} events to output files"
            )
    ...
    def get_events(
        ...
        if decision_levels:
            filtered_events = [
                e for e in filtered_events if e.decision_level in decision_levels
            ]

        # Filter by tags
        if tags:
            filtered_events = [
                e for e in filtered_events if any(tag in e.tags for tag in tags)
            ]
    ...
    def get_event_hierarchy(self, event_id: str) -> Dict[str, Any]:
        ...
        hierarchy = {"event": asdict(event), "children": [], "ancestors": []}
    ...
    def export_session_report(self, output_path: Optional[str] = None) -> str:
        ...
        if output_path is None:
            ...
            output_path = (
                self.audit_dir / f"session_report_{timestamp}_{self.session_id}.json"
            )
    ...
    def close_session(self):
        ...
        self.logger.info(
            f"Audit session {self.session_id} closed with {len(self.events)} events"
        )
