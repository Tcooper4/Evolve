...
""",
    ...
    def _initialize_llm(self):
        ...
        try:
            ...
            if api_key:
                ...
                self.logger.warning(
                    "OpenAI API key not found, LLM explanations disabled"
                )
    ...
    def explain_trade_decision(
        ...
        if risk_reward_ratio is None:
            risk_reward_ratio = (
                expected_return / expected_risk if expected_risk > 0 else 0
            )
    ...
    def _generate_model_explanation_details(
        self, explanation: ModelExplanation
    ) -> Dict[str, Any]:
    ...
    def _generate_feature_explanation_details(
        self, explanation: FeatureExplanation
    ) -> Dict[str, Any]:
        ...
        details = {
            ...
            "contributions": {
                ...
                "correlation_insights": self._analyze_correlations(
                    explanation.feature_correlations
                ),
            },
            "trends": {
                "feature_trends": explanation.feature_trends,
                "trend_analysis": self._analyze_feature_trends(
                    explanation.feature_trends
                ),
            },
    ...
    def _generate_forecast_explanation_details(
        self, explanation: ForecastExplanation
    ) -> Dict[str, Any]:
        ...
        details = {
            ...
            "factor_analysis": {
                "key_factors": self._categorize_factors(
                    explanation.details.get("key_factors", [])
                ),
    ...
    def _generate_trade_explanation_details(
        self, explanation: TradeExplanation
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            response = self.llm_client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are an expert trading analyst explaining complex trading decisions in clear, professional language.",
                    },
...
"""
    ...
    def _add_explanation(self, explanation: Explanation) -> str:
        ...
        # Log explanation
        self.logger.info(
            f"Generated explanation: {explanation.explanation_id} - {explanation.title}"
        )
    ...
    def get_explanations(
        ...
        if explanation_types:
            filtered_explanations = [
                ...
                if e.explanation_type in explanation_types
            ]

        # Filter by ticker
        if ticker:
            filtered_explanations = [
                e for e in filtered_explanations if e.ticker == ticker
            ]
        ...
        if tags:
            filtered_explanations = [
                e for e in filtered_explanations if any(tag in e.tags for tag in tags)
            ]
    ...
    def generate_summary_report(self, ticker: Optional[str] = None) -> Dict[str, Any]:
        ...
        explanations = (
            self.get_explanations(ticker=ticker) if ticker else self.explanations
        )
        ...
        report = {
            "summary": {
                ...
                "tickers_covered": list(
                    set(e.ticker for e in explanations if e.ticker)
                ),
                "explanation_types": defaultdict(int),
                "time_range": {
                    "start": (
                        min(e.timestamp for e in explanations) if explanations else None
                    ),
                    "end": (
                        max(e.timestamp for e in explanations) if explanations else None
                    ),
                },
            },
            "metrics": {
            ...
            "recent_explanations": [
                ...
                for e in sorted(explanations, key=lambda x: x.timestamp, reverse=True)[
                    :10
                ]
            ],
        }
        ...
        for explanation in explanations:
            report["summary"]["explanation_types"][
    ...
    def export_explanations(
        ...
        if output_path is None:
            ...
            output_path = (
                ...
                / f"explanations_{timestamp}{ticker_suffix}.{format}"
            )
    ...
    def _categorize_factors(self, factors: List[str]) -> Dict[str, List[str]]:
        ...
        for factor in factors:
            ...
            if any(
                tech in factor_lower
                for tech in ["rsi", "macd", "bollinger", "moving_average"]
            ):
                categories["technical"].append(factor)
            elif any(
                fund in factor_lower
                for fund in ["earnings", "revenue", "pe_ratio", "book_value"]
            ):
                ...
                macro in factor_lower
                for macro in ["gdp", "inflation", "interest_rate", "fed"]
            ):
    ...
    def _assess_forecast_risk(self, explanation: ForecastExplanation) -> Dict[str, Any]:
        ...
        return {
            ...
            "scenario_variance": (
                np.var(list(explanation.scenario_analysis.values()))
