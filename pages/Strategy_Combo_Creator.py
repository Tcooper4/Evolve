...
def main():
    ...
    try:
        ...
        combined_signal, metadata = pipeline.generate_combined_signals(data, selected_strategies)
...
def load_uploaded_data() -> Optional[pd.DataFrame]:
    ...
    uploaded_file = st.file_uploader(
        ...
        type=['csv'],
        help="Upload CSV file with OHLCV data"
    )
        ...
        if len(individual_signals) > 1:
            ...
            if agreement_data:
                agreement_df = pd.DataFrame(
                    agreement_data,
                    columns=['Strategy 1', 'Strategy 2', 'Agreement']
                )
