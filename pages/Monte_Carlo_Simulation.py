...
def generate_sample_returns(
    ...
    returns = pd.Series(
        np.random.normal(mean_return, volatility, n_days),
        index=dates
    )
    return returns
...
def create_monte_carlo_plot(
    ...
    final_returns = (simulator.simulated_paths.iloc[-1] - simulator.results['initial_capital']) / simulator.results['initial_capital']
    ...
    fig.add_vline(
        x=mean_return, line_dash="dash", line_color="red",
        annotation_text=f"Mean: {mean_return:.2%}",
        row=2, col=1
    )
    
    fig.add_vline(
        x=p5_return, line_dash="dash", line_color="orange",
        annotation_text=f"5th percentile: {p5_return:.2%}",
        row=2, col=1
    )
    
    fig.add_vline(
        x=p95_return, line_dash="dash", line_color="orange",
...
def display_summary_statistics(stats: dict):
    ...
    with col4:
        st.metric(
            "Volatility",
            f"{stats['std_total_return']:.1%}"
        )
        
        st.metric(
            "Max Drawdown (Median)",
            f"{stats.get('max_drawdown_p50', 0):.1%}"
        )
...
def main():
        ...
        if uploaded_file is not None:
            try:
                ...
                if 'returns' in df.columns:
                    ...
                    st.error("File must have a 'returns' column or be a single column of returns")
                    return
    ...
    if hasattr(st.session_state, 'simulator') and st.session_state.simulator is not None:
        ...
        with tab4:
            ...
            with col1:
                plot_type = st.selectbox(
                    "Plot Type",
                    ["Portfolio Paths", "Return Distribution", "Drawdown Analysis", "Volatility Over Time"]
                )
            ...
            if plot_type == "Portfolio Paths":
                ...
                if show_stats:
                    fig.add_vline(x=final_returns.mean(), line_dash="dash", line_color="red")
                    fig.add_vline(x=np.percentile(final_returns, 5), line_dash="dash", line_color="orange")
                    fig.add_vline(x=np.percentile(final_returns, 95), line_dash="dash", line_color="orange")
