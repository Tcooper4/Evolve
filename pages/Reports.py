...
def generate_performance_report(symbol: str, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
    ...
    try:
        ...
        profit_factor = np.sum(portfolio_returns[portfolio_returns > 0]) / abs(np.sum(portfolio_returns[portfolio_returns < 0])) if np.sum(portfolio_returns[portfolio_returns < 0]) != 0 else float('inf')
...
def generate_strategy_report(strategy_name: str, symbol: str, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
...
def plot_performance_report(report_data: Dict[str, Any]):
    ...
    try:
        ...
        fig = make_subplots(
            rows=3, cols=2,
            subplot_titles=('Portfolio vs Benchmark', 'Returns Distribution', 'Drawdown Analysis', 'Rolling Sharpe', 'Monthly Returns', 'Risk Metrics'),
        ...
        rolling_sharpe = returns_series.rolling(window=30).mean() / returns_series.rolling(window=30).std() * np.sqrt(252)
        ...
        returns_df = pd.DataFrame({
            'date': report_data['dates'],
            'returns': report_data['portfolio_returns']
        })
...
def main():
    ...
    with st.sidebar:
        ...
        report_type = st.selectbox(
            "Report Type",
            ["Performance Report", "Risk Analysis", "Strategy Report", "Model Evaluation"]
        )
        ...
        if report_type == "Strategy Report":
            strategy_name = st.selectbox(
                "Strategy",
                ["RSI Mean Reversion", "MACD Crossover", "Bollinger Bands", "Moving Average Crossover"]
            )
    ...
    with col2:
        ...
        if st.session_state.report_history:
            ...
            for i, report in enumerate(reversed(st.session_state.report_history[-5:])):
                with st.expander(f"{report.get('symbol', 'Portfolio')} - {report['generated_at'].strftime('%Y-%m-%d %H:%M')}"):
