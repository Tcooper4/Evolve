...
try:
    ...
    from trading.risk.risk_manager import RiskManager, VolatilityModel
...
try:
    from utils.math_helpers import calculate_volatility_regime
...
class RiskPreviewDashboard:
    ...
    def render_dashboard(self):
        ...
        st.set_page_config(
            page_title="Risk Preview Dashboard", page_icon="ðŸ“Š", layout="wide"
        )
    ...
    def _render_sidebar(self):
        ...
        model_type = st.sidebar.selectbox(
            "Volatility Model", ["rolling_std", "ewma", "garch", "hybrid"], index=3
        )
        ...
        confidence_threshold = st.sidebar.slider(
            "Confidence Threshold", 0.5, 0.9, 0.7, 0.05
        )
        ...
        if self.risk_manager:
            try:
                self.risk_manager.update_volatility_model(
                    VolatilityModel(model_type), window=window, alpha=alpha
                )
    ...
    def _render_risk_metrics(self):
        ...
        with col4:
            st.metric("Beta", f"{metrics.beta:.3f}", delta=f"{metrics.beta - 1.0:.3f}")
            st.metric(
                "Kelly Fraction",
    ...
    def _render_volatility_analysis(self):
        ...
        if volatility_forecast:
            ...
            with col2:
                st.metric("Model Type", volatility_forecast.model_type.upper())
                st.metric(
                    "Confidence Interval",
        ...
        if self.risk_manager.returns is not None:
            try:
                volatility_regime = calculate_volatility_regime(
                    self.risk_manager.returns, window=60
                )
    ...
    def _render_risk_charts(self):
        ...
        fig.add_trace(
            go.Scatter(
                x=cum_returns.index, y=cum_returns.values, name="Cumulative Returns"
            ),
            row=1,
            col=1,
        )
        ...
        fig.add_trace(
            go.Scatter(
                x=rolling_vol.index, y=rolling_vol.values, name="30-day Volatility"
            ),
            row=1,
            col=2,
        )
        ...
        fig.add_trace(
            go.Scatter(
                x=drawdown.index, y=drawdown.values, name="Drawdown", fill="tonexty"
            ),
            row=2,
            col=1,
        )
        ...
        if len(returns) > 100:
            ...
            market_returns = returns.rolling(
                window=60
            ).mean()  # Simplified market proxy
            correlation = returns.rolling(window=60).corr(market_returns)
            fig.add_trace(
                go.Scatter(
                    x=correlation.index, y=correlation.values, name="Market Correlation"
                ),
                row=3,
                col=1,
            )
    ...
    def _render_strategy_overlays(self):
        ...
        if overlay_settings.get("volume", False):
            ...
            fig_volume.update_layout(
                title="Volume", xaxis_title="Date", yaxis_title="Volume", height=300
            )
