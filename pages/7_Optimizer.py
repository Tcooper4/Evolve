...
try:
    from trading.data.providers import load_data, load_multiple_data
...
def main():
    ...
    if data is not None and not data.empty:
        ...
        st.sidebar.text(f"Date Range: {data.index.min().strftime('%Y-%m-%d')} to {data.index.max().strftime('%Y-%m-%d')}")
