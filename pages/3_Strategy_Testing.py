...
def _get_strategy_backend():
    try:
        ...
        from trading.strategies.ensemble import WeightedEnsembleStrategy, EnsembleConfig, create_ensemble_strategy
        ...
        return {
            ...
            "SMAConfig": SMAConfig,
            "CustomStrategyHandler": CustomStrategyHandler,
            "WeightedEnsembleStrategy": WeightedEnsembleStrategy,
            "EnsembleConfig": EnsembleConfig,
            "create_ensemble_strategy": create_ensemble_strategy,
            "MonteCarloSimulator": MonteCarloSimulator,
            "MonteCarloConfig": MonteCarloConfig,
            "BacktestEvaluator": BacktestEvaluator,
        }
    except Exception as e:
        import logging
        logging.warning(f"Strategy backend not available: {e}")
        return None

if "strategy_backend" not in st.session_state:
    st.session_state.strategy_backend = _get_strategy_backend()
_sbe = st.session_state.get("strategy_backend")
if _sbe:
    DataLoader = _sbe["DataLoader"]
    DataLoadRequest = _sbe["DataLoadRequest"]
    BollingerStrategy = _sbe["BollingerStrategy"]
    BollingerConfig = _sbe["BollingerConfig"]
    MACDStrategy = _sbe["MACDStrategy"]
    MACDConfig = _sbe["MACDConfig"]
    RSIStrategy = _sbe["RSIStrategy"]
    SMAStrategy = _sbe["SMAStrategy"]
    SMAConfig = _sbe["SMAConfig"]
    CustomStrategyHandler = _sbe["CustomStrategyHandler"]
    WeightedEnsembleStrategy = _sbe["WeightedEnsembleStrategy"]
    EnsembleConfig = _sbe["EnsembleConfig"]
    create_ensemble_strategy = _sbe["create_ensemble_strategy"]
    MonteCarloSimulator = _sbe["MonteCarloSimulator"]
    MonteCarloConfig = _sbe["MonteCarloConfig"]
    BacktestEvaluator = _sbe["BacktestEvaluator"]
else:
    DataLoader = DataLoadRequest = BollingerStrategy = BollingerConfig = None
    MACDStrategy = MACDConfig = RSIStrategy = SMAStrategy = SMAConfig = None
    CustomStrategyHandler = WeightedEnsembleStrategy = EnsembleConfig = None
    create_ensemble_strategy = MonteCarloSimulator = MonteCarloConfig = BacktestEvaluator = None
...
def _resolve_walk_forward_strategy(display_name, params_from_session=None):
    """
    Resolve strategy display name to a strategy instance for Walk-Forward.
    Uses same classes as Quick Backtest (Bollinger Bands, MACD, RSI, SMA Crossover).
    Returns (strategy_instance, None) or (None, error_message).
    """
    if not _sbe or not display_name:
        return None, "Strategy backend not available" if not _sbe else "No strategy selected"
    params_from_session = params_from_session or {}
    name_to_spec = {
        "Bollinger Bands": (BollingerStrategy, BollingerConfig, {"window": 20, "num_std": 2.0}),
        "MACD": (MACDStrategy, MACDConfig, {"fast_period": 12, "slow_period": 26, "signal_period": 9}),
        "RSI": (RSIStrategy, None, {"rsi_period": 14, "oversold_threshold": 30, "overbought_threshold": 70}),
        "SMA Crossover": (SMAStrategy, SMAConfig, {"short_window": 20, "long_window": 50}),
    }
    if display_name == "Bollinger_Bands":
        display_name = "Bollinger Bands"
    if display_name not in name_to_spec:
        return None, f"Strategy '{display_name}' not found. Use one of: {list(name_to_spec.keys())}"
    strategy_class, config_class, defaults = name_to_spec[display_name]
    merged = {**defaults, **params_from_session}
    try:
        if config_class:
            fields = getattr(config_class, "__dataclass_fields__", {})
            config_kw = {k: merged[k] for k in fields if k in merged}
            config = config_class(**config_kw)
            return strategy_class(config), None
        return strategy_class(**merged), None
    except Exception as e:
        return None, str(e)
...
'''
    ...
    with col2:
                        ...
                        if not validation_result["valid"]:
                            ...
                            if 'generate_signals' in test_namespace:
                                ...
                                with st.expander("📋 View Signals"):
                                    st.dataframe(signals[signals['signal'] != 0])
                            else:
        ...
        if handler.strategies:
            ...
            if selected_strategy and st.button("📂 Load"):
