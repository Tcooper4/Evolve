...
from trading.utils.visualization import plot_shap_values, plot_model_components
...
def show_shap_analysis(data, model_type, explainer_method="SHAP"):
    ...
    fig.update_layout(
        title=f"{explainer_method} Values Analysis",
        height=400,
        showlegend=False
    )
    st.plotly_chart(fig, use_container_width=True)
...
def show_model_components(data, model_type):
    ...
    if len(data) > 1:
        ...
        if len(numeric_cols) > 0:
            ...
            fig.add_trace(
                go.Scatter(y=trend, name='Trend', line=dict(color='red')),
                row=2, col=1
            )
