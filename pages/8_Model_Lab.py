...
try:
    ...
    # Single model registry: trading.models (used by Forecasting page and Model Lab)
...
if 'model_registry' not in st.session_state:
    try:
        st.session_state.model_registry = get_registry() if (MODEL_MODULES_AVAILABLE and get_registry) else None
...
if 'data_loader' not in st.session_state:
    try:
        ...
        try:
            from trading.data.providers import ProviderManager
                            ...
                            try:
                                ...
                                with col_save1:
                                    if st.button("💾 Save Model"):
                                        try:
                                            # Use session state for saved models (single registry has no save_model)
                                            ...
                                            st.session_state.saved_models[model_name] = {
                                                ...
                                                "trained_at": training_result.get("trained_at", datetime.now().isoformat()),
                                                "model": model,
                                            }
...
with tab2:
    ...
    if use_existing_data or st.session_state.training_data is not None:
        ...
        # Train Button
        train_button = st.button("🚀 Train Model with Configuration", type="primary")
                        ...
                        if st.button("💾 Save Model"):
                            try:
                                ...
                                st.session_state.saved_models[model_name] = {
                                    ...
                                    "trained_at": training_result.get("trained_at", datetime.now().isoformat()),
                                    "model": model,
                                    **({k: v for k, v in training_result.items() if k not in ("metrics", "trained_at")}),
                                }
...
with tab3:
    ...
    if data is not None:
        ...
        # Run Optimization
        optimize_button = st.button("🚀 Start Optimization", type="primary")
...
with tab4:
    ...
    if not available_models:
        ...
        if selected_model_name:
            ...
            with col2:
                if st.button("🔄 Re-evaluate Model"):
                    ...
                    try:
                        ...
                        if all('predictions' in available_models[m] for m in selected_models):
                            ...
                            if len(ensemble_actuals) > 0:
                                ...
                                st.dataframe(ensemble_comparison_df)
                            else:
...
with tab7:
    ...
    with registry_tab1:
        ...
        if sorted_models:
            ...
            st.dataframe(registry_df, height=400)
