...
def render_general_settings():
    ...
    with col1:
        ...
        if email_enabled:
            ...
            use_tls = st.checkbox(
                "Use TLS",
                value=st.session_state.alert_settings.get('email', {}).get('use_tls', True)
            )
    ...
    with col2:
        ...
        telegram_enabled = st.checkbox(
            ...
            value=st.session_state.alert_settings.get('telegram', {}).get('enabled', False)
        )
        ...
        if telegram_enabled:
            ...
            chat_id = st.text_input(
                "Chat ID",
                value=st.session_state.alert_settings.get('telegram', {}).get('chat_id', ''),
                type="default"
            )
    ...
    if selected_strategy:
        ...
        with col1:
            email_alerts = st.checkbox(
                "Email Alerts",
                value=strategy_settings.get('email_alerts', True)
            )
        
        with col2:
            telegram_alerts = st.checkbox(
                "Telegram Alerts",
                value=strategy_settings.get('telegram_alerts', True)
            )
        
        with col3:
            slack_alerts = st.checkbox(
                "Slack Alerts",
                value=strategy_settings.get('slack_alerts', False)
            )
