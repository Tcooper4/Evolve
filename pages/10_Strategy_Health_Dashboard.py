...
def get_risk_timeline(strategies):
    ...
    for strategy in strategies:
        ...
        timeline_data[strategy] = {"dates": dates, "var": var_timeline}
            ...
            alerts.append(
                {
                    ...
                    "message": (
                        f"Health score of {health['score']:.1f} requires attention. "
                        f"{health['issues'][0] if health['issues'] else 'Performance optimization needed.'}"
                    ),
                }
            )
