...
class EventLoopManager:
    ...
    async def submit_task_with_retry(
        self, coro: Callable[..., Any], *args, task_name: Optional[str] = None, **kwargs
    ) -> Any:
        ...
        for attempt in range(self._max_retries):
            try:
                ...
                result = await asyncio.wait_for(
                    coro(*args, **kwargs), timeout=30.0  # 30 second timeout
                )
                ...
                logger.warning(
                    f"â° Task {task_id} timed out, retrying in {delay}s (attempt {attempt + 1})"
                )
                await asyncio.sleep(delay)

            except RuntimeError as e:
                if "Event loop is closed" in str(e) or "no running event loop" in str(
                    e
                ):
                    ...
                    delay = self._base_delay * (2**attempt)
                    logger.info(
                        f"ðŸ”„ Retrying task {task_id} in {delay}s after loop recovery"
                    )
                    await asyncio.sleep(delay)
                else:
                    raise
    ...
    def run_in_executor_with_retry(
        self, executor: Any, func: Callable[..., T], *args, **kwargs
    ) -> asyncio.Future[T]:
    ...
    def is_healthy(self) -> bool:
        ...
        with self._lock:
            return (
                ...
                and self._loop not in self._crashed_loops
            )
...
def run_in_executor_with_retry(
    executor: Any, func: Callable[..., T], *args, **kwargs
) -> asyncio.Future[T]:
