...
class HybridEngine:
    ...
    def combine_forecasts(
        ...
        try:
            logger.info(
                f"Combining {len(forecast_results)} forecasts using {self.combination_method}"
            )
        ...
        for result in forecast_results:
            try:
                ...
                if "forecast" not in result:
                    logger.warning(
                        f"Forecast result missing 'forecast' field: {result.get('model', 'unknown')}"
                    )
                    continue
                ...
                if len(forecast_array) == 0:
                    logger.warning(
                        f"Empty forecast array: {result.get('model', 'unknown')}"
                    )
                    continue
                ...
                if np.any(np.isnan(forecast_array)) or np.any(np.isinf(forecast_array)):
                    logger.warning(
                        f"Forecast contains NaN or infinite values: {result.get('model', 'unknown')}"
                    )
                    continue
        ...
        try:
            ...
            for i, forecast in enumerate(forecasts):
                ...
                if outlier_ratio < 0.3:  # Less than 30% outliers
                    ...
                    logger.warning(
                        f"Removing forecast with {outlier_ratio:.2%} outliers: {forecast.get('model', 'unknown')}"
                    )
    ...
    def _calculate_weights(
        ...
        for forecast in forecasts:
            ...
            if model_name in self.model_performance:
                performance = np.mean(
                    self.model_performance[model_name][-10:]
        ...
        for forecast in forecasts:
            if "forecast" in forecast:
                try:
                    ...
                    # Log the error but continue processing other forecasts
                    logger.debug(f"Failed to process forecast: {e}")
                    continue
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        average_confidence = np.mean(
            [h["confidence"] for h in self.combination_history]
        )
        ...
        return {
            ...
            "recent_confidence": np.mean(
                [h["confidence"] for h in self.combination_history[-10:]]
            ),
        }
    ...
    def set_combination_method(self, method: str) -> None:
        ...
        if method in self.combination_methods:
            ...
            logger.warning(
                f"Unknown combination method: {method}. Using weighted_average"
            )
