...
def get_redis_client():
    ...
    return {
        "success": True,
        "result": redis.Redis(host="localhost", port=6379, db=0, decode_responses=True),
...
def get_agent_health_trends():
    ...
    try:
        ...
        return jsonify({"trends": trends, "timestamp": datetime.now().isoformat()})
...
async def get_active_alerts(security: SecurityManager = Depends(get_security_manager)):
    """Get active alerts."""
    try:
        # Check permissions
        if not await security.check_permission(request.user.id, "read:alerts"):
