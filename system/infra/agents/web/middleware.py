...
class AgentContext:
    ...
    def __init__(self, request_id: str, agent_id: Optional[str] = None, 
...
def inject_agent_context(f):
    ...
    def decorated_function(*args, **kwargs):
        try:
            ...
            context.add_trace('request_started', {
                'endpoint': f.__name__,
                'args': list(args),
            ...
            logger.info(f"Request started: {context.request_id} - {request.method} {request.path} - Agent: {context.agent_id}")
            ...
            try:
                ...
                context.add_trace('request_completed', {
                    'status': 'success',
                ...
                context.add_trace('request_error', {
    ...
    def decorated_function(*args, **kwargs):
        ...
        try:
            ...
            if context:
                ...
                context.add_trace('user_authenticated', {
...
def admin_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        try:
            ...
            if context:
                context.user_id = user_data.get('user_id')
                context.add_trace('admin_authenticated', {
                    'user_id': user_data.get('user_id'),
                    'role': 'admin'
                })
        ...
        if token:
            try:
                ...
                if user_data:
                    ...
                    if context:
                        context.user_id = user_data.get('user_id')
                        context.add_trace('user_injected', {
                            'user_id': user_data.get('user_id'),
                            'role': user_data.get('role')
                        })
