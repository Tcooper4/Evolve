...
class ReturnStatementAuditor:
    ...
    def should_function_have_return(self, node: ast.FunctionDef, content: str) -> bool:
        ...
        has_side_effects = any(re.search(pattern, function_text, re.IGNORECASE) 
    ...
    def generate_recommendations(self, priority_summary: Dict[str, Any]) -> List[str]:
        ...
        if critical_count > 0:
            recommendations.append(f"🚨 CRITICAL: Fix {critical_count} critical violations first (agents, models, services)")
        ...
        if priority_summary['medium']['count'] > 0:
            recommendations.append(f"📝 MEDIUM: Consider fixing {priority_summary['medium']['count']} medium-priority violations")
        
        if priority_summary['low']['count'] > 0:
            recommendations.append(f"💡 LOW: {priority_summary['low']['count']} low-priority violations can be addressed later")
    ...
    def print_report(self, report: Dict[str, Any]):
        ...
        print(f"\n📈 OVERALL COMPLIANCE:")
        print(f"   Files: {summary['compliant_files']}/{summary['total_files']} compliant ({summary['file_compliance_rate']:.1f}%)")
        print(f"   Functions: {summary['functions_with_returns']}/{summary['total_functions']} with returns ({summary['compliance_rate']:.1f}%)")
