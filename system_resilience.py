...
class SystemResilience:
    ...
    def register_fallback(self, component: str, primary_handler: Callable, 
                         fallback_handler: Callable, health_check: Callable,
    ...
    def get_performance_report(self) -> Dict[str, Any]:
        ...
        # Calculate averages
        cpu_avg = sum(m['value'] for m in self.performance_metrics['cpu_usage'][-100:]) / len(self.performance_metrics['cpu_usage'][-100:])
        memory_avg = sum(m['value'] for m in self.performance_metrics['memory_usage'][-100:]) / len(self.performance_metrics['memory_usage'][-100:])
        
        return {
            'cpu_usage_avg': cpu_avg,
            'memory_usage_avg': memory_avg,
            'recent_metrics': {
                k: v[-10:] for k, v in self.performance_metrics.items()
            }
        }
