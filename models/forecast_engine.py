...
class ForecastEngine:
    ...
    def generate_forecast(
        self, data: pd.DataFrame, model_type: ModelType, forecast_days: int, **kwargs
    ) -> ForecastResult:
        """
    ...
    def _prepare_data(self, data: pd.DataFrame, model_type: ModelType) -> pd.DataFrame:
        ...
        if model_type == ModelType.LSTM:
            ...
            prepared_data["Close_normalized"] = (
                prepared_data["Close"] - prepared_data["Close"].mean()
            ) / prepared_data["Close"].std()
    ...
    def _calculate_forecast_metrics(
        ...
        try:
            ...
            if not historical_data.empty and len(historical_data) >= len(forecast_values):
                ...
                self.logger.info(
                    f"Forecast evaluation - MSE: {mse:.4f}, MAE: {mae:.4f}, RMSE: {rmse:.4f}, R²: {r2:.4f}"
                )
    ...
    def _combine_forecasts(
        self, individual_forecasts: Dict[str, ForecastResult], weights: Dict[str, float]
    ) -> List[float]:
        """
    ...
    def _calculate_ensemble_metrics(
        self, ensemble_forecast: List[float], historical_data: pd.DataFrame
    ) -> Dict[str, float]:
        """
    ...
    def _trigger_retrain(self, model_id: str):
        ...
        try:
            ...
            self.logger.info(
                f"Retrain triggered for {model_id} - count: {self.model_performance[model_id]['retrain_count']}"
            )
