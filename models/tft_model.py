    ...
    def _prepare_data(self):
        ...
        for i in range(len(self.data) - self.sequence_length - self.prediction_horizon + 1):
            ...
            sequence = {
                ...
                'static': torch.FloatTensor(seq_data[self.static_features].values) if self.static_features else torch.zeros(1),
...
class TFTForecaster:
    ...
    def __init__(self, 
        ...
        logger.info(f"Initialized TFT Forecaster with sequence_length={sequence_length}, "
                   f"prediction_horizon={prediction_horizon}")
    
    def _prepare_data(self, data: pd.DataFrame, target_column: str) -> Tuple[TimeSeriesDataset, TimeSeriesDataset, TimeSeriesDataset]:
...
def create_tft_forecaster(data: pd.DataFrame,
